...
> 🎯 **RETOMAR = E6 cutover.** §209 (fix V6) + §210 (gates ②/③ 5/6) **MERGEADOS/LIVE**. Dueño verificando EN VIVO: atributos/aliados ✅; **3 fallos de render PÚBLICO → §211 fix (staged; cache bump → ver `05`)**: reseñas (link header), banner promocional (slot SP-1 restaurado), carrusel marcas (`load(true)` refetch) — regresiones SP-1/timing, **NO del cutover**. ⏳ Dueño: re-verificar público tras push + **verificar VEHÍCULOS en vivo** → desbloquea `vehicles` en gate (R-12/R-13). dealers F2 FROZEN (TODO-25). Follow-up: restaurar sección testimonios del home (diseño cinematic).
> 🏗️ **④ RBAC departamental (§193.4) — DISEÑO LISTO** (comité `winqw8w66`, crudo bóveda `2026-06-17-diseno-RBAC-*`): **pendiente presentar síntesis+decisiones al dueño + Gemini adversarial antes de implementar** (impl. tras gate vehículos). Clave: reusar **custom-claims de Bersaglio** (claim `role`+reconciler) + el blueprint departamental de la skill `crm-architect`; Altorra single-tenant → depto=`teamId`; dueño INAMOVIBLE (CEO §70/§72). Luego cutover (stub redirect global, NUNCA borrar admin.html) → E6.5.
