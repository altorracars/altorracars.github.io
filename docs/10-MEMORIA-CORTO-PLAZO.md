...
> **HECHO (detalle → brief; consolidar a ADR §NN):** pre-29d PASE-1·confirm.js·errors.js·gold-fantasma✅. **29/06 d DEPLOYED:** drift R8 (3-formas) + **§219 RBAC** (owner-only `users.*`/`roles.*`/`settings.backup-seo`/`audit.delete` NUNCA delegables ni el dueño en rol custom + subset; UI matriz "solo dueño"; suite 317/317; rules+functions live) + **accent picker ELIMINADO** (oro fijo `#D4A85A`).
> **HECHO 29/06 e-f DEPLOYED:** **(1) ✅ dataScope (P0-SEC-1, opción A)** — `scopeAllowsOwn` en leads/lead_intake/deals (asesor→own; dueño/all→todo; contactos/actividades compartidos); cliente `isAllScope()`+filtro ownerId; 2 índices; tests 220 (9/9), suite 326; deploy rules+indexes; dueño 'all' sin cambio. ('department'→'own' hasta denormalizar departmentId, latente sin asesores.) **(2) ✅ Perfil Telegram PORTADO** (status live onSnapshot). 2FA/recovery/trusted = login-MFA (TODO-43) → NO portar UI sola (inútil).
> **SIGUE:** P1 premium (emoji→SVG · voseo→tú-Colombia) · **Fase C** (mockups + design-system). **Owner-delete NO bloquea.** vestigial: perm `settings.theme` sin uso (limpiar).
