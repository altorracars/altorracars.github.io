...
> ✅ **OLA 1.1-1.8core CONSOLIDADA → ADR §268** (03-04/07; sesión cerrada por saturación de contexto — relevo curado).
> **▶️ ARRANQUE SESIÓN NUEVA (disparador: "continúa el plan maestro"):** boot normal → este `10` → plan §Ola 1 → **(1) 1.8b skeletons top-5** (contactos/vehículos/agenda/hub/usuarios; patrón `.skeleton` components.css:178, copiar de dashboard/inbox/deals/reportes que YA lo tienen) → **(2) 1.9** onboarding novato (checklist en Inicio con tenant vacío) + combobox typeahead en `convert-dialog.js:25` (el `<select>` muere a 100+ vehículos) → **(3) 1.9b** cola offline-rechazos captura → **OLA 2** en orden (2.1 Ctrl+K…). Cadencia probada hoy: implementar → build → preview `?mock=1` → commit+push+merge por bloque; suite emulador SOLO si tocas rules (L-63: si el puerto 8081 queda tomado, matar el java zombi).
