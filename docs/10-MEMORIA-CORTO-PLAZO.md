...
> **SIGUE (dueño dio GO 29/06):** **(1) dataScope (P0-SEC-1)** — reglas+queries scope rol (asesor→own+depto · gerente/CEO→all); ⚠️ RESTRINGE reads → emulador + alinear TODOS los `*.data.js` ANTES + deploy coordinado live-val (sin staging). **(2) Perfil paridad** — admin VIEJO tenía MÁS (**Telegram** notif. + **2FA/trusted/recovery**) NO portado → portar (cédula/doc SÍ está ya). · P1: emoji→SVG · voseo→tú · Fase C. **Owner-delete NO bloquea**. vestigial: perm `settings.theme` sin uso.
