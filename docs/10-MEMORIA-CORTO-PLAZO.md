...
| **TODO-38** | **🎨 REDISEÑO TOTAL F4/F5 ⟦OPUS-4.8⟧** — Bot ALTOR (widget) + ALTOR Hub (chat interno admin): de "arcaico" → moderno/premium/responsive/+funciones. **Módulo paralelo v2 flag-gated** (v1 intacto, A/B real), engine-agnóstico (LLM al final). Plan → `2026-06-24-EPIC-F4F5-rediseno-total-altor-PLAN.md` (Fases A-F). **Fase A track autónomo ✅** (24/06): mapa de contratos v1 verificado por código + inventario defectos + arquitectura (widget se auto-inyecta en body → flag decide en `components.js`; `admin-inbox.js` NO existe) → `…F4F5-faseA-discovery-contratos.md`. stitch+visualize conectados. | 🔄 Fase A | gate dueño: funciones nuevas + dirección visual → Fase B (mockups) |
