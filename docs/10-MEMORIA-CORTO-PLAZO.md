...
> **⚡ FABLE EJECUTA TODO hasta ~07-jul** (directiva dueño 03/07; Opus después). ✅ **OLA 1 CERRADA** (§269/§269.8) · ✅ **OLA 2: 2.1+2.2+2.3 → ADR §270** (Ctrl+K · campana/quick-create · perf server-side; detalle/gaps/backlog en los ADRs). ⚠️ el `dist/` se COMMITEA por bloque.
> **▶️ SIGUIENTE (disparador: "continúa el plan maestro"): OLA 2 en orden** — **2.4 export CSV por sección** (reusar `toCsv()/download()` de reportes: contactos/pipeline/vehículos/aliados/agenda) + **bulkbar reusable** (base `.inbox__bulkbar`) + **undo** toast 5-10s tras borrados → 2.5 agenda eliminar-cita → 2.6 seguridad P2 (tren rules+functions) → 2.7-2.12 (plan §Ola2). Cadencia: implementar → build → preview `?mock=1` → commit(+dist!)+push+merge por bloque; suite emulador SOLO si tocas rules (L-63: puerto 8081 = matar java zombi).
