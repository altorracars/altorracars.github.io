...
> EN CURSO: **F1.a ✅** (`f747f5e`: techo global gasto Anthropic-aware + memoria corta; `_brain.enabled=false`→LLM off→deploy F1 en UNIDAD; resto F1=App Check enforce+TTL justo antes de F3; App Check foundation ✅ `firebase-config.js:146`) · **F2.a ✅** (`2345138`: cédula fuera, mín nombre+celular, apellido/correo opcionales, saludo Colombia; **estática OK, live pend post-merge** [gate necesita Firebase, L-08]). 🔜 **F2.b = WhatsApp en gate** (reusa `handoverToWhatsApp`) + saludo bienvenida Colombia. **Pend dueño:** techo $/mes · retención TTL (recom. anonimizar 30d/borrar 90d) · consent legal abogado(P4) · merge p/verif-live · NO crear `config/altorCost` vía MCP (callejón e).
...
> (Histórico 12-22/06 §184-§231 → consolidado en `99`/`00`/`30`.) **22/06**: §229-§231 (rama única `dev` + pipeline Decisión Fuerte) · limpieza Firebase · TODO-34 diseño triple-validado.
> **23/06 (sesión EPIC ALTOR Hub v2)**: pipeline completo TODO-34 (verificó contratos · comité#2 arquitectura→B · Gemini→A · **veredicto Opción A** · comité#3 captura/UX/qualifier · Gemini-EPIC→reorden captura-antes-del-bot + 4 guardrails) → EPIC 6 fases + memoria `project-altor-hub-v2-epic`. **F1.a ✅** (techo global gasto + memoria corta) · **F2.a ✅** (cédula fuera + saludo Colombia). **Proceso-decision-fuerte v2** (comité meta: etapa arquitecto explícita + mapa de skills + gate empírico=lista cerrada de caminos). Skill `meta-ads-diagnostico` añadida ×4 repos + catalogada. Vault: 6 crudos committeados. **TODO-35 capturado** (anti-Knight-Capital, sesión fresca). Todo en `dev`. 🔜 F2.b.
