...
| §187 | **E5 EJECUTADA — blindaje**: **SEC-01** read estricto (8 colecciones canónicas a solo `crm.read`; pre-seed verificado con evidencia; 360 rotula permission-denied) · **SEC-06** whitelist hasOnly(34, censo LITERAL) + caps + shapes de maps en `solicitudes` (⚠️ presupuesto ~1000 expresiones de Rules) + subscriptions `consentGiven==true` + escapeHtml ×4 emails + Telegram fallback texto-plano + maxlength espejo en todos los forms + FIX soft-lead concierge (computeMeta reemplazaba) · **SEC-08** bookedSlots acotado (delete=super, público=1 clave+auth, self-heal de auth anónima + liberación de slot si el create falla) · `citas` legacy create:false · bootstrap de system con auth · **retry:true ×6 triggers** con guard tx.get(origen) + callback re-entrante + DLQ determinista protegido. Review adversarial: 21 confirmados corregidos/documentados. 189 tests. Residual: corrupción 1-fecha de cupos por anónimo-auth (→ App Check enforce ~16-23/06) | 43284 |
...
| §188 | **E6.6 EJECUTADA — auditoría del admin clásico** (workflow 16 agentes, 8 dim + refutación): 112 hallazgos, 2 refutados (presence con lector server-side index.js:1661 → widget público; wizard phase5 VIVO). Causa raíz de los KPIs envenenados = el portal nunca actualiza `solicitudes.estado` (D4-00). Mapa 21 secciones (muere/migra/mantener) · double-booking real por cita interna sin `kind` (fix paso 0.3) · PAT GitHub en localStorage · 14 riesgos cutover (stub redirect, NO borrar admin.html) · **plan strangler 29 pasos** → síntesis en bóveda `2026-06-12-e66-auditoria-admin-clasico.md` | 43297 |
| §189 | **E6 paso 0 EJECUTADO**: triggers `cacheSignal` (system/meta.lastModified server-side — llevaba 13 días stale, D7-01 confirmado) · `onUsuarioBloqueadoSync` (bloqueado→Auth disable+revoke; loginAttempts CERRADO — era world-writable) + portal expulsa bloqueados · **fix: E5 había ROTO la cita interna del clásico** (createdBy fuera de whitelist — 6º escritor no censado → L-41) + kind:'cita' (rebuild 5am borraba su reserva = double-booking) + convert legacy RETIRADO · desc-gen borrado + tag color-extract fuera. 192 tests. Cache `v20260612052500`. Deploy rules+5 functions ✅. Verificación live F39: próxima edición de inventario / walk-in / bloqueo real | 43310 |
| §190 | **E6 fase ② paso 1 — módulo Reseñas en el portal** (1er módulo público migrado, patrón validado): `modules/reviews/` shape VERBATIM del lector público + RBAC espejo reviews.* + mock + **`core/audit.js` NUEVO** (writeAudit best-effort, shape del clásico — no perder la auditoría que ya existía; semilla fase ④) + ruta `#/resenas` gated reviews.read. Preview mock: alta/edición/borrado ✓. SIN cache bump (L-32). **F39 live con doble valor**: crear reseña real desde el portal valida módulo + cacheSignal §189 en un gesto. Pendiente fase ②: banners (M) | 43322 |
