...
| **Branch activa** | `refactor/estructura` (en sync con `main` = `aa1698f`). §157–§166 LIVE en `main`. **§166 (Contactos — núcleo del CRM completo)** desplegada esta sesión (commit + merge + push `aa1698f`; sin rules/indexes/functions/cache). **Auto-deploy por fase autorizado** (memoria `feedback-auto-deploy-crm`). |
