...
> 🏗️ **CRM — plan E0→E6 (§176, TODO-21). E0 ✅ · E1a ✅ · adelanto E3 ✅ (§180) · E1b ✅ (§181)** — Pipeline v2 COMPLETO y deployado: etapas v3 con gates (Rules+UI+**paridad 7×7**), trigger `onDealUpdated` (E2E live: chip proyectado en 3s + lifecycle + activity determinista — **la divergencia Bandeja↔Pipeline es IMPOSIBLE**, triple capa), diálogo de conversión F7 + `anularConversion` (deshacer server-side), undo 10s, descarte con razón. F37 confirmado por su cron real (`sentTelegram:true` — el push llegó al dueño). RESTORE ensayado en emulador ✅ (gate cumplido). Migración v3 = no-op verificada (0 docs v2). **E2 tanda 1 ✅ (§182)**: F21.1 auditoría (configs DIVERGEN — la web vende citas en FESTIVOS) · F16 proyección citas→Agenda (E2E live 5s) · `crmDailyJob` 5am (backup diario + backfill + rebuild cupos + purga + digest — **verificar su 1ª corrida en `crm_alerts` al retomar**: digest con fantasmas eliminados). **⏸️ RELEVO por saturación (~884k, §183)** — retomar en sesión NUEVA con: **"continúa E2 tanda 2"** = F21 editor de disponibilidad en el portal (festivos CO→`blockedDates` que citas.js YA lee + maxPerSlot + overrides por asesor) **JUNTO con** el cambio del lector del validador viejo (separados = regresión de festivos; cache bump js/admin) → F18/F19 acciones de cita vía callable transaccional (bloques 30min, tupla asesor+vehículo al CONFIRMAR, token WhatsApp-first ROTADO al reprogramar, hold-expiry T-3h, no_show con follow-up) → F20 patas restantes del job horario → F28 invariantes completo. **Decisiones nuevas del dueño (§183)**: agenda del clásico muere EN el cutover (decisión delegada a Claude, holística) · **E6.5 comité de diseño del panel** al final (FIRME) · **E6.6 auditoría total del admin clásico** post-migración. ⏳ Cliente: merge+push hecho ✓; (1) borrar `leads/_test_sla_e1a` (⋯→Eliminar definitivo); (2) F42; (3) billing.
>
