...
> 🏗️ **CRM — plan VIGENTE = Comité CRM v2, épicas E0→E6 (ADR §176, TODO-21)**. Núcleo completo + E2E live ✅ (§175). El comité (33 agentes ×3 rondas) confirmó las 3 quejas del dueño (CERO sync lead↔deal · sin CRUD · calendarios desconectados + cupos que nunca se liberan) y entregó el plan: **VEREDICTO + checklist de épicas → bóveda `2026-06-09-comite-crm-v2-VEREDICTO.md`** · manual novatos → `docs/MANUAL-CRM-USO.md` (v1 = UI actual; evoluciona por one-pagers de sprint). Próximo paso: **OK del dueño a 3 decisiones** (estados lead v3 · etapas pipeline v3 · política F42 comisiones) → arrancar **E0** (red de seguridad + F17-urgente cupos). ⏳ Pendiente del cliente: (1) **merge+push a `main`** (fix §175 + cerebro + manual); (2) verificar la CAUSA del billing-disabled de hoy (¿tarjeta/aviso GCP en console.cloud.google.com/billing?) para que no se repita.
>
