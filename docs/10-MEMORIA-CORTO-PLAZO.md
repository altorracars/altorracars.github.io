...
> ▶️ **RESUME (sesión nueva)**: F-0.5 HECHO = `admin-app/firebase.js` multiTab+RTDB (build-verif; **dist NO reconstruido = SIN deploy aún**). Sigue: F-0.5 resto (**bot v2 en Vite** + montar staging + clean-slate E2E) → **F-2 Config admin-app (Usuarios/Roles — net-new, cero riesgo, EMPEZAR AQUÍ)** → F-3 Dashboard → F-4 Hub→admin-app → F-5 fugas (dedup `session:ID`, §235.7 Fase2) → F-6 cutover (unregister SW zombie + bridge auth). Detalle → §237.7 + plan §6.
...
| **TODO-39** | **🧭 PLAN UNIFICADO un-solo-panel-admin ⟦OPUS-4.8⟧ — FOCO MAESTRO** (spec `…PLAN-UNIFICADO…`). Portal único `admin-app/`, apagar `admin.html`. Survey+comité+Gemini verif (6 hallazgos). F-0.5 iniciado; camino F0..F6 → foco/RESUME arriba. **F4/F5 (bot v2) consolidado §236, su resto = F-1 aquí.** | 🔄 F-0.5 | dueño: dinero/legal/go-no-go |
...
> Histórico §184-§237 → `99`/`00`/`30`. **24/06 (F4/F5 + PLAN UNIFICADO)**: F4/F5 bot v2 Web Component + fixes live #7 gate-takeover/ícono WhatsApp + validación live (extensión Chrome manejada por mí) → **§236**; **survey 5-agentes (`wf_668bd020-e80`, in-cwd, NO se colgó) + comité + Gemini verif claim-a-claim → PLAN UNIFICADO §237** (Decisión Fuerte, foco maestro, Gemini revirtió su verdicto Hub); **F-0.5 iniciado**. Defectos vivos → `altor-hub-rediseno-defectos.md` (#3/#4/#5/#6/C#1-C#4).
