...
> 🏗️ **④ RBAC departamental (§193.4) — ④a COMPLETO (PASO 0-6)**. Blueprint ④a (bóveda) = **SSoT**. PASO 0-4+§217 (§213/§215/§216/§217/§218) + **§219: PASO 5** (`onUserRoleAssigned` siembra `role.nivel`→`user.nivel` SOLO si falta vía `computeNivelSeedOnAssign`, NO pisa en resync; desplegada) + **PASO 6** (paridad dual-portal materializa `nivel`/`departmentId`/`departmentName`/`dataScope`; cosmético, sin consumidores aún). Adversarial 3/3 SAFE. **Pend dueño → `05`**. **④b** (scoping `inDeptScope` + `nivel`→int) GATEADO: Gemini + negocio con ④a VISIBLE. **🔑 GATE ④b (concern §219): ANTES de enforce `nivel` añadir FLOOR server-side** (actor solo asigna roles de nivel < el suyo; acotar `role.nivel` en rules) — si no, el seed = escalada.
