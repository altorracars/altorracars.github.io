...
>   - **EJECUCIÓN (plan v5 APROBADO por el cliente 2026-06-09)**: **PRE-PASO-0 parte 1 ✅** — `brain-check.mjs` migrado de caps-líneas → **caps-chars** (dual) + sub-presupuesto de BOOT always-on (CLAUDE+05+10 = 44.173c/~12.6k tok vs objetivo 31.500c/~9k tok → `↗ destilar`). Nodo 10 (16.289c) = **target #1 de destilado**. Build verde (exit 0). **Próximo: (a)** partir linter `--boot`/`--full` (que SessionStart NO lea el 99 ~43k líneas); **(b)** PASO 0.(-1) blindar base git; **(c)** PASO 0 mecanismo en sandbox/inmobiliaria. 🚫 Callejones: §R6 skills clasificó por-nombre (mal) → leer SKILL.md; NO junctions/symlink (spec §170 L40 + verificado: skills/ vendor ≠ global de 7).
