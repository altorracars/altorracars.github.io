...
| §259 | **Aliados: MVP comisiones DESPLEGADO ⟦OPUS⟧** — §9 vehículo (tenencia+método)→venta→reporte (comisión real por aliado, mata $0). rules+functions+UI en prod. FUTURO: consigna=entidad·fiscal·facturación TODO-26. | 44263 |
| §260 | **TODO-52 P1: emoji→SVG premium (16 módulos) ⟦OPUS⟧** — sistema `icons.js`+`navIcon`; doctrina iconografía (deja 👋🇨🇴🎉🏖★☆✔ / convierte UI genérica); voseo→tú; L-60 ×3. RESTA: módulos restantes + iconos-dominio + Fase C. | 44275 |
