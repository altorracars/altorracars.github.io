...
> EN CURSO (todo en `dev` · detalle → spec §EPIC): **F1.a✅ · F2✅** (cédula fuera · WhatsApp gate · voz Colombia · botones) · **F3-a✅+F3-b✅** (Tool Calling: search_inventory + submit_lead→CRM consent-conservador) · **TTL✅** (anonimizar dry-run) · **wiring v2✅** (`engine:'v2'` cohorte). ⚠️ **LLM apagado** (`_brain.enabled=false`) + **functions PEND DEPLOY** (todo dormiente → prod corre v1, seguro). **Decisiones dueño 23/06:** techo $15 · App Check MONITOR · TTL anonimizar@30d · **GO al flip**. 🔜 **FLIP**: dueño mergea `dev`→`main` → YO deploy+`_brain.enabled=true`+verifico (kill+$15+fallback). **Pend dueño:** consent EXPRESO (P4 abogado) · merge.
