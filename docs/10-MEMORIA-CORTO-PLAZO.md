...
> 🏗️ **HANDOFF → SESIÓN NUEVA: RECONSTRUCCIÓN DEL CRM (arrancar en contexto limpio, 2026-06-02).** El cliente cierra esta conversación para abrir una fresca y arrancar fuerte con el CRM.
> **Objetivo**: reconstruir el CRM de Altorra (concesionario de carros usados, Cartagena).
> **Framework**: skill **`crm-architect`** (añadida, commit `6cc0055` → `skills/crm-architect/`). Calza el stack EXACTO de Altorra: Firebase + Firestore + Cloud Functions, data-model, pipeline de ventas, automatización, AI (scoring/copilot/NBA), reporting, integraciones (email/WhatsApp/calendar), **RBAC + Ley 1581 (Colombia)**. Trae templates listos (`assets/templates/`: firestore.rules, schema.types.ts, functions, design-tokens), scripts (`scaffold_crm.mjs`/`generate_module.mjs`/`validate_crm.mjs`) y **vertical `references/verticals/automotive-dealership.md`**. Registrada en `40`.
> **CRM ACTUAL (punto de partida — leer ANTES de reconstruir)**: `admin.html` (SPA) + `js/admin/admin-crm.js` + `js/admin/admin-crm-tabs.js` + `js/core/comm-schema.js`; Firestore `solicitudes/` (leads/citas), `clientes/{uid}` (+subcols crmNotes/cotizaciones/postventa), `mensajes/`, `usuarios/`; RBAC dinámico §61. Mapa → `20-ESPACIAL`.
> **PRIMEROS PASOS (sesión nueva)**: (1) invocar skill `crm-architect` + leer su `SKILL.md` + `automotive-dealership.md`; (2) invocar `brainstorming` para definir el ALCANCE ("reconstrucción" = ¿rebuild total? ¿modularizar el admin? ¿features nuevas? ¿qué duele del CRM actual?); (3) auditar el CRM actual vs el framework; (4) `writing-plans` → ADRs por sprint. **🚫 Callejones sin salida**: ninguno aún.
> **PREVIO CERRADO** (solo referencia, NO tocar): catálogo cinematic §122–§146 + a11y §147–§149 + diseño §150 (dropdown §150.d–f) + cerebro V5-lean/tooling §151–§154 + skill CRM — todo COMMITEADO (HEAD `6cc0055`), consolidado en ADRs `99`+`00`. **§150–§154 ya en `main`** (PR#788); solo la skill CRM + estos docs de handoff quedan local. Cache `v20260602140000`. `brain:check` SANO (frescura + refs cruzadas).
...
> 🗂️ **Todo lo de abajo (§140–§150.c + a11y §147/§148/§149) ya está CONSOLIDADO en ADRs (`99`) + `00-INDICE`, commiteado y §150 DESPLEGADO a `main`.** Los `⏳`/«SIN commit»/«pedir screenshot» eran ciertos EN SU FECHA — hoy NO son pendientes vivos (el dropdown se resolvió en §150.d–f). Seguimiento real → «Foco actual». Vaciable en la próxima poda.
...
- **2026-06-03 · Auditoría skills↔cerebro (a pedido del cliente, ANTES del CRM)**: mapeadas las ~88 skills de `skills/` → **hoja nueva `docs/skills-inventory.md`** (catálogo por categoría) + wiring (CLAUDE.md §0, `40 §Recursos Externos` con estructura real corregida, ruta en `00`). `brain:check` SANO. **Verdad clave**: el repo `skills/` NO es la fuente de mis skills cargadas (config solo habilita plugin `superpowers` + `~/.claude/skills/crm-architect`; `anthropic-skills:*` es bundle del entorno) → solape de nombres = curaduría, no cableado. **Pendiente decisión cliente**: 7 anomalías de repo (carpeta `ecommerce skills` con espacio, `SKILL-canvas-design.md` mal nombrado, `accessibility-audit-workspace`/`example-plugin` ruido, `code-modernization`/`code-simplifier` mal-clasificadas, sufijos `-main`) — detalle+recomendación en la hoja. NO tocadas (destructivo → requiere OK). SIN commit.
- **2026-06-03 · Protocolo CONSEJO EXTERNO (Antigravity/Gemini) MAPEADO**: neurona nueva `15-CONSEJO-EXTERNO.md` (red team multi-modelo: cuándo pedir crítica adversarial + matriz de selección Pro/Flash + anti-anclaje + plantilla de prompt + degradación por tokens + límites). Registrada en `CLAUDE.md §0` (boot-aware) + ruta en `00`. Disparador = decisión cara de revertir (arquitectura/datos/seguridad/fork 50-50/op irreversible). Cliente delegó selección de modelo (tiene Flash Low/Med/High + Pro Low/High). `brain:check` SANO (⚠️ CLAUDE.md 319/320 — próxima adición exige condensar §G.5). SIN commit.
- **2026-06-03 · Regla "verifica, no asumas" GENERALIZADA** (feedback cliente: asumo a veces). `§3.3` pasó de RCA-código → **UNIVERSAL** (evidencia antes de afirmar cualquier hecho) + gate; **M-11** en `30 §Meta` (recidiva, linaje M-02/04/06/10). Era una regla que existía pero mal-alcanzada. `brain:check` SANO. SIN commit.
- **2026-06-03 · Tooling determinista (hooks) + limpieza skills**: (1) **git pre-commit** (`githooks/pre-commit` + `core.hooksPath=githooks`) corre `brain:check` y BLOQUEA el commit si el cerebro está roto — probado feliz+bloqueo. (2) **SessionStart hook** (`.claude/settings.json`) corre `brain:check` al iniciar CADA sesión (activa en sesión nueva, no la actual). (3) Anomalías skills **4/7**: borradas `accessibility-audit-workspace`+`example-plugin`; renombradas `ecommerce skills`→`ecommerce/`, `SKILL-canvas-design`→`canvas-design-creative/SKILL.md`. Quedan 3 (code-modernization/code-simplifier=contenido real no-borrado; bundles `-main`=cosmético). `brain:check` SANO. SIN commit.
