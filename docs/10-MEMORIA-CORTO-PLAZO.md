...
> **🤝 TODO-54 PageSpeed (§296, spec `2026-07-08-pagespeed-audit-plan.md` = SSoT)** — 08/07 3ª ⟦OPUS⟧ dieta-JS: **2.1b ✅ `3b11ab10`** (diferir auth+SDKs; `auth/iframe.js` off-critical; nuevo `window.dbReady`; home-only) · **2.3 ✅ `73aadeaa`** (bot/IA ~18 archivos a idle) · **2.4 ✅** (GSI a idle; NO FedCM-off → preserva One-Tap) · **2.6 ✅** (reflujo parallax: scrollY al rAF) · **2.5 ✅ `12d51274`** (Lucide 82KB a idle, home-only; es defer no subset). **FASE 2 dieta-JS ~COMPLETA** (2.2 diferido: ya idle P14). **1.3 slice ✅ + llms.txt ✅ `bee56f0f`**. **🔬 1.3-core CERRADO empíricamente**: el above-fold móvil usa ~113KB/460KB (diseño rico) → critical-CSS NO es win limpio; el fix correcto = **pipeline de build (Cloudflare F4)**. **RETOMAR = TODO lo perf que queda está gateado en Cloudflare** (build → minify+critical+Brotli+caché; **gate = dominio ~$10/año = decisión DINERO del dueño**) · o 1.1b/1.2 (visual/equipo). Sin bump (js/core+public/home=networkFirst/SWR).
