...
> **SIGUE (dueño dio GO 29/06):** **(1) dataScope (P0-SEC-1)** — reglas+queries scope rol (asesor→own+depto · gerente/CEO→all); ⚠️ RESTRINGE reads → emulador + alinear TODOS los `*.data.js` ANTES + deploy coordinado live-val (sin staging). **(2) ✅ Perfil: Telegram PORTADO** (29/06 e, status live onSnapshot; verif. no-vinculado; vinculado=live-val dueño). 2FA/recovery/trusted = atados al login-MFA que el admin nuevo NO tiene (TODO-43) → NO portar UI sola (sería inútil). cédula/doc ya estaba. · P1: emoji→SVG · voseo→tú · Fase C. **Owner-delete NO bloquea**. vestigial: perm `settings.theme` sin uso.
