...
| **TODO-35** | **Código muerto (anti-Knight-Capital) ⟦OPUS-4.8⟧.** P0 `deadcode:check`✅ (26/06: **0 huérfanas**, 60src/59depl) + P1 skill✅ + propagada✅. **Huérfanos del cutover F-6**: `manifest-admin.json` + `js/admin/*` (los cargaba SOLO `admin.html`, ya en `_legacy/`). **DIFERIDO a ~2026-07-03** (≥1 sem post-flip): cuarentenar AHORA rompe a asesores con `admin.html` viejo CACHEADO que aún cargan `js/admin/*` (skill §4 cache-cliente; NO están en SW precache pero sí en cache de navegación). M-19. | 🔄 P0+P1✅ · admin-deadcode diferido | ~03/07 (cache) |
