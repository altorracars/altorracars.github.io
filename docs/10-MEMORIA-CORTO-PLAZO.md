...
> 🏗️ **CRM — ejecutando plan E0→E6 (§176, TODO-21). E0 ✅ (§177) · E1a núcleo ✅ (§178)** — deployado y verificado live: F1 inmutable (CEO denegado) · F17 cupos (ciclo completo en prod) · ⚡ lead rápido `lead_intake` offline (E2E live: ingerido en 2s, +57, owner forzado, consent 1581) · quick-log 📞 + próximo paso + 📋 Pendientes hoy · manual v2 con F42. **Falta de E1a** (próx. sesión): F37 (job horario SLA 2h + rotación intake) · F38 (routing alertas FCM/Telegram) · F33a (instrumentación fricción). **Luego E1b** (pipeline v2 = la migración de enums; gate: ensayo RESTORE en emulador). ⚠️ El UI nuevo del portal llega a prod con el **merge del cliente** (PR pendiente de la rama). ⏳ Cliente: (1) merge+push; (2) anunciar F42 al equipo (texto en manual §2); (3) CAUSA del billing-disabled (console.cloud.google.com/billing).
>
