...
| §209 | **Verificación INICIAL técnica lote V6 + fix blocker walk-in ⟦OPUS-4.8⟧** (workflow 17 agentes verify→refute adversarial): 6/8 limpias; blocker `createManualLead`→`solicitudes` permission-denied para TODOS (regresión SEC-06 §187, REINCIDENCIA L-41) → fix rama admin §209 (`crm.edit`+`_createdByUid` anti-spoof, append-only, público INTACTO) + new-lead optimista + reasignar rollback (§3.6). Test e6 emulador 5/5; suite 198/198; build ✓. | 43537 |
