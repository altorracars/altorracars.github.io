...
- ✅ **P3.4 LCP páginas de marca** (§291, `b345e703`): `#brandBanner src=""` (JS lo llena tras fetch) sin preload → LCP tardío. Fix SSG (como §287): hornea preload + `#brandBanner src`. Selftest OK; propaga vía CI. **Pendiente**: los 18 banners `b_*.png` son PNG grandes (73-331KB) → optimizar a WebP/AVIF + `<picture>`.
