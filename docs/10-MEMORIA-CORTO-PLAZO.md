...
> Histórico 12-22/06 §184-§231 → `99`/`00`/`30`. **23/06**: EPIC TODO-34 → **Opción A** (`dev`: F1.a·F2·F3-a ✅ dormiente, pend deploy; 🔜 F3-b). **Validador Chrome** (skill `validacion-live-chrome` ×4+global; gate `skills:desc-check`): §232 creada · §233 (`_asesorJoinedAnnounced`) + §234 (fuga privacidad en logout) **cerrados LIVE ✅**. Skill caza adversarial + sesión + diseño/UX + consistencia + **copy/flujo comercial** + escala a comité/Gemini gateado. **TODO-37 ✅ §235** (P0 leads: Decisión Fuerte completa verif→comité→Gemini→impl Fase0+1 Option G→re-validado LIVE; lead completo en CRM, sin dup, Write 200). Bug flujo cazado+fijo (`02a79a7`: fechas duplicadas en gate→`quickReplies:[]`). Defectos diseño/flujo → `altor-hub-rediseno-defectos.md`. grounding=LLM off.
