...
const CACHE_VERSION = 'v20260511020000'; // §41 — FIX DEFINITIVO subnav invisible. Causa raíz: admin-section-router.go() PATH A clickeaba el nav-item legacy del sidebar, que activaba la sección DOM-direct via admin-auth.js:1927-1946, pero ese handler legacy NUNCA llama notifyChange. Sin notifyChange, los subscribers (admin-topnav setActiveSection → renderSubnav) jamás corrian. Resultado: el subnav (Inventario/Hub/Config) permanecía hidden=true para siempre. FIX: en PATH A, después de btn.click(), explicitar _currentSection update + hash update + notifyChange(section, prev). Subnav ahora aparece correctamente para Vehículos/Marcas/Aliados/Banners/Reseñas (Inventario), ALTOR Hub/Cerebro AI/Lo que no entendí (Hub), y Usuarios/Atributos/Workflows/Auditoría/Ajustes (Config). // §40 — UI Telegram en sec-profile + force refresh subnav.
