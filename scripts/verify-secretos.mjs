 ...
 * publicación: `altorracars.github.io` sirve el árbol entero bajo el dominio propio y GitHub lo
 * indexa, así que una clave subida está cosechada antes de que nadie la note. Y no basta con
 * borrarla después — queda en el historial de git para siempre; lo único que sirve es que NO ENTRE.
 *
 ...
 *
 * LO QUE MIRA, y por qué esos. El despliegue usa cuentas de servicio de Google (Firebase/GCP), las
 * Functions manejan credenciales de terceros y el harness guarda tokens: los patrones cubren lo que
 * este proyecto tiene de verdad en las manos, no una lista genérica copiada de internet.
 *
 ...
 * gitignoreadas (`node_modules`, `backups`, `orphans_backup`, `.playwright-mcp`, `.superpowers`) ya
 * no aparecen solas y quedan aquí como cinturón: si alguna se versionara algún día, la exclusión
 * sigue siendo la decisión escrita.
 *
 * 🚫 NO están aquí, y es deliberado: `dist` — porque en ESTE repo `admin-app/dist/` SÍ se versiona
 * y GitHub Pages lo sirve (el .gitignore lo des-ignora explícitamente); excluir «dist» a secas
 * dejaría ciego un bundle PUBLICADO. Y `_legacy` — que aquí está versionado y por tanto también se
 * publica: una credencial en código retirado se cosecha igual que una en código vivo.
 */
const FUERA = new Set([
  ...
  'backups',        // exportaciones locales de Firestore con datos reales; gitignored a propósito
  'orphans_backup', // respaldo reversible de fotos huérfanas; gitignored
  '.playwright-mcp',
  '.superpowers',
]);
 ...
 * quedó solo con el sitio): 484 ficheros de texto barridos (antes 972, medidos el 31-ago-2026 con
 * docs/, skills/ y el kernel dentro). El piso viejo de 400 seguía pasando, pero por 84 ficheros:
 * un umbral se calibra contra el árbol que existe. Piso nuevo = la mitad de lo medido, como siempre.
 * RE-CALIBRADO el 2026-09-15 (S7-01): con la denylist de binarios en vez de la lista blanca de
 * extensiones, el barrido pasa de 483 a 602 ficheros. El piso viejo (240) seguia pasando, pero estaba
