...
| §197 | **Gap 7 Agenda — acción `update` en crmCitaAction**: observaciones + **REASIGNAR asesor sin cambio de estado** (helper `moveAdvisorBlocks` en `_internals`: entrante libre → libera saliente + reserva entrante MISMA tx; vehículo intacto; pendiente no retiene tupla C.2; confirmedAt/token NO se tocan) · UI "✏️ Editar / reasignar" en openCitaDetail. **Suite emulador 193 ✓** · deploy ✓. Transversal desbloqueado = VACÍO | 43406 |
| §198 | **F39 v2 (decisión del dueño)**: la verificación EN VIVO la ejecuta **Claude por LOTES en hitos** (no el dueño, no por merge) — tests/emulador/preview por cambio NO se relajan · comité/Gemini en entregas grandes · gates ②/③ se cierran tras el lote post-épica-vehículos (7 gestos + ciclo completo de vehículo). + M-14: el corte de sesión lo decide el dueño con números reales | 43418 |
| §199 | **ÉPICA VEHÍCULOS — mapa + V1**: workflow 7 lectores (~950k tok; crudo + síntesis con 11 decisiones de port y plan V1-V6 en bóveda `2026-06-12-epica-vehiculos-plan.md`) · **domain/vehicle.js** (Smart Fields VERBATIM: 7 RULES en orden, penalty acumulativo, 8 advisory; deriveTipoFromKm; daysInStock 60/90) · **`#/vehiculos`** lista (orden prioridad DESC/id ASC, búsqueda/filtros/badges, destacar con featuredWeek espejo + audit, delete con log previo y Storage intacto) · markAsSold NO se porta (venta=pipeline; R-12). Preview mock ✓. Siguiente: V2 wizard | 43427 |
| §200 | **Épica vehículos V2 — wizard 6 pasos**: `buildVehicleDoc` PURO (shape ~45 campos, derivados verbatim, _smartDerived) + AUDIT_FIELDS/computeChanges · create tx (counter ALT- + anti-colisión id, _version:1) · **update tx compare expectedVersion → version-conflict → SHAPE COMPLETO** (gate anti-vehicleAggregate) · wizard: required al avanzar, edición=todo visible, apartado disabled-persistible, vendido Fase 22, features value-largo + saveList merge, URLs validadas, smart preview live (score 95 exacto en mock). Fotos upload = V3 | 43438 |
...
| §202 | **Épica vehículos V4 — borradores**: `usuarios/{uid}/drafts` shape = KEYS DEL FORM CLÁSICO (interop; vCodigoUnico jamás) · guardado explícito OPTIMISTA + rollback completo · cierre = modal custom contra DOBLE baseline · retomar RE-DERIVA tipo desde km (§E.3) · publicar borra el draft · listener sin orderBy + retry 1200ms · panel 📝 Retomar/Eliminar optimista. Mock ✓ ciclo completo | 43454 |
