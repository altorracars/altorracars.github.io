...
> 🏗️ **④ RBAC departamental (§193.4)** — blueprint ④a (bóveda) = **SSoT** (pasos+congeladas+④b-gate; LEER al retomar). DEPLOYADO: PASO 0/§213 · PASO 1 (catálogo Departamentos; DUPLICADO `rbac-catalog.js`↔`functions`) · **PASO 2/§215** (callable `backfillNivelesRBAC` + botón sec-roles; ⏳ dueño 1 clic "Backfill niveles" tras merge) · **PASO 3/§216** (rules `departments/` 3a deployado + UI `admin-departments.js` 3b: CRUD DOM-puro, nav/sección/script/GROUPS cableados; clases reales verificadas L-37). **SIGUIENTE PASO 4**: `admin-users.js` asigna `departmentId/nivel/dataScope` + mantiene `userCount` → 5 §71-nivel → 6 dual-portal. Helpers nivel/dept/scope → ④b. **§215.7/§216.7**: PASO 4 verificar `users.edit`↛`'*'` a tercero · ④b `nivel`→int.
