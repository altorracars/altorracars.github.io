...
> **✅ Fase 2 DESPLEGADA (2026-06-06)**: commit `3419efe` → merge a `main` (`73c69ea`) → push origin (Pages sirve `/admin-app/dist/`) + `firebase deploy --only firestore:rules` (subrule `crmNotes`) LIVE. **⏳ Único pendiente**: **E2E live** — abrir `https://altorracars.github.io/admin-app/dist/` + login con cuenta admin + confirmar que la Bandeja muestra los leads reales del canónico (L-08: solo verificable contra el deploy, no localhost). (Opcional: budget alert $5.) **Sincronizar `refactor/estructura` con `main`** en la próxima sesión.
