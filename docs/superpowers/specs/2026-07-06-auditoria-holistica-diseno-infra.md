...
- ✅ **Barrido responsive del panel — COMPLETO 2026-07-08 (§294)**: auditoría objetiva (overflow de página + clips FUERA de scrollers) de las ~22 pantallas en **4 bandas de ancho** (390/600/768/1440px, Vite `?mock=1`). **3 clips reales corregidos** (Bandeja card · Agenda header 155px · Vehículos title `flex+min-width:0` colapsa; L-70); el resto LIMPIO en todas las bandas. 🚫 **Diferido — Agenda vista-mes en móvil**: los chips de evento recortan ~36px@390 / ~16px@600 (la hora no cabe en la celda de mes). Callejón probado: achicar padding/font del chip solo baja 36→19px, NO lo resuelve → el fix real = **vista día/lista en móvil** (feature UI = mockup+aprobación, flujo W-11). Base responsive del panel = SÓLIDA.
