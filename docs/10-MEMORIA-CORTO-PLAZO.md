...
> 🎯 **E6 cutover: gates ②/③ COMPLETOS (6/6)** — §209 (fix V6) + §210 (gates; vehículos tras R-13/R-12 + dueño verificó leads/creación en vivo) + §211 (web pública) + merge L-02 (cache → `05`) **en branch, pend. push del dueño** (push → Ctrl+Shift+R → re-verif público). Vender=Pipeline (financiero completo=TODO-25). dealers F2 FROZEN. Follow-up: testimonios home (diseño).
> 🏗️ **IMPLEMENTANDO ④ RBAC departamental (§193.4)** — **blueprint ④a en bóveda `2026-06-18-RBAC-4a-blueprint.md`** (LEER al retomar; plan completo PASO 0-6 + decisiones congeladas). HECHO+DEPLOYADO: **PASO 0/§213** (capa CF dueño-inamovible `isOwnerData`, §212→3 capas) + **PASO 1** (catálogo 'Departamentos' `departments.read/manage` + `nivel:100` CEO; sync `rbac-catalog.js`↔`functions` v1.1.0 — el catálogo está DUPLICADO, no fetch-dinámico, editar AMBOS). ⏳ **PASO 2 = correr `seedSystemRoles`** (callable super_admin desde sec-roles; siembra los perms nuevos + nivel CEO). **SIGUIENTE PASO 3** rules (helpers + colección `departments/`) → 4 sec-users selectores → 5 §71-nivel → 6 dual-portal. **Congeladas**: doc-read (claims→Fase5), `permissions[]` se queda, `nivel` per-usuario (CF no lo pisa), `dataScope` NO-enforce-④a, rules=la-frontera. **④b GATEADO** (Gemini + ¿visibilidad de datos o solo agrupar? Bersaglio retrocedió a roles planos → decidir con ④a VISIBLE). Luego cutover → E6.5.
