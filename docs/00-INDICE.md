...
| §206 | **Auditoría Nivel-2 del cerebro ⟦OPUS-4.8⟧** (ítem U comité v6, 29 agentes) — costo boot + monolitos (00 sobre cap) + §G load-bearing. Plan A0-A10: shard 30→33, crear 11-GOBERNANZA (cross-repo ×3 + Gemini). A0 ✅ | 43496 |
| §207 | **Auditoría Nivel-2: ¿el lazo funciona o es teatro? ⟦OPUS-4.8⟧** (50 agentes; 37/40 confirmados) — veredicto: MECANIZA o es TEATRO (confirma M-10/M-16: 05/10 mienten sobre git). 4 curas = gate kernel ×3 (TODO-29). | 43510 |
...
| §209 | **Verif. técnica lote V6 + fix blocker walk-in ⟦OPUS-4.8⟧** — `createManualLead`→permission-denied (regresión SEC-06 §187, L-41) → fix `crm.edit`+`_createdByUid` anti-spoof, append-only, público intacto. Suite 198/198. | 43537 |
| §210 | **Strangler E6: gates ②/③ (5/6) ⟦OPUS-4.8⟧** — `admin-cutover-gates.js` (aditivo/reversible, Ocultar≠Borrar) oculta+redirige las secciones clásicas portadas (reseñas·banners·marcas·atributos·aliados) en sidebar/tabs/hash. Vehículos EXCLUIDO hasta V6. | 43555 |
| §211 | **Fixes render SITIO PÚBLICO (verif V6) ⟦OPUS-4.8⟧** — reseñas (link→`resenas.html`; home la borró SP-1) · banner promo `#promoBannerSection` restaurado · carrusel marcas `load(true)` refetch. Cache bump. | 43573 |
| §212 | **FIX SEGURIDAD: dueño INAMOVIBLE server-side ⟦OPUS-4.8⟧** — pase adversarial ④ cazó 2 huecos: `users.edit` degradaba al CEO + `roles.edit/delete` tocaba `system_super_admin`. Fix `firestore.rules` (anti-spoof) + test 7/7. DEPLOYADO. | 43597 |
...
| §214 | **FIX gate §210: no encerrar al dueño fuera del clásico ⟦OPUS-4.8⟧** — `guardHash` va a `dashboard` (no al portal, que restaura sección migrada→encerraba sin Usuarios/Roles). Lección: cutover no debe encerrar lo aún-no-portado. | 43637 |
| §215 | **④a PASO 2: backfill fundación RBAC (callable `backfillNivelesRBAC`, no ADC L-43) ⟦OPUS-4.8⟧** — helper puro `shared/rbac-foundation.js` + 9 tests + rev. adversarial; dueño=100 con aserción post-write. Deploy ✓. | 43653 |
...
| §221 | **CMS cobaya VERIFICADA LIVE (editor "Acerca de" E2E) + fix bug site-wide `pageTitle` ⟦OPUS-4.8⟧** — RCA: el SSG hornea `<title>` sin `id=pageTitle`→`loadVehicles` revienta→catálogo de las 19 marcas roto; fix null-guard `cd2a4d1` (merge #878→`d25930a`). Verificado live. → L-45. | 43755 |
| §222 | **CMS por marca COMPLETO: banner editable + FASE 2.4 instant-publish + nav→canónica + review adversarial ⟦OPUS-4.8⟧** — banner editable (subir img→Storage→`bannerUrl`→SSG hornea hero+og:image; regla `scBannerUrlOk`); **2.4**: CFs `onSiteContentChange`/`onMarcaChange`+`cancel-in-progress`→guardar/alta-baja marca regenera SOLO; **nav**: 5 builders de enlaces de marca→`/marcas/{id}.html` (no la plantilla, que no muestra CMS); diseño `.brand-about` centrado. Review `wf_ea699144`: 0 críticos, 3 LOW (1 fixed/1 tarea/1 refutado). L-46. | 43773 |
