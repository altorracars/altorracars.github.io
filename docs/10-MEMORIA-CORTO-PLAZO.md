...
| **TODO-32** | **ESCALABILIDAD del cerebro ⟦OPUS-4.8⟧** — comité+Gemini+debate → **plan ESCALONADO** (no rebuild): índice COMPILADO del genoma (lossless, cura §227) · tombstoning anti-Data-Rot · shard por rango · BM25/tags DIFERIDOS (gated por métrica) · NO RAG; **subsume TODO-28**. **E1 ✅** `scripts/brain-index.mjs` SHADOW probado (224 ADRs, líneas exactas, gate OK; delta 1 a reconciliar). Diseño→bóveda `…TODO32-…SINTESIS.md`. | 🔄 E1 ✅ · cutover/tombstoning/kernel pend | reconciliar→cutover→×brains |
