...
| **TODO-35** | **Código muerto/viejo (anti-Knight-Capital) ⟦OPUS-4.8⟧ — ESCALADO: el código viejo YA rompe EN VIVO** (23/06, capturas dueño: botones→Free Core "no entendí" `concierge.js:698`; "finalizar conversación" exige forzar refresh ×N; Free Core da respuestas malas; **diseño widget terrible**, F4/F5 documentado). Construir el mecanismo (skill/agente/**gate de workflow** que dispare SIEMPRE) + **auditar y limpiar el bot** (concierge.js 4.3kL + dual-core + finalize flow §57.x) + rediseño UX (F4/F5). Detonante: Knight Capital ($460M). Brief→spec. **Nunca ejecutado (deferido ×N) = causa raíz de los bugs vivos — M-19.** | 🔴 ACTIVO | dueño exige YA |
