...
> **▶️ SIGUIENTE (disparador: "continúa el plan maestro"): OLA 2 en orden** — **2.5 agenda: eliminar cita owner-only** (`citaAction` + `'delete'` transaccional que LIBERE el cupo + purga de canceladas — toca functions+rules → suite emulador) → **2.6 seguridad P2** (tren rules+functions, lista en plan §Ola2) → 2.7-2.12. Cadencia: implementar → build → preview `?mock=1` → commit(+dist!)+push+merge por bloque; suite emulador SOLO si tocas rules (L-63: puerto 8081 = matar java zombi).
>
