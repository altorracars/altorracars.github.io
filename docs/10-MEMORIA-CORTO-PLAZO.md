...
> **HECHO 29/06 h-j (`479b34cc`→+4mód, dev→main):** emoji→SVG **estructural** en Bandeja·vehículos·deals/pipeline·dashboard·perfil·roles·config (tabs·acciones·toolbar·cards·badges·estados·títulos·búsqueda; **accesos del dash→`navIcon(to)`** = mismos del sidebar) + **voseo→tú** (login·seed-FAQs·perfil). FIX **L-60 ×2** (svg flex→`flex:0 0 auto` · grid-`<button>`→`padding:0`, icono anamórfico). `icons.js` ~30 glyphs. Build✅+live✅.
> **SIGUE P1:** resto módulos (hub.ui·dealers·departamentos·banners·unmatched·auditoria·…) + **iconos de DOMINIO** (rating 🔥🌤️❄️·nba·channel = **color semántico** en `classify/nba/scoring`, usados en cards de Bandeja Y dashboard → bloque propio que preserve `temp--hot/warm/cold`) + **Fase C** (mockups+design-system). **🎨 Doctrina (qué se DEJA como emoji):** saludo 👋·bandera 🇨🇴·toasts 🎉·badge ausencia 🏖 — cálidos/culturales/efímeros, NO UI genérica. ⚠️ Re-sync FAQ Firestore→bot (TODO-34/46). Owner-delete NO bloquea.
