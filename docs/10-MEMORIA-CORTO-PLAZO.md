...
>
> 🏗️ **CMS/Dinamismo (TODO-23) — COBAYA COMPLETA E2E ⟦OPUS-4.8⟧** (editor=Opción A; arquitectura comité v4 + Gemini → ADR §220 + bóveda `…cms-*`). Flujo: panel nuevo módulo `cms-dinamico` (nav "Contenido", gate `content.edit`) edita "Acerca de" por marca → regla `siteContent` (content.edit + anti-PII, DESPLEGADA) → cron hornea `aboutBrand` en `marcas/{slug}` (escapeHtml). **HECHO**: FASE 0 seguridad 0.1-0.5 (gate `SSG_SELFTEST` con dientes; 0.3 +fijó bug SEO vivo: marca.html sin `<meta robots>`→canonical/OG no inyectaban) · FASE 1.1 `content.edit` (catálogo v1.2.0, frontend+backend) · FASE 2.1/2.3 cron-inject · 2.2 regla · 2.5 editor (verificado en demo `?mock=1`; router.js ROUTES era el ancla faltante). **SIGUIENTE (opcional, lo hago YO)**: 2.4 CF onWrite (rebuild rápido; hoy = cron 4h / regenerar SEO manual) · test rules-unit emulador · 0.2b purga svg (consola/auth) · cron→`firebase-admin`+SA · concurrency/bake-integrity/health-check (FASE 3). **MERGE ✅ (20/06, cron bumpeó cache→05). 🔜 SIGUIENTE (próximo chat) = GUIAR al dueño en la PRUEBA LIVE** (pantallazo×pantallazo, NO es tarea suya: abrir panel nuevo→Contenido→elegir marca→escribir→Guardar→disparar publicación o regenerar SEO→ver en `/marcas/{slug}`). El dueño (CEO `'*'`) ya tiene `content.edit`.
