...
- **Causa**: confundí "la DECISIÓN de ir-live es del dueño" (dinero/producción, cierto) con "el ACTO de desplegar es del dueño" (FALSO). Familia **M-12** (el git entero es de Claude): misma frontera **dueño-DECIDE / Claude-EJECUTA**, ahora trasladada al deploy. Bajo contexto largo una regla always-on no disparó al redactar pasos.
- **Corrección (trigger duro)**: antes de escribir CUALQUIER "pasos/runbook para el dueño", escanear cada paso → si es `firebase deploy` / desplegar / publicar functions·rules = **es MÍO, lo ejecuto yo**. El dueño SOLO: go/no-go · dinero · A/B · gates legales (P4) — el git es MÍO de punta a punta, merge y deploy incluidos (C0 orden nº 2, 4-sep-2026; M-12). Config-docs de prod (`_brain.enabled`, `config/altorTTL.enabled`, etc.) = dueño-en-portal o yo-con-su-go, NUNCA vía MCP (callejón e). + §1 endurecido este turno.
