...
- **EN CURSO: SP-5.2 body migration soft pages — DECOMPUESTO en 3 lotes** (es un PORT;
  ...
  - **SP-5.2.c**: App-like (contenido generado por JS — reescribir el render a cinematic
    ...
    - **SP-5.2.c COMPLETO** ✅: c.1 resenas §130 · c.2 perfil §131 + favoritos §132 · c.3 comparador §134 (flotante) + §135 (página) · **c.4 simulador §136** (armonización tokens como perfil, cero cambios al cálculo). Todo el cuerpo de las soft/app-pages está cinematic. + **§137 QA/pulido** (flotante comparador reposicionado para no chocar con QuickTools `.qt-wrap`; gráfico del simulador armonizado). **⚠️ Falta push del simulador §136 (e92035a estaba +1 sobre origin/main) + este lote de pulido.** **← RETOMAR: SP-4** (motor recomendaciones GA-based — requiere analytics configurado primero).
