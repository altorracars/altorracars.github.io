...
- [x] Leída la fuente `admin-concierge.js` (read-path: chats/messages/presence + renderChatDetail). `admin-presence-ui.js` NO requerido para 3a (presence read salió de `startAttendingPresenceListener`/`onAttendingPresenceSnapshot`). (§249)
...
- [x] **(3a) Lista de chats + filtros/colas + detalle (read) + presence — verificado `?mock=1`** (lista+counts, header/radicado/meta, banners closed/claimed/attending, burbujas user/bot/asesor/system, responsive mobile 1-pane+back / desktop 2-pane, accent+dark). 0 errores de módulo. (§249)
...
- [x] **(3c) transfer / close / reopen / super-release / notas — verif `?mock=1`** (claim YA hecho en 3b). close/reopen optimistas con system-msg (⚠️ orden vs rules: msg ANTES de cerrar / status:active ANTES de msg, evita el race de rules no-super). transfer = modal con asesores online (presence read) + `runTransaction`. notas internas = subcol + merge cronológico en el stream + toggle (en nota NO se emite typing). super-release. Verificado: header Cerrar · mine-banner Transferir/Liberar · crear nota · modal transfer→claimedByOther · close→Reabrir→reopen · liberar→unclaimed. Realtime (notes listener, presence del transfer, fallos) = live-only. (§249)
- [x] **(3d) Smart suggestions + Resumen handover (LLM summary DIFERIDO=saldo) — verif `?mock=1`**. Suggestions heurísticas por keyword (degradadas: sin AltorraAI/NER/KB) que llenan el composer + modal "Resumen" local (cliente/conteos/estado/últimos msgs + copiar) con nota IA-diferida. **→ F-4 COMPLETO (§249).**
- [x] Wiring router/shell/main + RBAC `concierge.read` gateado como `firestore.rules:937` (no se pintan acciones que el server rechazaría). NAV "ALTOR Hub" 1º del grupo Comunicaciones. (§249)
- [x] dist NO reconstruido (source-only, L-53/§237.6). **[x] ADR §249** (F-4 completo 3a-3d) + fila `00` + lección L-53 (gotcha DS) + 05/10. **F-4 COMPLETO** — sigue F-5.
