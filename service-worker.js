...
const CACHE_VERSION = 'v20260511060000'; // §45 — 4 fixes: (1) skip-to-content link 'Saltar al contenido' eliminado del HTML por solicitud cliente. (2) firebase-messaging-compat.js movido de <script defer> estatico a carga dinamica desde admin-fcm.js cuando window.firebaseReady resuelva — antes crasheaba con 'Cannot read properties of undefined (reading INTERNAL)' porque se ejecutaba antes que firebase-app-compat.js. (3) admin-profile.js loadProfile usa helpers defensivos setText/setValue/setHTML con guard if(el) — antes crasheaba post-save si algun ID no existia. saveProfile post-success envuelto en try/catch para que errores cosmeticos NO disparen toast 'Error al guardar' tras success. (4) RTDB rules en database.rules.json correctas (incluyen guards anti-update sin uid) pero REQUIERE deploy manual: firebase deploy --only database. // §44 pre-paint optimista.
