...
const CACHE_VERSION = 'v20260511030000'; // §42 — Tabs por grupo replicando patrón CRM. Cliente pidió: "deberia ser como CRM, contactos y ventas, bandeja, pipeline — así con todos los que tienen subnavs". Eliminamos el <nav id="atnSubnav"> flotante (que pelaba con stacking contexts y nunca aparecía). Reemplazamos por js/admin-group-tabs.js que inyecta DENTRO de cada sección activa de un grupo (Inventario/Hub/Config) un .section-tabstrip con estilo idéntico al .crm-tabstrip de CRM. Click en tab navega a sección hermana via AltorraSections.go(). Active state con accent dorado (Inventario), verde (Hub), neutral (Config). Pre-inyección al init para que las tabs estén listas inmediatamente al activar la sección. SECTION_TO_GROUP en admin-topnav.js sigue siendo usado solo para marcar el tab grupo del topnav como activo. renderSubnav() convertido en no-op. Suscripción al subnav legacy gateada (subnavEl null si no existe). // §41 — FIX DEFINITIVO subnav invisible (notifyChange en PATH A).
