...
| §270 | **OLA 2 parcial 2.1-2.3 ⟦FABLE-5⟧** — Ctrl+K command palette (`core/command-palette.js`: NAV canSee inyectado + registros lazy 60s; lead→360 L-27) · campana in-app + "＋ Crear" polimórfico (`core/notifications.js`: localStorage 20 + badge + deep-link, fcm alimenta) · perf: `fetchDealerStats` counts server-side (getCountFromServer + 2 índices; consigna query filtrada) + limits vehiculos/team. Casi-bug cazado: orderBy(prioridad) excluiría docs legacy. | 44385 |
