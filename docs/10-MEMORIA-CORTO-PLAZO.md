...
> **⚡ FABLE EJECUTA TODO hasta ~07-jul** (directiva dueño 03/07; Opus retoma después). ✅ **OLA 1 código 100%** (04/07, `main`==`dev` 767ea179): 1.8b skeletons top-5 · 1.9 combobox typeahead (`core/combobox.js` REUTILIZABLE) + checklist 1ª-venta en Inicio + guía doble captura · 1.9b cola offline-rechazos (`capture/offline-queue.js` + banner Bandeja). ⚠️ el `dist/` se COMMITEA: no olvidarlo por bloque (se me pasó 2 commits, curado en 767ea179).
> **▶️ SIGUIENTE (disparador: "continúa el plan maestro"):** (1) **validación live Chrome cierre-Ola-1** (plan §Ola1 final: todas las secciones, screenshots, viewports 1440/1920/768/375 — Pages ya desplegado; requiere Chrome del dueño conectado; callejón j: confirm() nativo lo bloquea → reparto con dueño) → (2) **OLA 2 en orden** (2.1 Ctrl+K command palette → 2.2 campana in-app + quick-create → 2.3 paginación/perf `fetchDealerStats`…). Cadencia probada: implementar → build → preview `?mock=1` → commit(+dist)+push+merge por bloque; suite emulador SOLO si tocas rules (L-63: puerto 8081 tomado = matar java zombi).
