...
> - **22/06**: **TODO-32 paso (b) ✅** (§229): guardián del índice replicado SELECTIVO por convención (verificado `99`/`00` de los 4) → inmob instalado (commit `66beb92`); bersaglio (`e6ef5ff`)/insema (`852bcb1`, branch `cerebro/todo-32`) = ADR N/A honesto (no código muerto). Decisión: no fundir el guardián escritor al `brain-check` read-only. +insema a peers de cars. L-52.
