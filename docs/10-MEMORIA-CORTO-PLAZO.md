...
> 🔜 **EPIC "ALTOR Hub v2" — FOCO ÚNICO (decisión dueño 23/06): terminar TODO esto (bot LLM + captura + UX widget + UX chat interno) ANTES de retomar CRM / panel admin / migración panel viejo / demás pendientes.** Detalle+fases → spec `2026-06-22-TODO34-…` §EPIC. Pipeline Decisión Fuerte completo (3 comités + Gemini, crudos bóveda). **Arquitectura DECIDIDA = Opción A** (solo-LLM + Tool Calling + botones tontos + guards; se borra el motor 5,600L que falló). **Plan 6 fases**: F1 guards+frenar-hemorragia (App Check enforce + gate costo techo-manual/auto-enforce/modo-captura + rate-limit por App Check token no sessionId + tope turnos + fix ingestión anónimos) · F2 bot LLM+Tool Calling tras `engine:'v2'` (`submit_lead` con `lead_quality` GRATIS, el LLM PROMUEVE hot→asesor, nunca rechaza) · F3 captura (**quitar CÉDULA**, mínimo nombre+celular, **fallback WhatsApp**=cero-pérdida, consent inline, saludo tono COLOMBIA — hoy está en voz argentina) · F4 UX widget (módulo PARALELO v2, no in-place) · F5 UX chat interno admin (fase separada) · F6 poda. **Hallazgos verificados clave**: hoy chatLLM sin App Check + sessionId brincable + sin tope turnos = troll/scraper conversa infinito y caro; pedir cédula = mayor fuga de leads; no hay fallback WhatsApp; voz argentina en negocio colombiano. Crudos: `…comite-arquitectura…` · `…gemini-redteam…` · `…comite-UX-captura…`. **PEND: capa 3 Gemini (la corre el dueño) → confirmar → arrancar F1.**
