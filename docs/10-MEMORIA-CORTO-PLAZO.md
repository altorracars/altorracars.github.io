...
| **TODO-46→F-1** | **Bot v2 ⟦OPUS⟧ ~CODE-COMPLETE** (verif.preview): `shared/lead-flow.js`+escalado→Hub+botones retoma/WA+§80+§234 auth-wipe(self-contained)+gate-form(nombre/cel/consent). v1 intacto. **RESTA: cablear en `components.js`(flag)+validar LIVE→flip**. | 🔄 cablear | LIVE |
