...
| **TODO-24** | **Comité BORRADORES** (rediseño profesional) §202.5 → **f1+2 ✅ LIVE (§227, #896)**: recuperación local opt-in §107-safe + por-cuenta + galería pro + modal custom (comité ACOTADO 4+peer-review). **f3 EN CURSO** = Storage huérfanos. **Gemini red-team ✅** (canceló la Parte A sobre-ingenierizada `drafts/`+copy-on-publish+Callable → **80/20 mark-and-sweep**; cazó el bug FATAL #7 URL-pública-vs-path-del-Admin-SDK). **Fase 1 (dry-run read-only + workflow de botón + guardas anti-#7) CONSTRUIDA** (`feat/storage-orphans-f3` `897f5ba`). Vault crudo+síntesis (63c1e50). Pendiente: dueño mergea + corre el workflow → reporte → Fases 2-4 (purga c/respaldo · auto Cloud Function · expiración de drafts). | 🔄 f1+2 LIVE · f3 Fase1 lista | f3: dueño corre el dry-run |
...
| **TODO-32** | **ESCALABILIDAD del cerebro ⟦OPUS-4.8⟧** — paso (a) §228 + paso (b) §229 hechos (guardián del índice + replicación SELECTIVA ×brains: inmob instalado, bersaglio/insema N/A; detalle→§229/L-52). **RESTA (futuro/YAGNI)**: genoma frontmatter (índice lossless de descripciones) + tiering 100x; bersaglio se sumaría al guardián si normaliza headers (`§N` mecánico). | 🔮 futuro | YAGNI · reabrir si el índice crece |
| **TODO-33** | **Reconciliación CEREBRO ↔ WEB REAL ⟦OPUS-4.8⟧** (autocrítica del dueño 22/06: el cerebro decía "27 functions" y hay **59** → derivó sin reconciliar). Meta: `brain-check` valida la estructura INTERNA, no la realidad EXTERNA (infra desplegada). Auditar en vivo vs documentado: functions (59, varias muertas — bot diferido + migraciones 1-vez), colecciones Firestore (27 — `drafts_activos`/`comments`-test/`unmatchedQueries` sospechosas) vs schema `20-ESPACIAL`, Storage, rules, índices. Corregir frescura + podar. **Al final de TODO** (tras limpieza Firebase). | 🔮 al final | tras poda Firebase |
...
> GC ×14 (12-22/06): §184-§227 + §229 consolidados (→`99`/`00`/`30`/`33`). 21/06: §223-226 + **TODO-31 ✅** (detalle→§223-226/bóveda). 22/06: §227 borradores f1+2 LIVE (#896, L-51/M-17) + §229 TODO-32 paso (b).
> - **22/06**: **TODO-24 f3 (Storage huérfanos) — Fase 1** ⟦OPUS-4.8⟧: Gemini red-team (vía Antigravity) canceló la Parte A (race conditions) → 80/20 mark-and-sweep; cazó el bug fatal #7 (URL pública de Firestore vs path del Admin SDK → borraría TODO). Construido `scripts/storage-orphans.mjs` (read-only, doble-llave, `--backup`, escaneo de doc completo, guardas: aborta si 0 refs o >90% huérfanos) + `.github/workflows/storage-orphans.yml` (botón, SA del cron §225) + alias `orphans:scan` → `feat/storage-orphans-f3` `897f5ba`. Crudo+síntesis→bóveda (`63c1e50`). **Pendiente dueño**: mergear + correr el workflow → reporte → gate de Fase 2 (purga). Divergí de Gemini en auditLog (cosmético) y colchón 30d.
> - **22/06**: **TODO-32 paso (b) ✅** (§229): guardián del índice replicado SELECTIVO ×brains (inmob ✅ `66beb92`; bersaglio `e6ef5ff`/insema `852bcb1` N/A honesto); no se fundió al `brain-check` read-only; +insema a peers; L-52.
