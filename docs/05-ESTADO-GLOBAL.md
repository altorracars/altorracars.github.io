...
| **Branch activa** | `refactor/estructura` (en sync con `main` = `9e392c3`). §157–§165 LIVE en `main`. **§165 (Reportes/KPIs)** desplegada esta sesión (commit + merge + push `9e392c3`; sin rules/indexes/functions/cache). **Auto-deploy por fase autorizado** (memoria `feedback-auto-deploy-crm`). |
