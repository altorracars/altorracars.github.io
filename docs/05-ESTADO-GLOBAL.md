...
| **Deploys backend pendientes** | ⏳ **Fase 1 CRM** (montado en `refactor/estructura`, SIN commit/deploy): `functions:onSolicitudCreated` (ingestión canónica) + `firestore:rules,firestore:indexes` (colecciones `contacts`/`leads`/`activities`/`failedIngestions`). Deploy MANUAL. Falta E2E emulador (req. Java) antes de desplegar. |
