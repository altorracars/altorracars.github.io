...
**Estado (2026-06-09 — ÚNICO estado canónico de App Check; 05/10 apuntan aquí)**: SEC-03 (tope) + SEC-04 (candado Telegram) **DESPLEGADOS ✅ y EN VIVO** (`firebase deploy --only functions`, 28 funciones OK; revisión adversarial OK + `node -c` OK). `TELEGRAM_WEBHOOK_SECRET` seteado (v1) + webhook re-registrado con `secret_token` (`getWebhookInfo`: 0 errores, 0 pendientes). **SEC-02 App Check: monitor LIVE en `main`** (commit `dc5ebca`; site key registrada en consola). **Falta**: observar % Verified 1-2 sem → enforce por-recurso (si el monitor no muestra métricas, re-verificar la clave secreta en consola) + SEC-01 (Opción A; pre-seed + OK).
...
- **SEC-02 App Check (anti-spam)** ✅ **MONITOR LIVE en `main`** (2026-06-09; commit `dc5ebca` mergeado — verificado `git log origin/main`): site key reCAPTCHA v3 `6Lfz…` registrada (proyecto `altorra-cars`, dominios `altorracars.github.io`+`localhost`); activado en `js/core/firebase-config.js` (app **namespaced** `firebase.appCheck(app)`, carga **no-bloqueante** `.catch`) + `admin-app/src/core/firebase.js` (modular, build OK); cache bump `v20260608231721`; `firebase-messaging-sw.js` **intacto**. API verificada vs docs Firebase (compat acepta `appCheck(app)`). **Pendiente SOLO**: (c) observar % Verified 1-2 sem → enforce por-recurso (si no hay métricas en consola, re-verificar la clave secreta). Init/rollout:
