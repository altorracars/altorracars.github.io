...
>
> 🧠 **Iniciativa cerebro+skills (2026-06-06)** → spec `docs/superpowers/specs/2026-06-06-cerebro-skills-roadmap.md`. **Fase A ✅**: doctrina arquitecto `§3.8`, manifiesto ya en `46`, doctrina legal en Trigger 🔵 `§G.2`, workflow reutilizable `.claude/workflows/adversarial-review.js`, lección **L-34**. **Fase B** (skills `legal-colombia` · `software-architect` · `comité-expertos` con 4ª voz Antigravity/§15 — evaluar `llm-council` primero — vía `skill-creator`) + **Fase C** (revisión holística CRM clase-mundial con `crm-architect` + **blindaje** rate-limit/RBAC-read → nace `41-SEGURIDAD`) → **sesión fresca** (esta ya es muy larga).
>
