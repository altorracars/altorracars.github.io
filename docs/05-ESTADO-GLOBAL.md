...
|---|---|
| **Build** | 🟢 `main`: CRM canónico + E6 + RBAC ④a (④b parqueado) + CMS marca + F4/F5/§237/F-0.5 ✅ (dueño mergeó #938-945). **`dev` = `origin/main` + F-2 (1/6) `usuarios` §238** (source; sync ff-only al arrancar). **⚠️ dist admin-app GATEADO a staging** (batch F-0.5 multiTab/RTDB + F-2 sale junto tras E2E; §237.6). |
