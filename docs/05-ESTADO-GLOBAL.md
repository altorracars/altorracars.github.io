...
|---|---|
| **Build** | 🟢 Catálogo cinematic + a11y **en producción**. Cerebro ENDURECIDO (§156). **🏗️ CRM: Fase 1 (ingestión canónica `onSolicitudCreated`) LIVE + Fase 2 (Bandeja Inteligente, app `admin-app/` greenfield Vite + Firebase modular) CONSTRUIDA** — `npm run build` verde, verificada en `?mock=1` (snapshot+eval), 2 bugs cazados+fixeados. ✅ DESPLEGADA. **+ Fase 3a (Pipeline `deals`) + Fase 3b (Agenda unificada) DESPLEGADAS a `main`** — verificadas `?mock=1` (54 módulos, **0 bugs c/u**). Portal nuevo: 3 superficies live (Bandeja+360, Pipeline, Agenda) + **Captura MANUAL de leads externos** (§162) LIVE + **Canal AUTO #1: registro de cuenta → contacto (`onClienteCreated`) DESPLEGADO** (§163, 28 tests Vitest verdes). ⏳ Solo falta **E2E live** del cliente. `brain:check` SANO. Ver ADR §150–§163. |
...
| **Branch activa** | `refactor/estructura` (en sync con `main`). §157–§162 LIVE en `main` (`599ba89`). **§163 (`onClienteCreated`) ya DESPLEGADO** (functions); su código en commit de esta sesión → merge a `main`. **Auto-deploy por fase autorizado** (memoria `feedback-auto-deploy-crm`). |
...
| **Deploys** | ✅ Fase 1–3 + captura manual LIVE: `/admin-app/dist/` + `firestore:rules` (`crmNotes`+`deals`) + `indexes` (`deals`). ✅ **`functions:onClienteCreated`** desplegada (§163, registro→contacto). ⏳ **Backfill** de cuentas YA registradas (script idempotente, futuro) + **E2E live** (L-08). |
