...
- **EN CURSO — §119 Reestructuración de frontend** (rama `refactor/estructura`, NO mergeada a main). Plan de 5 fases, riesgo escalonado. Objetivo: orden modular sin romper URLs públicas (ruta=URL en GitHub Pages, sin bundler). NO replicar la imagen React/Vite literal. La migración Vite real es TODO-01 (~1 año).
  ...
  - **Fase 2** ⏳ reagrupar `js/`. ANÁLISIS DE DEPENDENCIAS HECHO (mapa abajo).
    ...
    - **⏳ próximo 2.2b** (DELICADO — toca cron): `comparador.js`, `citas.js`, `vehicle-hotspots.js` están en las plantillas `detalle-vehiculo.html`/`marca.html` → al moverlos hay que actualizar plantillas + las 45 páginas ya generadas en `vehiculos/`+`marcas/` (o re-correr el generador). comparador además está en ~8 HTML. Hacer con cuidado extra + verificar las 45.
    - **⚠️ OPERATIVO: hay un hook que auto-commitea con mensaje "ACTUALIZACION" y auto-pushea** (visto en §118 y §119). Preempta los mensajes de commit detallados → **el audit trail real de cada fase vive en ESTE archivo / ADR §119**, no en los mensajes de git. No pelear el hook con --amend/force-push.
