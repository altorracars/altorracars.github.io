...
| §211 | **Fixes render SITIO PÚBLICO (verif V6) ⟦OPUS-4.8⟧** — 3 sondas: reseñas (link header→`resenas.html`; sección home la borró SP-1=regresión), banner promocional (slot `#promoBannerSection` borrado por SP-1 → restaurado en index), carrusel marcas (`initBrands` rendea de cache viejo → `load(true)` refetch). NINGUNO del cutover. Cache bump `v20260617174623`. Staged para push. | 43573 |
