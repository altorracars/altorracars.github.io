...
> EN CURSO (todo en `dev`): **F1.a✅** techo $ + memoria corta · **F2.a✅** cédula fuera + saludo · **F2.b✅** WhatsApp en gate + voseo→Colombia + botones tontos bienvenida · **F3-a✅** Tool Calling `search_inventory` read-only tras `engine:'v2'` (v1 INTACTO; `submit_lead`=F3-b). ⚠️ **LLM apagado** (`_brain.enabled=false`) + **functions PEND DEPLOY** (F1.a+F3-a como UNIDAD justo antes del flip; prod corre v1 → seguro). **Decisiones dueño 23/06:** techo **$15**✅ · App Check **MONITOR** (enforce al crecer; techo=el muro anti-DoW) · TTL **anonimizar@30d** (no borrar). 🔜 **F3-b** (`submit_lead`+validación backend+cero-pérdida) → TTL mecanismo (`autoResolveIdleChats`) → flip A/B. **Pend dueño:** consent texto legal (P4 abogado) · merge `dev`→`main`.
