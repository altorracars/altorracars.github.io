...
>
> 🎨 **REDISEÑO portal W-11 — F1 EN CURSO** (spec `…portal-crm-rediseno-W11.md`): **F1(a) selección múltiple + acciones masivas en Bandeja = BUILT + VALIDADO LIVE ✅** (build prod nuevo `index-G4eiZWxO`, 26/06: 2 sel→barra→limpiar, 0 errores). Hallazgo: la Bandeja real ya era fuerte → no hacía falta tabla densa; el gap era multi-select. PEND F1: (b) "Nuevo lead" topbar · (c) drawer móvil. F2: grupos `<details>`+fix `setActive`+iconos SVG. Palette Ctrl+K diferido. Aliados=aliado (§205 ok; comercial→TODO-25). Bot LLM=saldo. (Build/cache → `05`.)
...
| **TODO-44** | **Fiabilidad del cerebro ⟦OPUS-4.8⟧** — auditoría §257 + cura **M-22**. **Check #16 (`verificado-vivo`/stale) en kernel + PROPAGADO+COMMITEADO byte-idéntico ×4 ✅** (26/06, sha `4905D566`: cars `dev` · bersaglio `Desarrollo` · inmobiliaria `cerebro/instalacion` · insema `cerebro/todo-32` +4 skills catalogadas; brain-index.mjs añadido a bersaglio/insema). **RESTA**: adopción (marcadores `verificado-vivo:` en afirmaciones de realidad) + reconciliación exhaustiva (Storage/índices/289 ADRs). Absorbe TODO-33. | 🔄 mecanizado+propagado ✅ | adopción + reconcil. |
