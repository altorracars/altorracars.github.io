...
| **TODO-24** | **Comité BORRADORES** §202.5 → **f1+2 ✅ LIVE (§227, #896)**. **f3 = Storage huérfanos**: diseño 80/20 post-Gemini (bóveda `63c1e50`, L→bug#7 URL-vs-path); dry-run corrido (**406 huérfanas/78MB/$0**, mecanismo `scripts/storage-orphans.mjs`+workflow listos). Purga REAL pend. credencial de borrado (dueño: `gcloud auth application-default login` → yo corro `orphans:scan --backup --purge`). Fases auto 3-4 → futuras. | 🔄 f1+2 LIVE · f3 purga pend-cred | f3: dueño 1 comando gcloud |
...
> - **22/06**: **Limpieza Firebase (dueño)** ⟦OPUS-4.8⟧ — auditoría en vivo (Firebase MCP). **Firestore**: −5 docs basura (`drafts_activos`×2 viejos + `comments` test×3, vía MCP delete). **Functions**: `proactiveEngagement` ELIMINADA del deploy (corría cada 5min y FALLABA ~288×/día por índice faltante; des-exportada en `functions/index.js`, bloque source → TODO-34). **Storage**: 406 huérfanas/78MB/**$0** → purga pend. credencial. **Hallazgo→autocrítica**: el cerebro decía 27 functions, hay **59** (TODO-33). Visión bot → **TODO-34**.
