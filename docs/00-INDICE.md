...
| §165 | **CRM Fase 4: Reportes/KPIs** (5ª superficie del portal, `#/reportes`). Tablero determinista: KPIs (período + estado actual), embudo monotónico (join lead→deal ganado), **rendimiento por canal** (`channelOf`, atribución ⭐), forecast por etapa, tendencia (`dayKey` LOCAL), equipo, **export CSV**. `domain/reports.js` PURO + `modules/reportes/{data,charts,ui}` + agregación CLIENTE sobre `getDocs` acotado (índice automático, L-30). Charts **SVG/CSS** (no hay librería). Mock +leads/deals cerrados. **SIN reglas/índices/functions/cache** (Vite hash-busting, no toca sitio público). Build verde, `?mock=1` reconciliado a mano + revisión adversarial 5-dim (0 bugs correctness, 4 fixes). L-32. | 42994 |
| §166 | **CRM: Contactos (directorio)** — núcleo del portal COMPLETO (sin "Pronto"). `modules/contacts/contacts.list.js` (`mountContactos`) + `loadContactsList` (getDocs `contacts` orderBy createdAt + `leads` para mapear; campo único auto-índice). Lista buscable/filtrable (chips por ciclo de vida). Clic en persona con lead → abre la **ficha 360 existente** (espeja store.leads + detailLeadId atómico, L-27); suscriptor sin lead → fila informativa no interactiva (anti falso-affordance). 360 (`contacts.ui.js`) INTACTO. Mock +getMockContacts/+suscriptor. **Sin reglas/índices/cache**. Build verde, `?mock=1` (16 contactos, filtros/búsqueda/360 OK) + revisión adversarial 3-dim (mayoría falsos positivos, fixes reales aplicados). L-33. | 43004 |
| §167 | **Cerebro Fase A** (ajuste de capacidades, sin código de app): **Lente de Arquitecto always-on** `CLAUDE.md §3.8` (6 pilares → lóbulo `46` + skill futura `software-architect`; manifiesto ya estaba en `46`, solo +tabla monolito) · **regla Legal=Colombia** en Trigger 🔵 `§G.2` (fuentes primarias + gate abogado; skill futura `legal-colombia` + lóbulo `42`) · **workflow reutilizable** `.claude/workflows/adversarial-review.js` + **L-34** (triar findings vs código real). Anti-bloat: NO ley-dump, NO duplicar `46`, §3.6 comprimido para caber. Comité "×3" (dinámico + 3 niveles + 4ª voz Gemini/§15) DISEÑADO. Spec → `docs/superpowers/specs/2026-06-06-cerebro-skills-roadmap.md`. Fase B/C (skills + holística CRM + blindaje→`41`) en sesión fresca. | 43014 |
...
| §177 | **E0 EJECUTADA** ("ok todo"): spec única `crm-spec` (enums v3+matriz, 16 tests) · F34 `crmExport/crmRestore` + guards `_migration` ×4 · **F1 lead convertido inmutable** (rules; atacado LIVE desde sesión CEO → denegado ✅; 5 tests emulador — Java SÍ está, L-26 act.) · Bandeja: Activos default + "N ocultos" + SLA chip 45/60min + convertido→link Pipeline · **F17-urgente**: cancelar/reprogramar LIBERA cupos (transacción única, drag-drop incluido). Deploy rules+6 fns · cache `v20260610011703` | 43163 |
| §178 | **E1a EJECUTADA** (núcleo): **⚡ lead rápido** `lead_intake` (<30s, OFFLINE, owner=registrador forzado en rules, guion 1581, E2E live: ingerido en 2s con +57/consent/tags ROI) · módulo compartido `ingestLead.js` (una sola ruta de alta) · E.164 default CO · quick-log 📞 + **P2.b próximo paso** (presets 1 tap) + **📋 Pendientes hoy** · banner Agenda→clásico · manual v2. 71 tests. Gotcha: gap de propagación Eventarc (1er doc perdido). Quedan F37/F38/F33a | 43175 |
| §181 | **E1b EJECUTADA — Pipeline v2**: GATE restore ENSAYADO en emulador ✅ · migración v3 = no-op verificada (0 docs v2) · enums v3 espejo + **test de PARIDAD 7×7** · Rules v3 (gates acumulados, retroceso con razón, vendido terminal) · **trigger `onDealUpdated`** (E2E live 3s: chip proyectado + lifecycle opportunity + activity determinista ✅) · diálogo F7 + `anularConversion` + undo 10s + descarte con razón. ~100 tests. La divergencia Bandeja↔Pipeline = imposible (triple capa) | 43211 |
| §182 | **E2 tanda 1 — calendario**: F21.1 auditoría = configs DIVERGEN (web vende citas en FESTIVOS: availability 9-17/60min sin festivos vs calendarConfig 8-18/30min con 18) · **F16** `onSolicitudWritten` (startAt -05:00 canónico + proyección `activities/cita_{id}` idempotente anti-bucle; **E2E live 5s**: cita web → Agenda) · **`crmDailyJob`** 5am (BACKUP diario primero F34 → backfill startAt → rebuild cupos anti-fantasma → purga → digest). 101 tests. Tanda 2: F21 editor+festivos→blockedDates, F18/F19/F20 | 43223 |
...
| §179 | **E1a CERRADA**: **F37** `crmHourlyJob` (SLA 2h HÁBILES lun-sáb 8-18 UTC-5, idempotente `_slaAlertedAt`, callable manual) + **rotación de intake** en la transacción (`config/crmIntake`; E2E live: lead web nació con owner CEO ✅) · **F38** `notify.js` (crítica=push Telegram individual + `crm_alerts`; info=solo doc) + alerta al asesor auto-asignado · **F33a** `friction.js` (mediana/p90 vs umbrales F39, `__friccion()`). 72 tests. Demo pendiente: lead `_test_sla_e1a` → alerta del cron → borrar | 43187 |
| §180 | **Adelanto E3** (fricción #1 del dueño ×2): **F13 Archivar** (menú ⋯, reversible, fuera de Bandeja Y Reportes) + **F15 `crmPurgeLead`** (cascada server: activities+deals+doc entrada+contact huérfano+audit; doble confirmación; super admin) + **fix popup GitHub** (credencial `altorracars` fijada con `git config credential.username`). Verificación live del purge = el dueño borra el lead-demo. F14 anonimización sigue en E3 | 43199 |
| §185 | **E3 EJECUTADA**: **F40e índice `dedup/{key}`** (los 4 escritores de ingestión resuelven vía índice en tx; trigger `onContactWritten` con espejo `dedupKeys[]` — E2E live crear+borrar ✓; reconcile nocturno=F27e) · **F12** editar (Rules `_version`+1, server-fields intocables, delete:false) + **fusión resumible** `merges/{id}` con compactación de cadenas · **F14** supresión 1581 (gracia 72h, retiro de índice POR QUERY minuto 0, finalizador: stub aleatorio + tombstones borrados en cascada + snapshot purgado + auditoría por hash; retención backups 45d). Review adversarial 19 agentes: 1 crítico + 9 majors corregidos pre-commit. 139 tests | 43258 |
| §184 | **E2 COMPLETA (tanda 2)**: **F21** SSoT=`config/availability` (validador clásico mapeado bidireccional, módulo **Disponibilidad** del portal `#/config` con preset festivos CO + `advisorOverrides`, fix set-sin-merge latente) · **F18/F19** `crmCitaAction` transaccional (tupla asesor+vehículo bloques 30min en `resource_slots/{day}`, reschedule ROTA token C.4, no_show con follow-up, create manual `_ingestedAt` preseteado) + HTTP **`citaConfirm`** tokenizada (E2E live: curl→confirmada/whatsapp_link en 2s; token malo→410) · **F20** `citaSweep` horario (24-48h, T-3h, hold-expiry→`caducada`) · **F28 v2** (rebuild resource_slots, reproyección, F22, apartados vencidos). 125 tests emulador + **carrera C.5 en verde**. Festivos→blockedDates = 1 clic del dueño (botón 🇨🇴) | 43245 |
| §186 | **E4 EJECUTADA**: **F25-completo** `vehicleAggregate.recalcVehicleState` (agregado transaccional won→vendido TERMINAL / apartado / disponible; jamás pisa manuales; bump `_version`; badge **'Apartado' visible en la WEB** + LimitedAvailability; índice `deals(vehicleId,status)`) · **F10** `dealWon.applyWonSideEffects` (wonAt + checklist postventa + **commissionSnapshot server-only** + 3 tareas create-or-skip; retoma `recibeVehiculo` + callable `crmCrearBorradorRetoma`; panel Post-venta en Pipeline) · **F26** colisión (warning client-side cero reads + alerta daily job a owners+CEO) · **F42** Comisiones del mes en Reportes (query dedicada de wons, `dealLiquidable`=won+checklist completo, CSV anti-inyección). Rules: vendido cerrado salvo whitelist + coherencia status↔stage + amount>0 al ganar + anti-forja en create. **FIX pipeline páginas roto 16 días** (yml ruta cache-manager) + `retry:true` en onDealUpdated (Eventarc no reintenta por defecto). Review adversarial 4 dim: 23 confirmados corregidos pre-commit (+2 self-review). 169 tests. Cache `v20260611031500` | 43271 |
