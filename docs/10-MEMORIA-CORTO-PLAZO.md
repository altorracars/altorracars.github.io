...
>
> 🎨 **REDISEÑO portal W-11 — F1 EN CURSO** (spec `…portal-crm-rediseno-W11.md`): **F1(a) selección múltiple + acciones masivas en Bandeja = BUILT + VALIDADO LIVE ✅** (build prod nuevo `index-G4eiZWxO`, 26/06: 2 sel→barra→limpiar, 0 errores). Hallazgo: la Bandeja real ya era fuerte → no hacía falta tabla densa; el gap era multi-select. **F1(b) "Nuevo lead" en topbar GLOBAL ✅ BUILT+verif preview** (commit `587737b`/`index-CqzT8cf7`; antes el CTA vivía solo en la Bandeja → ahora visible desde cualquier sección; reusa `openQuickLeadForm` §178, gate `crm.edit` = inbox; falta validación LIVE dueño). PEND F1: (c) drawer móvil real. F2: grupos `<details>`+fix `setActive`+iconos SVG. Palette Ctrl+K diferido. Aliados=aliado (§205 ok; comercial→TODO-25). Bot LLM=saldo. (Build/cache → `05`.)
