...
> **HECHO 29/06 h-i (`479b34cc`+deals, dev→main):** emoji→SVG **colas Bandeja** (🔥👤🆕📥→flame/user/userPlus/inbox; QUEUES `icon`→`iconId`) + **vehículos** (acciones/toolbar/thumb/stock/modal) + **deals/pipeline** (vistas🎯🏁·acciones-tarjeta·postventa·badges·gate) + **voseo→tú** (login+seed FAQs). FIX **L-60 ×2** (svg hijo-flex→`flex:0 0 auto` · grid-`<button>`→`padding:0`; ambos cazados live, icono anamórfico). Build✅ + live `?mock=1`✅.
> **SIGUE P1:** emoji→SVG **headers de tarjetas restantes** (contacts/inbox cards) + resto de superficies (hub.ui·config·perfil·roles·dashboard·dealers·departamentos·banners·etc. — barrido amplio) · **Fase C** (mockups+design-system). ⚠️ **Re-sync FAQ Firestore (M-25)**: seed→"tú" pero `knowledgeBase/` sembrado sigue voseo → re-sync con el bot (TODO-34/46). **Owner-delete NO bloquea.**
