...
> **HECHO 29/06 h-k (`479b34cc`→+5mód, dev→main):** emoji→SVG **estructural** en Bandeja·vehículos·deals·dashboard·perfil·roles·config·**hub** (tabs·acciones·cards·badges·estados·títulos·banners; dash-accesos→`navIcon`) + **voseo→tú** (login·seed·perfil·hub-sistema). FIX **L-60 ×3** (svg flex→`flex:0 0 auto` · grid-`<button>`→`padding:0` · combo L-23 `max-width:100%`→`max-width:none` en flex apretado). `icons.js` ~43 glyphs. Build✅+live✅.
> **SIGUE P1:** resto módulos (dealers·departamentos·banners·unmatched·auditoria·brands·cms·…) + **iconos de DOMINIO** (rating 🔥🌤️❄️·nba·channel = **color semántico** en `classify/nba/scoring`, cards Bandeja Y dashboard → bloque propio que preserve `temp--hot/warm/cold`) + **Fase C** (mockups+design-system). **🎨 Doctrina (qué se DEJA):** saludo 👋·bandera 🇨🇴·toasts 🎉·ausencia 🏖·chips-sugerencia hub — cálidos/culturales/contenido, NO UI genérica. ⚠️ Re-sync FAQ Firestore→bot (TODO-34/46). Owner-delete NO bloquea.
