...
> 🎯 **RETOMAR = E6 cutover → ahora ④ RBAC.** En branch (pend. push del dueño): §209 (fix V6) · §210 **gates ②/③ COMPLETOS 6/6** (vehículos AÑADIDO tras **R-13 paridad-esquema CI + R-12 venta=Pipeline-server VERIFICADOS** + dueño verificó leads/creación-veh en vivo) · §211 (fixes web pública). Vender = 100% Pipeline (markAsSold clásico oculto; comisión vía `commissionSnapshot`; captura financiera completa = restructura TODO-25). ⏳ Dueño: push branch + Ctrl+Shift+R + re-verif público. dealers F2 FROZEN. Follow-up: testimonios home (diseño).
> 🏗️ **AHORA ④ RBAC departamental (§193.4)** — diseño LISTO (comité `winqw8w66`, crudo bóveda `2026-06-17-diseno-RBAC-*`) → **pasada adversarial consejo externo (§15/Gemini) → implementar** (DESBLOQUEADO: gate vehículos ✅). Clave: reusar custom-claims Bersaglio (claim `role`+reconciler) + blueprint depto skill `crm-architect`; single-tenant→depto=`teamId`; dueño INAMOVIBLE (CEO §70/§72). Luego cutover (stub redirect, NUNCA borrar admin.html) → E6.5. **Directiva dueño 17/06: NO preguntar qué opción — decidir por plan+comité+Gemini+investigación y proceder (memoria).**
