...
| **TODO-32** | **ESCALABILIDAD del cerebro ⟦OPUS-4.8⟧** — comité+Gemini+debate → plan ESCALONADO (no rebuild). **E1-2 ✅ (§228)**: `brain-index.mjs`=GUARDIÁN de completitud (cutover REFUTADO=lossy: índice a mano tiene riqueza+entradas especiales irreemplazables) + tope `00` 36k→48k (on-demand NO se comprime, ratchet §173). **Tombstoning ✅ cars-local** (`⛔ REEMPLAZADO POR §M` + guardián valida destino; 1er caso §111→§112, anti-Data-Rot). **Resta E3 (cross-brain, gate dueño)**: genoma frontmatter (índice lossless completo) + kernel ×brains + tiering. Diseño→bóveda. | 🔄 E1-2+tombstone ✅ · E3 ×brains pend | frontmatter→kernel×brains |
