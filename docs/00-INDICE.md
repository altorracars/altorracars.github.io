...
| §247 | **F-4 (1/3) "Lo que no entendí" (visor `unmatchedQueries`) en admin-app ⟦OPUS⟧** — arranque de F-4 Comunicaciones por el incremento más tratable (strangler intra-fase): Unmatched 380L < KB 933L < Hub 2979L. Visor realtime (filtros Sin revisar/Todas/Promovidas + conteos, optimistic marcar-vista/eliminar) = señal de FUGA de leads del bot. RBAC gateado EXACTO como rules (read/promote/delete) — no pinta botones que el server rechace. "Crear FAQ" degrada honesto hasta portar KB (cablea el prefill ahí). Patrón visor `auditoria`. Verif `?mock=1` OK, 0 errores. **dist gateado**. Sigue F-4(2/3)=KB → (3/3)=Hub (F-0.5 RTDB ya listo). | 44118 |
