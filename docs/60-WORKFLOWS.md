...
| **W-12** | **Auditoría de código viejo (cuarentena anti-Knight-Capital)** | Antes de mover/borrar CUALQUIER archivo "muerto" (css/js/functions/html legacy) | Borrar algo VIVO (la receta de 2.8 confundía `animations.css` muerto con `animaciones.css` VIVO en 27 páginas) o dejar trampas (botón llamando una function borrada) | (1) CENSO por referencias reales: `git grep -l --fixed-strings <archivo>` sobre `*.html *.js *.mjs` excluyendo docs — los hits en comentarios de versión (SW/cache-manager) NO son consumidores; (2) clasificar: cero refs → cuarentena a `_legacy/` (`git mv`, no delete) · consumidor solo-legacy → difiere al cutover del consumidor · VIVO → no tocar; (3) al mover: actualizar TODOS los consumidores + `node --check` + preview con red limpia (cero 404 locales); (4) remociones grandes por script de anclas con verificación y aborto (no edits a ciegas); (5) functions → `npm run deadcode:check`. Skill `anti-codigo-muerto` (×4 repos). ADR §273 |
| **W-13** | **Auditoría anti-fugas (dinero)** | Antes de lanzar / tras incidente / barrido periódico de flujos de PLATA (caja · pagos · stock con valor · arqueos · saldos · comisiones) | Fugas en las COSTURAS: duplicados por carrera, reversas tragadas por replay-guard, doble cobro invisible, arqueo ciego, robo enmascarable | (1) Mapa del dinero (¿quién escribe? ¿qué vistas leen? ¿cuál es la autoridad?); (2) N auditores READ-ONLY en paralelo con: **lista CERRADA de archivos** · formato obligatorio **`archivo:línea` + escenario paso a paso** · regla **anti-humo** ("si no trazas el escenario completo, NO lo reportes") · **tope ≤8 hallazgos** + declarar SANAS las áreas limpias; (3) **SIEMPRE verificación adversarial PROPIA de cada P0/P1** ANTES de arreglar (probada 2026-07-10 bersaglio §181: 4/6 P0 reales, 2 descartados al verificar — los auditores alucinan rutas plausibles); (4) fix + test de integración del ESCENARIO (no del paso) + datos históricos por asientos inversos auditados, jamás editar/borrar. **SSoT del método = skill `auditoria-financiera`** (7 invariantes del dinero) + `comite-expertos §4b`. L-73 |
...
- El reflejo always-on equivalente lo origina **el escritor único del kernel/§G = inmobiliaria-operador** desde 2026-07-10 (traspaso §302 + kickoff §7; antes cars-operador, L-31). Cars = peer EN PAUSA, NO edita el kernel.
