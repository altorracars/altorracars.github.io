...
| **TODO-32** | **ESCALABILIDAD del cerebro ⟦OPUS-4.8⟧** — comité+Gemini+debate → plan ESCALONADO (no rebuild). **E1-2 ✅ (§228)**: `brain-index.mjs` = GUARDIÁN de completitud (cutover REFUTADO por verificación = sería lossy: el índice a mano tiene riqueza + entradas especiales irreemplazables) + tope `00` subido 36k→48k (índice on-demand NO se comprime, ratchet §173). **Etapa 3 ABIERTA**: genoma frontmatter (índice lossless completo) + tombstoning anti-Data-Rot + tiering + kernel ×brains (gate dueño). Diseño→bóveda. | 🔄 E1-2 ✅ §228 · E3 pend | frontmatter→tombstoning→×brains |
