...
>
> 🏗️ **CMS/Dinamismo (TODO-23) EN CURSO ⟦OPUS-4.8⟧** — editor=**Opción A** decidido. Escaneo Paso 1 + arquitectura **comité v4** ✅ (bóveda `…cms-escaneo-paso1` + `…cms-arquitectura-comite-v4`). **Pase Gemini**: prompt entregado al dueño EN CHAT (corriendo; advisor-only). **FASE 0 seguridad COMPLETA** (gate `SSG_SELFTEST` con dientes, pruebas negativas): 0.1 safeJsonLd 6 sinks · 0.5 escapeAttr`<>` · 0.2 storage.rules anti-svg (DESPLEGADO) · 0.3 anchor-guard marcas **+FIJÓ bug SEO vivo** (marca.html sin `<meta robots>` → canonical/OG/Twitter NO inyectaban; +robots → activado) · 0.4 netlify→_legacy. **Pend 0.2b purga svg viejos = GATEADO dueño** (gcloud local sin auth → consola Firebase o `gcloud auth login`). **Gemini integrado** (CONVERGE v4; refuté templating-engine); sus nuevos → FASE 3: concurrency-group · bake-integrity (`</html>`+min bytes) · health-check `lastBaked` · **cron→`firebase-admin`+SA GATEADO dueño**. **SIGUIENTE = PASO 0** (ADR: CMS en `admin-app/src/modules/cms-dinamico/`, flat CONGELADO) → **FASE 1** (`content.edit` + fixture paridad escape + mapa campo→sink) → **cobaya** `aboutBrand` en `marcas/` (Hueco A: index no horneable). Detalle → bóveda v4+gemini.
