...
| §150 | **Consistencia de diseño global** (post-QA): de-blue→**near-black cálido** (`#0D0B09` en 9 CSS, `--cin-bg` intacto) + skip-link §149 REMOVIDO (cliente, A11Y-04→5/6) + QuickTools dock siempre visible + dropdown "Vehículos" abre por hover CSS en todas las páginas. **§150.d FIX layout dropdown**: panel colapsaba a 120px por regla universal `*{max-width:100%}` (style.css:6450) vs `.nav-dd` 580px → `.nav-dd-pro{max-width:none}`, verificado por render local (120→580px), desktop-only sin regresión móvil. **§150.e** eliminado "Camionetas" duplicado (=Pickup). **§150.f (Opción A)** dropdown single-column: `busqueda.html` IGNORA `?tipo=` (Nuevos/Usados no filtraban) pero el filtro nuevo/usado YA existe en el panel "Tipo de Vehículo" (`v.tipo`=condición) → eliminada la columna "Por condición"; queda "Por categoría" + CTA "Ver todos", panel 580→300px. §150→§150.e commiteados (`56bd195`); §150.f SIN commit. Cache `v20260602140000` | 42781 |
| §151 | **Evaluación crítica "Cerebro V5"** (propuesta externa Antigravity): pedido de ser crítico con una reforma del cerebro. ADOPTADO (lean): reflejo de **saturación por SÍNTOMA** fundido en §G.2 🔴 Trigger (loops/contexto saturado → relevo curado > `/compact`) + bitácora **"🚫 Callejones sin salida"** en la convención de handoff de `10`. RECHAZADO con razón: contador de turnos/"Entropía" en `05` (turnos≠saturación, no medible, tope 25/25), `HANDOFF.md` como nodo (redundante con `10`), `docs/skills/` (choca con `skills/` existente), inyecciones verbosas a `CLAUDE.md` (>320), proactividad obligatoria (ya `spawn_task`), ROI inventado. Net-neutral, sin cache bump → M-08 | 42832 |
