...
> 🔜 **TODO-34 — Pipeline Decisión Fuerte COMPLETO ✅ (3 capas), pend confirmación dueño + impl.** Verificado: el "híbrido" viejo = **DualCore + Free Core ~5,600L NLP a mano** (lo que FALLÓ: desalineado, no entendía); chatLLM **sin App Check** + inventario-en-prompt + **sin Tool Calling real**. **Comité #2 (4/4→B) ↔ Gemini (→A)**: discrepancia. **VEREDICTO FINAL = Opción A** = **solo-LLM + Tool Calling real + botones tontos de navegación**, guards-first (A↔B parcialmente semántica; con guards las ventajas de B son marginales; A gana por mantenibilidad + corte limpio del determinista + honra el instinto "solo LLM" del dueño; lo que faltaba era Tool Calling). **Precio refutado a Gemini**: Haiku 4.5 = **$1/$5** (no $0.25/$1.25 de 3.5 Haiku). **Anti-bot: captcha visible = cosmético → App Check ENFORCE + hard cap server-side ~$15/mes** (App Check NO para Selenium → el cap ES el salvavidas). Plan F1→F5 en el spec. **F1 = frenar hemorragia** (quitar `throw` de normalizeSolicitud → lead `incompleto`/'bot' cero-pérdida + App Check enforce + contador gasto). Crudos bóveda `…comite-arquitectura…` + `…gemini-redteam…`. **PEND: confirmación dueño (A · $15/mes · App Check) → arrancar F1.**
