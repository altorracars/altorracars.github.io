...
> ✅ **Cerrado previo**: F-4 Hub (§249) · F-5 fugas LIVE (§250) · Hub SMOKE ✅ (detalle=mock, espera chat real). Deploys=míos (mem `feedback-auto-deploy-crm`). LLM/bot=saldo (TODO-34). **Bug FIXED+DEPLOYED (25/06)**: `onChatTransferred` (functions §88) trataba `fcmTokens` como string pero son objetos `{token,…}` → push de transferencia fallaba. Fix = extraer `.token` (patrón de `onChatEscalated`); `functions:onChatTransferred` desplegada a prod. **DATO durable**: `usuarios/{uid}.fcmTokens[]` = OBJETOS `{token,deviceLabel,addedAt,lastUsedAt}` → TODO consumer extrae `.token`; ya consistente (onChatEscalated + onChatTransferred + admin-fcm.js + admin-app/fcm.js).
