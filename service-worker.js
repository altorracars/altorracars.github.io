...
const CACHE_VERSION = 'v20260511160000'; // §51 — Bot Telegram NO enviaba alertas: causa raíz era el WEBHOOK del bot NO configurado. Sin webhook, Telegram no enruta /start de los asesores a nuestra Cloud Function linkTelegramChat → telegramChatId nunca se persistía → onChatEscalatedTelegram tenía eligible=[] → return silencioso. Fix: (1) Nueva Cloud Function callable setupTelegramWebhook (super_admin only) que configura el webhook con un click. (2) Nueva Cloud Function callable getTelegramWebhookStatus para diagnóstico. (3) Botón "Configurar webhook" en Mi Perfil → Integraciones (visible solo super_admin) con auto-check de estado al cargar perfil. (4) onChatEscalatedTelegram con logging extra cuando eligible.length === 0 para diagnóstico (3 razones posibles enumeradas). (5) telegramSkipReason persistido en doc del chat para auditar.
