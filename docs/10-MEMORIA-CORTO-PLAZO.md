...
> 🏗️ **IMPLEMENTANDO ④ RBAC departamental (§193.4)** — diseño validado (comité+adversarial); **blueprint ④a en bóveda `2026-06-18-RBAC-4a-blueprint.md`** (LEER al retomar). **PASO 0/§213 HECHO+DEPLOYADO** (capa CF del dueño-inamovible `isOwnerData`, completa §212 = 3 capas). **Decisiones CONGELADAS** (no re-preguntar): doc-read (claims→Fase5), `permissions[]` se queda, `nivel` per-usuario (CF no lo pisa), `dataScope` se-escribe-NO-enforce-en-④a, `departments/` colección, rules=la-frontera. **SIGUIENTE PASO 1**: catálogo+seeder (rbac-catalog +Departamentos/nivel) → 2 backfill `nivel` → 3 rules+colección `departments/` → 4 sec-users selectores → 5 §71-nivel → 6 dual-portal. **④b (data-scoping) GATEADO** por Gemini + decisión negocio del dueño (¿visibilidad de datos o solo agrupar? Bersaglio retrocedió a roles planos → decidir con ④a VISIBLE, no en abstracto). Luego cutover → E6.5.
