...
| §244 | **HUB de Visibilidad: 7 skills + agente `seo-auditor` ⟦OPUS⟧** (TODO-42, pausa de F-2) — cars=HUB construye+propaga skills portables SEO·AEO·GA4·GSC·Maps·SSG·Feeds·Imágenes (vertical vía `tenant_config`). Arquitectura **IoC + core-funciones-puras + D′ vendored** (comité ACOTADO + Gemini VERIFICADO claim-a-claim: adopté IoC+versionado, refuté "A=cero-infra" [4 orgs≠same-org→PAT]; $0). Cero-demo · schema-en-HTML. Global+repo, catalogado. Bóveda: `…visibilidad-HUB-arquitectura-comite-crudo`. Propagación ×3 siblings = pend. | 44081 |
| §245 | **F-2 (6/6) módulo `ajustes` (tema + SEO/sitemap) en admin-app ⟦OPUS⟧** — CIERRA el GAP Config (admin.html pierde su última exclusiva de Config). (a) Apariencia: 6 paletas de acento (`.theme-accent-{id}` remapea `--gold-*`+derivados, persist localStorage, boot-applied). (b) SEO/sitemap: botón → callable **server-side** `triggerSeoRegeneration` (super_admin, secret GITHUB_PAT). **Decisión arq (Lente §3.8 + L-53 insight)**: NO porté el flujo client-PAT del clásico (smell + redundante: la regen YA es auto server-side) → cero PAT en el navegador, sin regresión. `vite build` temporal clean (129 mód) + preview `?mock=1#/ajustes` (theme picker end-to-end, 0 errores). **dist gateado a staging.** F-2 COMPLETO → sigue F-1 (bot v2 Vite). L-53. | 44094 |
| §246 | **F-3 módulo `dashboard` (Inicio) en admin-app ⟦OPUS⟧** — gap §2.C: el portal aterriza por fin en su **Inicio accionable** (antes en Bandeja). Landing snapshot (no realtime): saludo + **6 KPIs hero** (activos/SLA/sin-asignar/nuevos-hoy/pendientes/pipeline pond.) + **"Tus próximas acciones"** (NBA priorizado, toggle Míos/Todos, click→360) + Pendientes hoy + accesos. **Cero duplicación**: reusa `loadReports`/`reports`/`enrichAll`/`fetchPendingTasks`. router default `bandeja`→`inicio`. Verif `?mock=1`: render/KPIs/NBA/click-360/scope/responsive OK, 0 errores. **dist gateado** (source-only). Bug cazado→L-54. Sigue F-4/F-1. | 44106 |
| §243 | **F-2 (5/6) módulo `auditoria` (visor) en admin-app ⟦OPUS⟧** — el LECTOR de `auditLog` (port admin-activity): el portal ya ESCRIBÍA (audit.js) pero no LEÍA. Visor read-only: lista cronológica (onSnapshot 200) + buscador + filtro acción/usuario; ACTION_LABELS portados+extendidos (role_/dept_/workflow_). timestamp ISO verificado en ambos paneles (orderBy correcto). NO borra (inmutable). `vite build` 126 mód, preview+filtros verif. **dist gateado a staging.** Sigue F-2(6/6) Ajustes = ÚLTIMO. | 44069 |
| §242 | **F-2 (4/6) módulo `workflows`/Automatización en admin-app ⟦OPUS⟧** — port admin-automation: NO era CRUD (verificar-no-asumir) sino 4 reglas FIJAS toggle-able + visor de historial (`automationLog`). UI de gestión (no motor: el engine sigue legacy super_admin client-side). Toggle→`config/automationRules`. Flags: gap RBAC (workflows.edit no mapeado a las rules del doc) + motor debería ser server-side (→TODO-41). `vite build` 123 mód, preview verif. **dist gateado a staging.** Sigue F-2(5/6) visor Auditoría. | 44057 |
| §241 | **F-2 (3/6) módulo `departamentos` en admin-app ⟦OPUS⟧** — catálogo ④a `departments/{id}`: grid + modal (nombre/color/nivel/activo/desc), orden activos-primero/nivel-desc, id slug `dept_<slug>` idéntico al clásico. Guard §66 (no borrar con userCount>0, UI+rules). Port limpio de admin-departments (sin icon-picker lucide). `vite build` 120 mód, preview `?mock=1` verif (Inactivo + guard). **dist gateado a staging.** Sigue F-2(4/6) Workflows. | 44045 |
| §240 | **F-2 (2/6) módulo `roles` en admin-app ⟦OPUS⟧** — CRUD de `roles/{id}` + matriz de 82 permisos (9 categorías, toggle por categoría + indeterminate + críticos). Port limpio de admin-roles + `domain/rbac-catalog.js` (SSoT compartido backend → copia ESM con aviso de sincronía hasta cutover). CEO solo-lectura; create slug, delete con guard userCount. `vite build` 117 mód, preview `?mock=1` verif. **dist sigue gateado a staging.** Sigue F-2(3/6) Deptos. | 44033 |
