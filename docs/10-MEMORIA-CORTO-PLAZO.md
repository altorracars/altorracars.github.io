...
> 🔜 **EPIC "ALTOR Hub v2" — FOCO ÚNICO (decisión dueño 23/06): terminar TODO esto (bot LLM + captura + UX widget + UX chat interno) ANTES de retomar CRM / panel admin / migración panel viejo / demás pendientes.** Detalle+fases → spec `2026-06-22-TODO34-…` §EPIC. Pipeline Decisión Fuerte completo (3 comités + Gemini, crudos bóveda). **Arquitectura DECIDIDA = Opción A** (solo-LLM + Tool Calling + botones tontos + guards; se borra el motor 5,600L que falló). **Plan 6 fases REORDENADAS** (Gemini cazó: captura ANTES del bot, o la cohorte LLM choca con el form viejo de cédula): F1 guards+frenar-hemorragia (App Check enforce + **TECHO GLOBAL de gasto manual+auto-enforce+modo-captura** [el muro real; identidad=bache] + tope turnos + **truncamiento memoria ~6 msgs** + fix ingestión anónimos + **TTL** conversaciones anónimas) · F2 **captura** (**quitar CÉDULA**, mínimo nombre+celular, correo opcional, **fallback WhatsApp**=cero-pérdida [reusa `handoverToWhatsApp` ya existente], consent inline, saludo COLOMBIA) · F3 bot LLM+Tool Calling tras `engine:'v2'` (`submit_lead` con `lead_quality` GRATIS + **validación backend** anti-inyección + **guardrail PROHIBIDO NEGOCIAR PRECIOS**; el LLM PROMUEVE hot→asesor, nunca rechaza) · F4 UX widget (módulo PARALELO, no in-place) · F5 UX chat interno admin (fase separada) · F6 poda. **Pipeline completo = 3 comités + 2 Gemini, verificado por-claim.** Crudos: `…comite-arquitectura…` · `…gemini-redteam…` · `…comite-UX-captura…` · `…gemini-redteam-EPIC…`. **EN CURSO F1** (luz verde dada): **F1.a ✅** (functions/index.js chatLLM — techo global $ Anthropic-aware `checkMonthlyBudget`/`recordSpend` + memoria corta `MAX_HISTORY_MSGS=8`; config en `config/altorCost.monthlyBudgetUsd` default $15, contador `altorSpend/{YYYY-MM}`; sintaxis OK). **Verificado: `knowledgeBase/_brain.enabled=false` → LLM APAGADO → sin gasto vivo → deploy de F1 como UNIDAD (no a pedazos).** Sigue: F1.b ingestión cero-pérdida + TTL · F1.c App Check enforce (último, verificar firma web antes). NO crear `config/altorCost` vía MCP (callejón ↓ e); default $15 cubre.
...
| **TODO-34** | **REESTRUCTURA bot ALTOR → LLM-only + captura-CRM ⟦OPUS-4.8⟧**. **Diseño + Gemini + Comité ACOTADO ✅** → spec `…2026-06-22-TODO34-…` + bóveda (`22d52a9`/`242bc41`). Hallazgo: **el bot HOY NO está conectado al CRM**. **VERDICTO: F1+F2 NO listas** — el comité cazó lo que Gemini no vio: **Ley 1581 sin consentimiento** (plomería `mapConsent` ya existe ✅) · **GCP-billing≠gasto-Anthropic** (gate costo en chatLLM) · **Circuit-Breaker vaporware** (debe ser server-side Firestore) · **premisa: HÍBRIDO determinista→LLM vs 100%-LLM**. ✅ **Contratos verificados 23/06**: claim "colisión `sanitizeContactId`" REFUTADO; **GAP real = ingestión RECHAZA leads sin email/tel (anónimos se PIERDEN, no se fusionan)** → F1 captura handle antes de `solicitud`. Plan 5 fases en el spec. Pend: **3 decisiones dueño** (LLM-puro vs híbrido · techo costo · borrar js/ai/). | 🔄 F1/F2 NO listas | 3 decisiones dueño |
...
> GC ×16 (12-22/06): §184-§231 consolidados (→`99`/`00`/`30`/skills). 21/06: §223-226 + TODO-31 ✅. **22/06 (sesión maratón)**: §229 TODO-32 guardián ×brains (L-52) · §230 limpieza Firebase (**406 fotos purgadas** + Firestore −5 + functions 59→57) · §231 doctrina (**rama única `dev`** + **pipeline Decisión Fuerte**, skill `proceso-decision-fuerte`) · TODO-33 reconciliación parcial · **TODO-34 bot diseño triple-validado** (Gemini+comité→spec). Todo en `dev`.
