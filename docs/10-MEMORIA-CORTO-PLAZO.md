...
> ▶️ **RESUME**: F-0.5 ✅ · **F-2 (1/6) `usuarios` §238 + (2/6) `roles` §240 + (3/6) `departamentos` §241 ✅** (CEO candado, matriz 82 permisos, guard §66, port LIMPIO; build+preview verif). **⚠️ dist GATEADO a staging** (batch F-0.5+F-2, §237.6). Sigue **F-2 (4/6) Workflows** (`workflows/{id}`, rules `workflows.*`) → visor Auditoría (`auditLog` solo-lectura) → Ajustes → F-1 bot v2 Vite + staging + clean-slate E2E → F-3 Dashboard → F-4 Hub→admin-app → F-5 fugas (dedup `session:ID`) → F-6 cutover. Receta port → L-53. Detalle → §241/§237.7 + plan §6.
...
| **TODO-39** | **🧭 PLAN UNIFICADO un-solo-panel-admin ⟦OPUS-4.8⟧ — FOCO MAESTRO** (spec `…PLAN-UNIFICADO…`). Portal único `admin-app/`, apagar `admin.html`. Survey+comité+Gemini verif (6 hallazgos). **F-0.5 ✅ · F-2 (1/6) Usuarios §238 + (2/6) Roles §240 + (3/6) Deptos §241 ✅** (dist gateado a staging); camino F0..F6 → foco/RESUME arriba. **F4/F5 (bot v2) consolidado §236, su resto = F-1 aquí.** | 🔄 F-2 (3/6) | dueño: dinero/legal/go-no-go |
