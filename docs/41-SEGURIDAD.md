...
- **SEC-02 App Check (anti-spam)** ✅ CÓDIGO ESCRITO (modo monitor, 2026-06-08): site key reCAPTCHA v3 `6Lfz…` registrada (proyecto `altorra-cars`, dominios `altorracars.github.io`+`localhost`); activado en `js/core/firebase-config.js` (app **namespaced** `firebase.appCheck(app)`, carga **no-bloqueante** `.catch`) + `admin-app/src/core/firebase.js` (modular, build OK); cache bump `v20260608231721`; `firebase-messaging-sw.js` **intacto**. API verificada vs docs Firebase (compat acepta `appCheck(app)`). **Pendiente**: (a) cliente pega la clave SECRETA en Firebase App Check + Guardar; (b) merge a `main`; (c) observar % Verified 1-2 sem → enforce por-recurso. Init/rollout:
