...
> EN CURSO (todo en `dev` · detalle → spec §EPIC): **F1.a✅ · F2✅** (cédula fuera · WhatsApp gate · voz Colombia · botones) · **F3-a✅+F3-b✅** (Tool Calling: search_inventory + submit_lead→CRM consent-conservador) · **TTL✅** (anonimizar dry-run) · **wiring v2✅** (`engine:'v2'` cohorte). **functions DEPLOYED ✅** (#917 merged · Pages live · chatLLM v2 + TTL dry-run en prod) — drift code↔deployed RESUELTO. 🟡 **LLM aún apagado** (`_brain.enabled=false`). **Decisiones dueño 23/06:** techo $15 · App Check MONITOR · TTL anonimizar@30d · GO. 🔜 **ENABLE bloqueado por saldo Anthropic** (encender sin saldo solo cae a Free Core, no prueba v2). Al recargar: `_brain.enabled=true` en **admin clásico** (Cerebro→Activo) → verifico logs. **Pend dueño:** saldo → enable · consent (P4).
...
| **TODO-35** | **Código muerto/viejo (anti-Knight-Capital) ⟦OPUS-4.8⟧.** Veredicto+evidencia→**brief/spec**. **P0 gate `deadcode:check`✅ + P1 skill `anti-codigo-muerto`✅ (2026-06-23; 0 huérfanas live; skill instalada global+cars).** 🔜 propagar skill ×3 peers + P2 workflow bounded + ADR + auditar/limpiar bot viejo (botones→Free Core `:698`, finalize §57.x, diseño F4/F5) → luego frenar bot. M-19. | 🔄 P0+P1✅ | resto: peers·P2·bot |
| **TODO-36** | **Validador Live vía extensión Claude-in-Chrome (post-merge) ⟦OPUS-4.8⟧** — prompt a la extensión Chrome → valida en vivo (bot log/no, finalizar, respuestas) + observabilidad → yo actúo. Portable ×4. **Brief→spec. SESIÓN NUEVA** (disparador en el brief). | 🆕 documentado | sesión nueva |
