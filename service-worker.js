...
const CACHE_VERSION = 'v20260514090000'; // §82 Fase A Smart Update Prompts. SILENT_DEV_MODE flag ON por default (cero notificacion durante fase dev). showUpdateBanner reescrita: modal fullscreen rgba(0,0,0,0.72) + backdrop-filter:blur(6px) + inset:0 ELIMINADO. Reemplazado por pill sutil bottom-right 320px maxwidth con auto-dismiss 20s + boton dismiss X. Cross-tab dedup via BroadcastChannel altorra-cache-updates con fallback sessionStorage. Daily dedup 24h (vs 5min anterior). Patron industry-standard: Slack pill + GitHub auto-dismiss + Linear silent dev + Notion swap on navigation. Cliente puede toggle SILENT_DEV_MODE via localStorage.altorra_silent_updates 1 o 0 sin redeploy. Para produccion el flag se pasa a false en cache-manager. Cero JS admin, cero schema, cero deploy backend. Solo Ctrl+Shift+R cliente. Reduced-motion respetado.
