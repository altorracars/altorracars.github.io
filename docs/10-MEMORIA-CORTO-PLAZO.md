...
> 🔜 **PRÓXIMA SESIÓN (arranque fresco)**: decidir **TODO-34 #1** (bot **LLM-puro vs HÍBRIDO** — recomiendo híbrido por costo/seguridad) + techo de costo USD/mes → implementar **F1** (puente bot→CRM + consentimiento Ley 1581) + **F2** (modo OFF server-side, Circuit-Breaker en Firestore). Spec `…2026-06-22-TODO34-…`. **Antes de codear F1: verificar `sanitizeContactId`/`onSolicitudCreated`** (posible colisión de anónimos). Rama única **`dev`** (sync a `origin/main` al arrancar). Decisión Fuerte → skill **`proceso-decision-fuerte`**.
...
| **TODO-34** | **REESTRUCTURA bot ALTOR → LLM-only + captura-CRM ⟦OPUS-4.8⟧**. **Diseño + Gemini + Comité ACOTADO ✅** → spec `…2026-06-22-TODO34-…` + bóveda (`22d52a9`/`242bc41`). Hallazgo: **el bot HOY NO está conectado al CRM**. **VERDICTO: F1+F2 NO listas** — el comité cazó lo que Gemini no vio: **Ley 1581 sin consentimiento** (bloqueante legal) · **GCP-billing≠gasto-Anthropic** (gate costo en chatLLM) · **Circuit-Breaker vaporware** (debe ser server-side Firestore) · colisión `sanitizeContactId` (⚠️verificar) · **premisa: HÍBRIDO determinista→LLM vs 100%-LLM**. Plan 5 fases en el spec. Pend: **3 decisiones dueño** (LLM-puro vs híbrido · techo costo · borrar js/ai/). | 🔄 F1/F2 NO listas | 3 decisiones dueño |
