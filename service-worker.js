...
const CACHE_VERSION = 'v20260511070000'; // §46 — 3 fixes: (1) Dynamic Island estilo iPhone para admins activos: pill colapsado con avatares apilados + drag-to-reposition con click-hold + auto-save posicion en localStorage.altorra_dynisland_pos + vista expandida con lista detallada. Default top:80px right:24px (no obstruye contenido). (2) Threshold presence ahora 1min heartbeat + 10min stale (antes 2min/5min) — admins en tab background ya NO desaparecen fantasma de la isla ni de Sesiones Activas. (3) Fix ALTOR Hub padding: sec-concierge fixed con top:var(--atn-h,56px) left:0 (compat topnav §36), display flex column, tabstrip §42 arriba + .altor-hub flex:1. // §45 — 4 fixes: skip-to-content removido, FCM messaging-compat carga dinamica, admin-profile defensive helpers, RTDB rules require deploy manual.
