...
> ✅ **2.1 Ctrl+K HECHA** (04/07): `core/command-palette.js` — NAV canSee inyectado + registros lazy 60s (lead→360 L-27, vehículo→sección) + `--z-cmdk`. E2E preview mock ok.
> **▶️ SIGUIENTE (disparador: "continúa el plan maestro"): OLA 2 en orden** — **2.2 campana in-app + quick-create polimórfico** (topbar: mismos eventos FCM, dropdown+deep-link; ＋Crear → Lead/Cita/Vehículo por permiso) → 2.3 paginación/perf `fetchDealerStats` (congeló render 30s) → 2.4 export CSV+bulkbar+undo → 2.5-2.12 (plan §Ola2). Cadencia: implementar → build → preview `?mock=1` → commit(+dist!)+push+merge por bloque; suite emulador SOLO si tocas rules (L-63: puerto 8081 tomado = matar java zombi).
