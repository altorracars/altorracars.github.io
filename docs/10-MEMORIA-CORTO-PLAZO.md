...
> 🏗️ **CRM E0→E6 (§176, TODO-21)**. E0→E5 ✅. **E6 cutover gates 6/6** (§190-214 consolidados) · §211 web pública · §212/§213 dueño INAMOVIBLE 3 capas (rules + CF `isOwnerData`) DEPLOYADO · §214 gate-fix (ya NO encierra al dueño).
> ⏳ **PENDIENTE DUEÑO = `git push`+merge** (HEAD `f7ad4d7`, **ahead 2** = §214 + RELEVO; git real → `05`). Cierra merge cron↔cache + publica §211/§214 + ④a catálogo; luego **Ctrl+Shift+R**. Mientras no pushee, clásico aún encierra → workaround `admin.html#/users`. (Seguridad §212/§213 + functions ya LIVE de mi lado.) Vender=Pipeline (fin.=TODO-25). dealers F2 FROZEN. Follow-up: testimonios home.
> 🏗️ **④ RBAC departamental (§193.4) EN IMPLEMENTACIÓN** — blueprint ④a (bóveda `2026-06-18-RBAC-4a-blueprint.md`) = **SSoT de PASOS+decisiones congeladas+④b-gate** (LEER al retomar). DEPLOYADO: PASO 0/§213 (CF dueño-inamovible) + PASO 1 (catálogo 'Departamentos'+CEO nivel:100; DUPLICADO `rbac-catalog.js`↔`functions` → editar AMBOS). **PASO 2 ✅ §215** — callable `backfillNivelesRBAC` **DEPLOYADO** (vía elegida por el dueño; el `.mjs` daba PERMISSION_DENIED, ADC→bersaglio L-43) + botón "Backfill niveles" en sec-roles del CLÁSICO (§214: portal nuevo sin users/roles). Helper PURO `shared/rbac-foundation.js` (9 tests) + revisión adversarial 3 lentes (2 fixes). ⏳ **dueño: push de `admin-roles.js` → 1 clic "Backfill niveles"**. **SIGUIENTE PASO 3** (rules: helpers `getUserNivel/Dept/Scope` + bloque `departments/`) → 4 sec-users → 5 §71-nivel → 6 dual-portal. **Carry-forward §215.7**: PASO 4 verificar que `users.edit` no escriba `permissions:['*']` a un tercero · ④b coaccionar `nivel` a int. Luego cutover paso 24+ (stub redirect, NUNCA borrar admin.html) → E6.5.
