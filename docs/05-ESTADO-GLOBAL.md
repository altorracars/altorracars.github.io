...
| **Branch activa** | `refactor/estructura` (en sync con su origin). §156 + **§157 (fix hero) + `crm-handoff`** commiteados (último `9a3b245`); al cierre solo `00`/`10`/`99` (consolidación §157) sin commitear. **El fix del hero está en `refactor/estructura`, NO en `main`** → falta llevarlo a `main` para desplegar (GitHub Pages sirve `main`). `origin/main` avanza por cron/PRs. |
...
| **Deploys backend pendientes** | ✅ **Fase 1 CRM DESPLEGADA** (2026-06-05): `functions:onSolicitudCreated` (ingestión canónica) + `firestore:rules,firestore:indexes` (`contacts`/`leads`/`activities`/`failedIngestions`) **LIVE** en `altorra-cars`. Commiteada+pusheada por el cliente. ⏳ Falta E2E **visual** (vía Bandeja, Fase 2); smoke admin-SDK no corrió local (sin ADC). |
