...
> 🏗️ **CRM — plan E0→E6 (§176, TODO-21). E0 ✅ · E1a ✅ · adelanto E3 ✅ (§180) · E1b ✅ (§181)** — Pipeline v2 COMPLETO y deployado: etapas v3 con gates (Rules+UI+**paridad 7×7**), trigger `onDealUpdated` (E2E live: chip proyectado en 3s + lifecycle + activity determinista — **la divergencia Bandeja↔Pipeline es IMPOSIBLE**, triple capa), diálogo de conversión F7 + `anularConversion` (deshacer server-side), undo 10s, descarte con razón. F37 confirmado por su cron real (`sentTelegram:true` — el push llegó al dueño). RESTORE ensayado en emulador ✅ (gate cumplido). Migración v3 = no-op verificada (0 docs v2). **Siguiente: E2 — Calendario unificado** (orden interno OBLIGATORIO: F21.1 auditar configs → F21 SSoT+overrides por asesor → F17-resto rebuild+purga → F16 proyección `startAt`+backfill `_migration` → F18/F19 bloques 30min+tupla al confirmar+WhatsApp-first tokenizado+hold-expiry → F20 job 4 patas → F28 chequeo diario auto-reparador). ⏳ Cliente: (1) merge+push de la rama; (2) borrar `leads/_test_sla_e1a` (pruébalo: ⋯ → Eliminar definitivo, ya está mergeado); (3) anunciar F42; (4) CAUSA del billing-disabled.
>
