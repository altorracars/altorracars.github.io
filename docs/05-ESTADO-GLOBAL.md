...
|---|---|
| **Build** | 🟢 `main`==`dev`: PLAN UNIFICADO COMPLETO (§238-256) — portal único `admin-app/` LIVE; F-6 cutover ✅. Aliados TODO-25/§259 VALIDADO LIVE ✅ 29/06. **Queda (dueño)**: `ZZZ PRUEBA` (deal+veh.47) · FCM · dead-code (~3-jul). **EN CURSO (03/07)**: TODO-52 — emoji→SVG COMPLETO (§260-262) ✅ · **Fase C ref+mockups** (§264) ✅ · **pulido premium Gemini-verificado** (§265: elevación/pill/drag/tabular) ✅ · cerebro SHARDEADO (§263). EPIC=mandato permanente → seguir puliendo LIVE vs la referencia. Bot=saldo. |
