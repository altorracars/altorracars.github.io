...
- Copy dev restante: **voseo argentino** → tú-Colombia (skill `catalogo-voz`); "(demo)" inline; "13 listas" hardcode→derivado. **🔬 SCOPE mapeado (09/07):** el grueso del voseo está en el **BOT PÚBLICO** (`js/ai/small-talk.js` ~15+ · `vehicle-guide.js` · `inventory-search.js` — copy de salida customer-facing) + el **admin LEGACY** (`js/admin/*`: admin-auth/calendar-tabs/anomaly/fcm…); en `admin-app/src` solo 3 (`cerebro.data.js:87` FAQ pública · `perfil.ui.js:386` · `hub.ui.js:516`). **⚠️ `js/ai/intent.js` = PATRONES DE RECONOCIMIENTO de input (tenés/podés/etc. que el bot DEBE seguir matcheando) → NO cambiar esos.** Pase con juicio por-instancia (bot-output→tú · reconocimiento→intacto · "acá/dale" son OK en Colombia), coherente/completo, NO sweep ciego.
