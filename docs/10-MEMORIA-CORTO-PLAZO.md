...
> **HECHO+DEPLOYED 29/06 d (prod ✅):** §218 defensa roles (cliente NO crea `'*'`/isSystem, 6/6 tests) + drift R8 hardening (crmPurge/crmBackup/triggerSeo → 3 formas). `firebase deploy --only firestore:rules,functions:crm*,triggerSeo` OK. Owner-takeover ya estaba amurallado (§212/§217); subset-check = decisión de diseño del dueño (NO implementado — ver brief P0-SEC-2). Falta consolidar a ADR §NN.
> **SIGUE:** **dataScope (P0-SEC-1)** = único backend que queda — reglas+queries honran scope rol (asesor→own+depto · gerente/CEO→all). ⚠️ RESTRINGE reads → Decisión-Fuerte; emulador + alinear TODOS los `*.data.js` ANTES; deploy coordinado con live-val (sin staging). · **P1 resto** = emoji→SVG (acotar, ~408) · separar `--brand-gold` del accent picker · voseo→tú · **Fase C** mockups. **Owner-delete NO bloquea** (completo; dueño purga ZZZ hoy).
