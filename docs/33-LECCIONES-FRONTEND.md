...
- **Contraste**: la spec culpó a `.alt-footer-bottom-link{--ink-text-faint}` (#8A8A95, ~5.6:1); el ganador REAL de especificidad era `style.css .footer-legal a{rgba(255,255,255,0.22)}` (0-1-1 > 0-1-0) = **1.83:1** sobre `#030303` (falla AA). Receta: mide el `getComputedStyle().color` REAL, BLÉNDEALE el alpha sobre el 1er ancestro con bg opaco, y arregla la regla que GANA (0.22→0.62 = 7.81:1 AAA) — no la primera que lees.
- **Rol**: `role="menu"` exige hijos `role="menuitem"` (axe `aria-required-children`; sin ellos = "árbol a11y mal formado"). Barra de lanzadores/acciones ≠ menú de comandos → `role="toolbar"` (sin hijos obligatorios) + `aria-labelledby` + disclosure (`aria-controls`/`aria-expanded`). `menu` solo para menús reales (el bot `#cncHeaderDropdown` sí lo es).
- **Gotcha verif**: el SW cachea CSS viejo en preview → `document.styleSheets`=0.22 mientras `fetch(no-store)`=0.62. Verificar de verdad = unregister SW + `caches.delete()` + hard-reload. **Familia**: L-16/L-21 (cascada) · L-20/L-23/L-54 (medir) · §280.

### L-67 · Hero de CSS `background-image` = TRAMPA de LCP → usar `<img>`/`<picture>` (TODO-53 P3) ⟦OPUS-4.8⟧
- `background-image` se descubre TARDE (tras el CSS render-blocking + layout), no admite `fetchpriority`, y NO usa el preload responsivo `imagesrcset` (ése es para `<img srcset>`) → LCP altísimo (**22.6s móvil**), LCP element = el `<div>` del bg. Fix: hero = `<picture>` real (`<source>` AVIF + `<img>` WebP srcset, `sizes=100vw`, `fetchpriority=high`) → el preload COINCIDE (Load 6ms); UN solo formato (AVIF) para no doble-descargar; `object-fit:cover`/`object-position` ≡ `background-size/position`; `filter`/`transform` en el CONTENEDOR (afectan img+pseudos igual). **Verifica el LCP con TRACE real** (el observer del preview-MCP dio 0, falso). Detalle → ADR §286.
