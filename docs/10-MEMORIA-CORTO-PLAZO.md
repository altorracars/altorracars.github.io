...
> 🏗️ **AHORA ④ RBAC departamental (§193.4)** — diseño LISTO + **pase adversarial HECHO** (`wuue9vkhr`): cazó 2 huecos FATALES (el dueño NO estaba inamovible server-side) → **§212 fix DEPLOYADO** (owner-guard usuarios + isSystem-guard roles; test 7/7; prereq resuelto). **SIGUIENTE: IMPLEMENTAR ④** — síntesis+plan+prompt-Gemini en crudo bóveda `2026-06-17-RBAC-adversarial-*`; reusar custom-claims Bersaglio + blueprint `crm-architect`; single-tenant→depto=`teamId`; matriz granular sección×acción. Notas impl: rules=la-frontera (no UI), budget-rules MEDIR en emulador, `dataScope` critical:true, actualizar `41-SEGURIDAD`. Luego cutover (stub redirect, NUNCA borrar admin.html) → E6.5.
