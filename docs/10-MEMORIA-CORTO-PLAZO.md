...
> 🔜 **TODO-34 — Decisión Fuerte EN CURSO** (pipeline `proceso-decision-fuerte`). **DATO NUEVO dueño 23/06: el híbrido determinista YA se intentó y FALLÓ** (desalineado, mal inventario, no entendía). Verificado: el "híbrido" viejo = **DualCore + Free Core ~5,600L NLP a mano** (lo frágil que falló); chatLLM **sin App Check** + inventario-en-prompt + **sin Tool Calling real**. **Comité #2 arquitectura ✅ (4/4 → Opción B-moderno)**: router = **chips/UI + LLM+Tool Calling** (no NLP a mano), `search_inventory`/`create_lead`/`book_visit`, inventario NUNCA en prompt. **Captcha-de-UI = COSMÉTICO** → el gate real = **App Check ENFORCE en la función** (bot hace POST directo saltándose el HTML). Costo: B 4-5× más barato, **tope $12-15/mes** + gate server-side Anthropic-aware. Orden: **F1 cero-pérdida PRIMERO** (handle + fallback `lead_anonimo`; la ingestión RECHAZA leads sin email/tel → anónimos se pierden) → guards → engine v2 tras flag A/B → poda determinista AL FINAL. Crudo bóveda `2026-06-23-TODO34-comite-arquitectura-CRUDO.md`. **PEND: capa 3 Gemini (la corre el dueño) + 3 decisiones reencuadradas** (confirmar B · techo USD/mes · aceptar App Check>captcha).
