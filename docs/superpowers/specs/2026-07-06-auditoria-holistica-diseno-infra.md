...
- ✅ **P3.3 forced reflow del carrusel** (§289, `76b01728`): 2× `updateArrows` escribían `.disabled` y leían `scrollWidth` (write→read=reflow) en cada scroll/resize → fix = batch lecturas-antes-escrituras + rAF-throttle. Verif móvil 4x: 248→219ms (~12%). Diagnóstico peso-0; localhost sub-representa. **⚠️ Antes de más P3: re-medir PageSpeed LIVE** (el LCP 22.6s→sub-1s debió mover el score; medir el real).
