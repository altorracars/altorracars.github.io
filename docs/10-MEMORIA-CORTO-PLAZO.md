...
| **TODO-37** | **🔴 P0 leads anónimos perdidos ⟦OPUS-4.8⟧** — gate guest→`updateSoftContact`→regla UPDATE `solicitudes` solo-admin→400→`.catch` mudo→lead perdido (cazado por validador, cruce bot↔CRM). **Decisión Fuerte** (spec): verif+comité+Gemini✅. **Fase0✅** (`f2cf6ee` gate honesto+rescate WhatsApp+stop-placeholder) + **Fase1✅** (`f0d7463` Option **G**=gate CREATE completo; reglas `+consentGiven` DESPLEGADAS; SIN callable/duplicado — el phantom no ingesta, normalize.js lanza sin tel/correo). 🔜 re-validar live tras merge. | 🔄 Fase0+1✅ | merge+re-val |
