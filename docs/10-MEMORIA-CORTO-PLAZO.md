...
> ⏳ **PENDIENTE DUEÑO = SOLO merge a `main` (GitHub web)** — Claude ya pusheó `72899a9` (PASO 2 + §211/§214 + ④a); el merge publica todo; luego **Ctrl+Shift+R**. Mientras no se mergee, clásico aún encierra → workaround `admin.html#/users`. (Seguridad §212/§213 + functions ya LIVE.) Vender=Pipeline (fin.=TODO-25). dealers F2 FROZEN. Follow-up: testimonios home.
> 🏗️ **④ RBAC departamental (§193.4) EN IMPLEMENTACIÓN** — blueprint ④a (bóveda `2026-06-18-RBAC-4a-blueprint.md`) = **SSoT de PASOS+decisiones congeladas+④b-gate** (LEER al retomar). DEPLOYADO: PASO 0/§213 (CF dueño-inamovible) + PASO 1 (catálogo 'Departamentos'+CEO nivel:100; DUPLICADO `rbac-catalog.js`↔`functions` → editar AMBOS). **PASO 2 ✅ §215** — callable `backfillNivelesRBAC` **DEPLOYADO** (vía elegida por el dueño; el `.mjs` daba PERMISSION_DENIED, ADC→bersaglio L-43) + botón "Backfill niveles" en sec-roles del CLÁSICO (§214: portal nuevo sin users/roles). Helper PURO `shared/rbac-foundation.js` (9 tests) + revisión adversarial 3 lentes (2 fixes). ⏳ **dueño: merge a `main` (web) → Ctrl+Shift+R → 1 clic "Backfill niveles"**. **SIGUIENTE PASO 3** (rules: helpers `getUserNivel/Dept/Scope` + bloque `departments/`) → 4 sec-users → 5 §71-nivel → 6 dual-portal. **Carry-forward §215.7**: PASO 4 verificar que `users.edit` no escriba `permissions:['*']` a un tercero · ④b coaccionar `nivel` a int. Luego cutover paso 24+ (stub redirect, NUNCA borrar admin.html) → E6.5.
