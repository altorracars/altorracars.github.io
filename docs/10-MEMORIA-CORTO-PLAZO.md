...
> EN CURSO: **F1.a ✅** (`f747f5e`: techo global gasto Anthropic-aware + memoria corta) · **F2.a ✅** (`2345138`: cédula fuera, nombre+celular, apellido/correo opcionales, saludo Colombia). ⚠️ **LLM apagado en prod** (`_brain.enabled=false`); resto F1 = App Check enforce + TTL justo antes de F3 (foundation ✅ `firebase-config.js:146`). **F2.b ✅** (fallback WhatsApp en el gate, reusa `handoverToWhatsApp` + sweep voseo→Colombia ~20 strings). F2.a+F2.b **static OK, live pend post-merge** (L-08). 🔜 resto F1 (App Check enforce + TTL) → F3 bot LLM v2. **Pend dueño** (detalle en ledger TODO-34): techo $/mes · TTL · consent abogado(P4) · merge p/verif-live.
...
> (Histórico 12-22/06 §184-§231 → `99`/`00`/`30`.) **22/06**: rama única `dev` + pipeline Decisión Fuerte (§229-§231) · limpieza Firebase · TODO-34 triple-validado. **23/06 (EPIC ALTOR Hub v2)**: pipeline TODO-34 (comité#2→B · Gemini→A · **veredicto A** · comité#3 captura/UX · Gemini-EPIC→reorden captura-antes-del-bot + 4 guardrails) → 5 crudos en bóveda (índice en README archive) + spec §EPIC + memoria. **F1.a✅ · F2.a✅** · `proceso-decision-fuerte v2` · skill `meta-ads-diagnostico` ×4. **TODO-35 capturado** (anti-Knight-Capital). **F2.b ✅** (fallback WhatsApp en gate + voseo→Colombia ~20 strings). Todo en `dev`. 🔜 resto F1 → F3.
