...
|---|---|
| **Build** | 🟢 Catálogo cinematic + a11y **en producción**. Cerebro ENDURECIDO (§156). **🏗️ CRM: Fase 1 (ingestión canónica `onSolicitudCreated`) LIVE + Fase 2 (Bandeja Inteligente, app `admin-app/` greenfield Vite + Firebase modular) CONSTRUIDA** — `npm run build` verde, verificada en `?mock=1` (snapshot+eval), 2 bugs cazados+fixeados. ✅ DESPLEGADA. **+ Fase 3a (Pipeline `deals`) + Fase 3b (Agenda unificada) DESPLEGADAS a `main`** — verificadas `?mock=1` (54 módulos, **0 bugs c/u**). Portal nuevo: 3 superficies live (Bandeja+360, Pipeline, Agenda) + **Captura MANUAL de leads externos** (Meta/WhatsApp/TikTok/llamada/referido — reusa la ingestión, §162) verificada (57 módulos, 0 bugs). ⏳ Solo falta **E2E live** del cliente. `brain:check` SANO. Ver ADR §150–§162. |
...
| **Branch activa** | `refactor/estructura` (en sync con `main`). Fase 3a (`1e154c2`) + Fase 3b (`347c790`) mergeadas+LIVE. **Captura manual (§162) en commit de esta sesión** → merge a `main` (solo estático). **Auto-deploy por fase autorizado** (memoria `feedback-auto-deploy-crm`). |
