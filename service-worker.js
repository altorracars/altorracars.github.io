...
const CACHE_VERSION = 'v20260511150000'; // §50 — 3 fixes: (1) MOBILE NO SE CIERRA SESIÓN AL BORRAR CACHE: saveDeviceTrust ahora persiste deviceId (fingerprint deterministic = screen + WebGL + canvas + locale) + userAgent en Firestore trustedDevices array. isDeviceTrusted con Path C nuevo: si no hay token local pero server tiene un device entry con deviceId matching el browser actual + dentro del TTL → re-emit token a localStorage + cookie automáticamente. Resuelve el caso Safari iOS "Borrar historial y datos" que limpia localStorage+cookies+IDB. PC no tenía el problema porque "Vaciar caché" no toca storage. (2) Telegram + FCM ahora notifican DESDE EL MINUTO 0 — eliminado filtro asesoresAvailable > 0 en onChatEscalated y onChatEscalatedTelegram. Cooldown 5min anti-duplicado se mantiene. (3) Banner de queue ahora dice "1 a 10 minutos" (antes 5 a 10).
