...
> **⚡ FABLE EJECUTA TODO hasta ~07-jul** (directiva dueño 03/07; Opus después). ✅ **OLA 1 CERRADA** (§269/§269.8) · ✅ **OLA 2: 2.1-2.4 → ADR §270** (Ctrl+K · campana/quick-create · perf server-side · CSV×5+undo; bulkbar DIFERIDA por YAGNI §270.8). ⚠️ el `dist/` se COMMITEA por bloque. El undo vivo lo estrena la purga ZZZ del dueño.
> **▶️ SIGUIENTE (disparador: "continúa el plan maestro"): OLA 2 en orden** — **2.5 agenda: eliminar cita owner-only** (`citaAction` + `'delete'` transaccional que LIBERE el cupo + purga de canceladas — toca functions+rules → suite emulador) → **2.6 seguridad P2** (tren rules+functions: isOwnerDoc 3-formas · diff-guard usuarios ampliado · config/* least-privilege · whitelist events/automationLog · RTDB typing · kb.usageCount · triggerSeoRegeneration sin rama PAT) → 2.7-2.12 (plan §Ola2). Cadencia: implementar → build → preview `?mock=1` → commit(+dist!)+push+merge por bloque; suite emulador SOLO si tocas rules (L-63: puerto 8081 = matar java zombi).
