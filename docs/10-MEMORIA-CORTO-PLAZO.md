...
> **Hecho (pre-29d):** A.1/A.2-live/Fase-B✅ · PASE-1✅ · confirm.js 16/16✅ · P0-OWNER-DELETE✅ · errors.js COMPLETO (13 mód)✅ · `var(--gold)` fantasma MUERTO✅.
> **HECHO+DEPLOYED 29/06 d (prod ✅):** drift R8 hardening (crm*/triggerSeo→3 formas) + **§219 RBAC** (decisión del dueño, verbatim en brief P0-SEC-2): **owner-only** (`users.*`/`roles.*`/`settings.backup`/`settings.seo`/`audit.delete` NUNCA delegables — ni el dueño en un rol custom, solo su `'*'`) **+ subset** (otorgas ⊆ lo tuyo); UI matriz deshabilita owner-only ("solo dueño"). Suite **317/317**; `firestore:rules`+functions DEPLOYED. Consolidar a ADR §NN.
> **SIGUE (dueño dio GO 29/06):** **(1) dataScope (P0-SEC-1)** — reglas+queries scope rol (asesor→own+depto · gerente/CEO→all); ⚠️ RESTRINGE reads → emulador + alinear TODOS los `*.data.js` ANTES + deploy coordinado live-val (sin staging). **(2) Perfil paridad** — el admin VIEJO tenía MÁS (asociar **Telegram** p/notificaciones, **documento/cédula**, etc.) que NO porté → portarlo. **(3) Eliminar accent picker** (cambiar color del panel = inútil, dueño). · P1: emoji→SVG · voseo→tú · Fase C mockups. **Owner-delete NO bloquea**.
