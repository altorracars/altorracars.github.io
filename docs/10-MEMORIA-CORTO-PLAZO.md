...
>   - **EJECUCIÓN (plan v5 APROBADO 2026-06-09)**: **PRE-PASO-0 ✅** — (1) `brain-check.mjs` caps líneas→**chars** (dual) + sub-presupuesto BOOT (CLAUDE+05+10 = 44.5k c/~12.7k tok vs objetivo 31.5k/~9k → `↗ destilar`, **nodo 10 = target #1**); (2) linter partido **`--boot`/`--full`** (SessionStart ya NO lee el 99 ~43k L; `pre-commit`/manual corren `--full`). Verificado ambos modos SANO. Commits: `c7d1f83` (docs M1+2), `0450b0b` (chars), +`--boot`. **Próximo: PASO 0.(-1)** blindar base git (try/catch en cada `execSync`, `kernelHash` bootstrap-safe) → **PASO 0** mecanismo (`brain:diff`/`pull` + gates + meta-fixtures) probado en sandbox/inmobiliaria. 🚫 §R6 skills clasificó por-nombre (mal) → leer SKILL.md; NO junctions/symlink (§170 L40).
