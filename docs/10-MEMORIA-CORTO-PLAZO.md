...
> **⚡ FABLE EJECUTA TODO hasta ~07-jul** (directiva dueño 03/07; Opus después). ✅ **OLA 1 CERRADA** (§269+§269.8: código 100% + val.live; gaps/backlog nombrados ahí — responsive por otra vía, 8 secciones menores, tarea ZZZ huérfana). ⚠️ el `dist/` se COMMITEA por bloque.
> ✅ **2.1 Ctrl+K** (`core/command-palette.js`: NAV canSee inyectado + registros lazy 60s; lead→360 L-27) · ✅ **2.2 campana+quick-create** (`core/notifications.js`: registro localStorage 20 + badge + dropdown popover + deep-link, alimentada por fcm onMessage; topbar "＋ Crear" polimórfico por permiso — Vehículo NAVEGA a su sección, el wizard no es autocontenido). Ambas E2E preview mock ok.
> **▶️ SIGUIENTE (disparador: "continúa el plan maestro"): OLA 2 en orden** — **2.3 paginación/perf** (`fetchDealerStats` congeló render 30s → agregados server-side; `subscribeVehicles` limit; `fetchTeam` paginado) → 2.4 export CSV+bulkbar+undo → 2.5-2.12 (plan §Ola2). Cadencia: implementar → build → preview `?mock=1` → commit(+dist!)+push+merge por bloque; suite emulador SOLO si tocas rules (L-63: puerto 8081 = matar java zombi).
