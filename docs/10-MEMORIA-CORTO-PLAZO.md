...
> **⚡ FABLE EJECUTA TODO hasta ~07-jul** (directiva dueño 03/07; Opus retoma después). ✅ **OLA 1 CERRADA** (04/07): código 100% (§269: skeletons · combobox · onboarding · cola offline) + **validación live ejecutada (§269.8)** — combobox E2E real, checklist correcto, consola limpia. ⚠️ el `dist/` se COMMITEA por bloque (se me pasó 2 commits, curado).
> Gaps de la val.live (§269.8): responsive multi-viewport (el resize de la extensión NO cambia el viewport de captura → probar devtools-emulate) · 8 secciones menores · red parcial. Backlog menor ahí mismo (tarea ZZZ huérfana · select full-width · 🏷️ emoji-texto).
> **▶️ SIGUIENTE (disparador: "continúa el plan maestro"): OLA 2 en orden** — **2.1 Ctrl+K command palette** (saltar a secciones reusa NAV+canSee + buscar registros vía data layers, montada en el shell) → 2.2 campana in-app + quick-create → 2.3 paginación/perf `fetchDealerStats` (congeló render 30s) → 2.4 export CSV+bulkbar+undo → 2.5-2.12 (plan §Ola2). Cadencia: implementar → build → preview `?mock=1` → commit(+dist!)+push+merge por bloque; suite emulador SOLO si tocas rules (L-63: puerto 8081 tomado = matar java zombi).
