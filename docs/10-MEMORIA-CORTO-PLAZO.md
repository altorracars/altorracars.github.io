...
> Histórico 12-22/06 §184-§231 → `99`/`00`/`30`. **23/06**: EPIC TODO-34 → **Opción A** (`dev`: F1.a·F2·F3-a ✅ dormiente, pend deploy; 🔜 F3-b). Validador Chrome TODO-36 ✅ (§232: skill `validacion-live-chrome` + skills ×4+global + gate `skills:desc-check`). **1ª caza LIVE §233**: `_asesorJoinedAnnounced` sin declarar (split §119, strict) atrapaba al reabrir chat finalizado → fix + **re-validado ✅ 5/5**. **§234**: el logout no limpiaba `altorra_concierge_session` → chat del logueado visible al siguiente anónimo (equipo compartido, Ley 1581) → fix `auth.js _wipeConcierge`, **🔜 re-validar tras merge**. Skill = **CRÍTICO ADVERSARIAL** + cobertura login→logout→sin-login; gate `skills:desc-check`. Follow-ups: `confirm()`→modal F4/F5; logout 2 clics (race header); grounding=LLM off.
