...
> **Estado:** W-11 — **F1 ✅ + F2 ✅** (motor + chasis + iconos), en `main` vía dev; falta validación LIVE del dueño + F3 (diferido). Mandato: "avanza solo, solo párate para el prompt de Gemini" (gate ya pasado). Fecha: 2026-06-26. Modelo: Opus 4.8 ⟦rev-Fable⟧.
...
- 🔄 8. IMPLEMENTAR — **F1(a) selección múltiple + acciones masivas (Asignar/Contactado/Archivar) en Bandeja = BUILT + verificado** (preview ?mock=1: select/multi/limpiar OK, 0 errores; Vite build limpio 148 mód; aditivo vía clase `--selectable`, optimista+rollback). **Hallazgo (verify-real)**: la Bandeja real YA era fuerte (orden por urgencia/SLA/temp/NBA, 1-clic, SIN blur) → Gemini/comité criticaron una versión ASUMIDA; el gap real era multi-select, NO reescribir a tabla densa (la tarjeta es buena). **F1(b) "Nuevo lead" en topbar GLOBAL = BUILT + verificado** (commit `587737b`; preview ?mock=1: CTA gold 1ª acción → abre "⚡ Lead rápido"/3 campos, 0 errores console; Vite 148 mód limpio. Reusa `openQuickLeadForm()` —camino rápido §178, modal auto-contenido que NO acopla al inbox— gateado por `hasPermission('crm.edit')`, idéntico al gate de la Bandeja; subido del inbox al shell global). **F1(c) drawer móvil off-canvas = BUILT + verificado** (commit `da84051`): a ≤560px el sidebar se vuelve drawer deslizante (hamburguesa SVG + backdrop + focus-trap + Escape + cierre-al-navegar). **Tabulabilidad por CSS `visibility:hidden`, NO `inert`** (decisión §3.8/L-55: gestionar `inert` con un listener `matchMedia change` tiene fallo latente = nav muerto si el evento no dispara al cruzar a desktop; el CSS lo elimina por construcción). Verif con transiciones neutralizadas (artefacto headless L-20/L-23/L-28: el preview no avanza transiciones → inyectar `*{transition:none}` y leer end-states del cascade): cerrado/abierto/Escape/backdrop/navegar(Pipeline)/foco-retorno + **cruce móvil→desktop = nav visible+interactivo** (el fix). Desktop (rail @860) intacto (1280px: grid `256px 1fr`, sidebar `static`, hamburguesa/backdrop `display:none`). **F1 COMPLETA.** **F2 chasis (estructural) = BUILT + verificado** (commit `1c0aa07`): nav en 2 zonas con `<details>` (Primaria plana + grupos Sitio web·Comunicaciones·Ajustes) + **fix regresión `setActive`** (`activeBtn.closest('details.navgroup').open=true` → auto-expande el grupo del ítem activo; el gate de no-regresión del comité pt.2) + **degenera-a-plano** (grupo con <2 ítems visibles tras RBAC → plano, pt.4; verif contra roles reales de `roles.data.js`: Asesor→hub plano, Gerente→aliados+usuarios planos, Marketing→Sitio web acordeón) + persist localStorage (`altorra:navgroup:<id>`) + rail @860 = grupos como iconos (pt.8). Verif preview con transiciones neutralizadas (L-55): estructura/auto-expand/persist-cross-reload/rail/0-errores. **F2-iconos = BUILT** (commit `759c08f`): set SVG inline de 27 iconos (24 ítems + 3 cabeceras de grupo, trazo Feather/Lucide, currentColor, módulo `nav-icons.js` + helper `iconSpan` con **fallback a emoji** si falta un id); verif 27/27 SVG válidos (0 fallback/0 cajas rotas) + **mockup `show_widget`** entregado al dueño para aprobar el set visual. **F2 COMPLETA.** Falta verificación LIVE del dueño (capa 9). SIGUE F3 (opcional/diferido: palette Ctrl+K + estados vacíos).
