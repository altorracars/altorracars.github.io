...
- **FASE 0 — quick wins** ✅ SHIPPED §296 (`018ad1d8`): imágenes→webp/favicon real (−1.1MB) + preload 412KB eliminado · aria-label btnLogin/btnRegister · burbuja CTA close 28px + aria-label contenedor retirado · logos marca `onerror`→local. Verificado en preview.
...
- **FASE 1.2 fuentes** ⏳: consolidar familias (Manrope/Instrument Serif/Cardo), no-bloqueante + preload 1 woff2 + `size-adjust` (arregla CLS ord). NO quitar Poppins sin verificar uso legacy.
- **FASE 1.3** ⏳: CSS crítico inline + diferir los 9 render-blocking (truco `media=print` ya usado en índex 277-287).
- **FASE 2 — dieta JS** ⏳: (2.1) diferir Auth+AppCheck/reCAPTCHA+storage/functions/rtdb/analytics hasta interacción/idle (mantener app+firestore); (2.2) home Firestore `onSnapshot`→`get()`+limit; (2.3) diferir bot/IA hasta abrir bot; (2.4) GSI solo al login + desactivar FedCM (mata errores consola); (2.5) subset lucide; (2.6) fix reflujo `home-chrome.js:439`.
- **FASE 3 — build/minify** ⏳: minificar CSS/JS (−142/−32KB), podar CSS sin usar (−98KB; los 41KB reCAPTCHA se van con 2.1), categorías `image-set()` variantes optimizadas (−~800KB), width/height a 36 imágenes.
