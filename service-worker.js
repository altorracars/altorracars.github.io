...
const CACHE_VERSION = 'v20260510260000'; // §30 ADR-030 VISIONARY W3 — Bug fixes + Dynamic Island + FAB Material You + alt-toggle iOS + sparkles. Override de --admin-surface legacy a --vis-surface stack en stat-card/info-card/form-card/[class*=card]. Cleanup transition: all (caro) por transitions específicas spring. Dropdown menus radius 14 + Mica strong. Section padding consistente 28x32. Links color brand-300 con hover underline gold. Toasts Dynamic Island-style: radius pill + Mica + spring entry translateY+scale+blur + glow per type (success verde / error rojo / info gold). FAB Material You: gradient gold + glow stacked + hover scale 1.05 lift 3px. Sparkle on hover en hero-kpi (border gradient diagonal con stops dorados). alt-toggle iOS-style: 44x26 pill con thumb gradient blanco-gris + slide spring + on state gold gradient con glow. KBD premium con border-bottom 2px + inner shadow Mac-style. Loader conic-gradient con mask radial (anillo dorado rotativo). Progress bars con shimmer dorado infinite + box-shadow glow. Sidebar group-items expand spring + chevron rotate -90 collapsed. NBA dash items con border-left color por priority (gold/amber/gray). Pipeline lanes con bg gradient + header uppercase. Charts container border + radius. Section page-header breathe animation 8s. Mobile responsive blurs reducidos 16px. Print mode: oculta sidebar/topbar/overlays/FABs + bg blanco texto negro. High contrast a11y borders más opacos
