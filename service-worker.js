...
const CACHE_VERSION = 'v20260511080000'; // §47 — 3 fixes 2FA mobile: (1) reCAPTCHA "already rendered" fix: cleanRecaptchaContainer ahora reemplaza el elemento DOM completo por uno fresh con el mismo ID via parentNode.replaceChild — Firebase Compat v11 cachea internamente por containerId STRING y .clear()+innerHTML='' no invalida el cache (sobre todo Safari iOS). El nuevo Node DOM bypassa el cache → resend code funciona en mobile. (2) isDeviceTrusted con fallback graceful Path B §47: si server tiene array vacío (rules §43 sin deploy) PERO localStorage tiene token válido <30d, aceptar trust + warning explicito en consola. Resuelve loop infinito de 2FA en cada refresh para editores con rules pendientes. (3) saveDeviceTrust catch ahora alerta al editor con AP.toast warning si falla por permission-denied, explicando "rules pendientes" + comando firebase deploy. // §46 — Dynamic Island redesign + drag + Hub padding fix.
