...
> **🤝 TODO-54 PageSpeed (§296, spec `2026-07-08-pagespeed-audit-plan.md` = SSoT)** — 08/07 3ª ⟦OPUS⟧ dieta-JS: **2.1b ✅ `3b11ab10`** (diferir auth+SDKs; `auth/iframe.js` off-critical; nuevo `window.dbReady`; home-only) · **2.3 ✅ `73aadeaa`** (bot/IA ~18 archivos a idle) · **2.4 ✅** (GSI a idle; NO FedCM-off → preserva One-Tap) · **2.6 ✅** (reflujo parallax: scrollY al rAF) · **2.5 ✅ `12d51274`** (Lucide 82KB a idle, home-only; es defer no subset). **FASE 2 dieta-JS ~COMPLETA** (2.2 diferido: ya idle P14). **1.3 slice ✅ `dfb7e1b9`** (quicktools.css diferido; los otros 8 CSS son críticos above-fold). **RETOMAR = solo GATES/BUILD**: 1.3 CSS crítico core (gate FOUC, **necesita live** — extracción CSS crítico) · 1.1b animaciones (gate visual) · F3 minify+subset-lucide (build) · 1.2 fuentes (🚫 equipo) · F4 Cloudflare (dinero). Sin bump (js/core+public/home=networkFirst/SWR).
