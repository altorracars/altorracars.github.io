...
|---|---|
| **Build** | 🟢 `main`==`dev` `a070ef9e`: **PLAN MAESTRO = SSoT; 🤖 AHORA = OPUS 4.8 (Fable agotada) → arranque = §0+§0.b**. **OLA 0-1 ✅ · OLA 2: 2.1-2.8, 2.10, 2.12a ✅ (§267-§275; suite 374) → RESTAN solo GATES DUEÑO: 2.9 MFA · 2.11 App Check · 2.12b cutover `_legacy` (DIFERIDO §275.4). SIGUE ejecutable: OLA 3 web pública.** Gates dueño: saldo bot · purga clientes/suppressions · validar-live (supresión contactos, presencia §274) · CUÁNDO MFA · OK para borrar admin viejo. |
