...
>   - **Captura MANUAL de leads** (gap que el cliente señaló: leads externos Meta/WhatsApp/TikTok/llamada/referido NO entran solos), ADR §162. Form "＋ Nuevo lead" en la Bandeja → escribe `solicitudes` → **reusa la ingestión** (dedup+consent), cero backend. `channelOf` extendido + orgánico/pauta/campaña (ROI). ✅ verificada `?mock=1` (57 módulos, 0 bugs). En commit de esta sesión.
> **⏳ Único pendiente**: **E2E live** — `https://altorracars.github.io/admin-app/dist/` + login admin + recorrer Bandeja/Pipeline/Agenda + agregar un lead manual, con datos reales (L-08).
> **Próximo**: slice de **canales AUTO** (newsletter/cuenta/bot → canónico, completa la captura) **o** **Fase 4 (Reportes/KPIs** — ya hay atribución orgánico/pauta/campaña por lead) → luego migrar Inventario/Config al portal → **cutover**. Roadmap → `crm-handoff.md §9.5`.
>
