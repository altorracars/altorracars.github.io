...
| §211 | **Fixes render SITIO PÚBLICO (verif V6) ⟦OPUS-4.8⟧** — 3 sondas: reseñas (link header→`resenas.html`; sección home la borró SP-1=regresión), banner promocional (slot `#promoBannerSection` borrado por SP-1 → restaurado en index), carrusel marcas (`initBrands` rendea de cache viejo → `load(true)` refetch). NINGUNO del cutover. Cache bump. Staged para push. | 43573 |
| §212 | **FIX SEGURIDAD: dueño INAMOVIBLE enforced server-side ⟦OPUS-4.8⟧** — el pase adversarial de ④ RBAC (`wuue9vkhr`) cazó 2 huecos FATALES pre-existentes: `users.edit` podía degradar/bloquear al CEO + `roles.edit/delete` podía editar/borrar el rol `system_super_admin`. Fix `firestore.rules` (owner-guard usuarios + isSystem-guard roles, anti-spoof sobre `resource.data`) + test 7/7 (suite 205/205). **DEPLOYADO**. Prereq de ④. | 43597 |
| §213 | **④a PASO 0: capa CF del dueño-INAMOVIBLE ⟦OPUS-4.8⟧** — §212 cerró 2/3 capas; faltaba functions. `isOwnerData()` en `verifySuperAdmin` (reconoce dueño por rol/roleId/`'*'`) + guard target-is-owner en `updateUserRoleV2`/`deleteManagedUserV2`. node -c + deploy ✓. Inicia la implementación de ④a (blueprint bóveda `2026-06-18-RBAC-4a-*`). | 43619 |
| §214 | **FIX gate §210: no ENCERRAR al dueño fuera del clásico ⟦OPUS-4.8⟧** — `admin.html` redirigía a `/admin-app/.../banners` (el clásico restaura la última sección = migrada) → dueño encerrado SIN Usuarios/Roles (no portados al portal nuevo). `guardHash` ahora va a `dashboard` (clásico) en vez del portal; click-guard intacto. Workaround: `admin.html#/users`. Lección: gate de cutover NO debe encerrar secciones aún-no-portadas. node -c ✓. | 43637 |
| §215 | **④a PASO 2: backfill de fundación RBAC (callable 1-clic, no ADC) ⟦OPUS-4.8⟧** — la ADC local está ligada a bersaglio (L-43) → el script `.mjs` da PERMISSION_DENIED en cars; vía correcta = callable `backfillNivelesRBAC` (SA de Functions, botón "Backfill niveles" en sec-roles del clásico). Lógica PURA `shared/rbac-foundation.js` (1 dueño: callable + .mjs fallback + 9 tests). Dueño=100 INAMOVIBLE con aserción post-write; idempotente/no destructivo. Deploy ✓. Revisión adversarial 3 lentes → 2 fixes reales. SIGUIENTE PASO 3. | 43653 |
| §216 | **④a PASO 3: departamentos (rules 3a + UI admin-departments.js 3b) ⟦OPUS-4.8⟧** — bloque rules `match /departments/` (read/manage + guard §66 userCount=0) deployado (be68b74) + módulo CRUD `admin-departments.js` (DOM puro `el()`, sin innerHTML; clases reales `.role-card*`/`.roles-modal-*` verificadas vs CSS — cacé clases inventadas L-37/L-42) + cableado (nav/sección/script/GROUPS gate=canManageUsers). Revisión adversarial abortó por 529-overload → auto-verificada. SIGUIENTE PASO 4 (admin-users). | 43671 |
| §217 | **FIX seguridad: ningún users.edit/users.create MINTA un dueño (vector §215.7) ⟦OPUS-4.8⟧** — §212 cerró degradar-al-dueño-EXISTENTE pero faltaba el simétrico: un `users.edit` podía escribir `permissions:['*']`/`roleId:system_super_admin` a un 3º o a SÍ MISMO → auto-escalada. Helper `isOwnerDoc()` + guard `!isOwnerDoc(request.resource.data)` en create+update (solo Admin SDK minta dueños). Test 217-mint (7 casos) — suite rules 80/80. Deploy ✓. | 43687 |
