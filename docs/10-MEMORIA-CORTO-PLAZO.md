...
> 🎯 **RETOMAR = E6 cutover.** **§209** (verif. INICIAL V6 + fix walk-in blocker `createManualLead`/reasignar; **rules DEPLOYADAS**; e6 5/5·198/198) + **§210 gates ②/③ (5/6)**: `js/admin/admin-cutover-gates.js` (aditivo/reversible, Ocultar≠Borrar) oculta+redirige al portal reseñas·banners·marcas·atributos·aliados en 3 superficies. Todo **staged para el push del dueño** (el clásico solo va a prod al push). ⏳ **Falta del dueño**: `git push`+merge a main (§209 code/dist + §210 gate) + **verificación EN VIVO** del lote V6 (verif-división · §198 F39 v2).
> **SIGUIENTE tras verif. viva**: añadir `vehicles:'vehiculos'` a `MIGRATED` (gate ③ completo — hoy EXCLUIDO por R-12/R-13: paridad CI `generate-vehicles`) → **④ RBAC** §193.4 → cutover (paso 24+: stub redirect global, NUNCA borrar admin.html). dealers FASE 2 FROZEN (TODO-25).
