...
| **TODO-38** | **🎨 REDISEÑO TOTAL F4/F5 ⟦OPUS-4.8⟧** — Bot ALTOR (widget) + ALTOR Hub (chat interno admin): de "arcaico" → moderno/premium/responsive/+funciones. **Módulo paralelo v2 flag-gated** (v1 intacto, A/B real), engine-agnóstico (LLM al final). Convoca comité+Gemini+arquitecto+caza-bugs+validador+skills/tools de diseño. **PLAN documentado** → spec `2026-06-24-EPIC-F4F5-rediseno-total-altor-PLAN.md` (Fases A-F). **SESIÓN NUEVA** (disparador en el plan). | 🆕 plan documentado | sesión nueva: Fase A |
