...
const CACHE_VERSION = 'v20260510350000'; // §36.4 — Console cleanup: 3 fixes de fondo: (1) admin-sync.js updateStats null guards (stats genericos eliminados en §27.2 causaban TypeError null.textContent en cada onSnapshot de vehiculos+marcas+appointments). (2) RTDB heartbeat con uid en payload para satisfacer reglas .validate cuando el nodo fue removido por race con cleanup. (3) Lucide seedling reemplazado por package-plus (no existe en v0.468, generaba ~30 warnings cascada cada refreshIcons global) // §35 ADR-035 PERF KILL — admin-perf-kill.css cargado ULTIMO purga 5 anti-patterns acumulados: (1) `* { transition }` global aplicado a TODOS los elementos del DOM → reemplazado por transitions selectivas en interactivos. (2) 18 animaciones `infinite` decorativas (visHeaderBreathe, visHeroOrb, visSparkle, visSkeletonShine, novaShimmerLoading, novaBadgeUrgentPulse, voicePulse, dictatePulse, etc.) → keyframes redefinidos sin movimiento. (3) backdrop-filter en cards (multiplicaba costo GPU x N elementos) → reemplazado por background solido tinted. (4) view-transition con filter:blur(4px) en .section.active → keyframes visSectionIn/Out/VT redefinidos sin blur. (5) admin-restoring backdrop-filter:blur(2px) inline en admin.html → eliminado. JS: admin-visionary-fx.js + admin-visionary-master.js + admin-nova-fx.js convertidos en stub APIs (eliminan pointermove/cursor-follow listeners + 3 MutationObserver subtree:true). admin-empty-states-autoupgrade.js sin MutationObserver. FIX scroll: sidebar overflow-y:auto (era visible bloqueando) + .section.active overflow:visible+height:auto (admin.css legacy ponia overflow:hidden sin altura). Cache bump force refresh clientes.
