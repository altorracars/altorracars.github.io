...
| **TODO-44** | **Fiabilidad del cerebro ⟦OPUS-4.8⟧** — auditoría §257 + cura **M-22**. **Check #16 (`verificado-vivo`/stale) IMPLEMENTADO en kernel + PROPAGADO byte-idéntico ×4 ✅** (26/06, sha `4905D566`; cars+bersaglio+inmobiliaria commiteados; insema: kernel alineado EN DISCO, commit pend por 4 problemas pre-existentes SUYOS en `cerebro/todo-32`). **RESTA**: adopción (marcadores `verificado-vivo:` en afirmaciones de realidad) + reconciliación exhaustiva (Storage/índices/289 ADRs). Absorbe TODO-33. | 🔄 mecanizado ✅ | adopción + insema |
