...
| **TODO-37** | **🔴 P0 leads anónimos perdidos ⟦OPUS-4.8⟧** — gate guest→`updateSoftContact`→regla UPDATE `solicitudes` solo-admin→400→`.catch` mudo→lead perdido (cazado por validador, cruce bot↔CRM). **Decisión Fuerte** (spec `…TODO37-leadloss…`): verif✅+comité✅ (D bleed-stop + C callable `enrichLead`>B) · ⏳ Gemini→impl por fase. | 🔄 Decis.Fuerte | Gemini+go |
