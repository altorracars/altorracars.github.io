...
| §271 | **OLA 2.6 tren de seguridad P2 ⟦FABLE-5⟧** — 10 ítems rules+functions+RTDB: isOwnerDoc 3-formas (cierra minteo dueño legacy vía `rol`) · diff-guard usuarios (rol privilegiado/dataScope whitelist/roleId+`roleAssignedBy` anti-spoof) · onUserRoleAssigned valida SUBSET del asignador con rollback (cierra escalada indirecta por asignación de rol) · config least-privilege por docId · SEC-06 events/automationLog/conciergeChats (create whitelist + BLOCKLIST claims cliente) · mensajes create:false · kb.usageCount +1 · departments.userCount server-only · subscriptions.email regex · RTDB presence sin anónimos + typing por sesión reclamado por uid · isOwnerData COMPARTIDA (3 copias→1) · PAT-del-cliente fuera de triggerSeoRegeneration. Suite 366/366 (+19). Anti-pattern: guard demasiado ancho rompe contrato → apuntar al VALOR privilegiado. | 44402 |
...
| §275 | **OLA 2.12a fusión Base de conocimiento (tabs) + cutover DIFERIDO ⟦OPUS-4.8⟧** — wrapper `conocimiento.ui.js` (tabs FAQs\|Sin respuesta, reusa segmented del 360; handoff "Crear FAQ" in-place vía `onSwitchToKb`) fusiona cerebro+unmatched en 1 entrada del sidebar; rutas legacy retiradas (any-of perm). Verificado E2E mock (sidebar 1 entrada, switch, modal prellenado, 0 errores). `a070ef9e`. **Parte B (borrado `_legacy/admin.html`+9 CSS) DIFERIDA**: censo W-12 → fallback FUNCIONAL + `_legacy` versionado a propósito (rollback) + dueño recién en vivo → doctrina guardián manda; gate dueño (validación-live + confirmación). Anti: `admin.html` raíz = REDIRECT vivo (functions/FCM/manifest) ≠ `_legacy/admin.html`. | 44442 |
