...
- [x] **(3d) Smart suggestions + Resumen handover (LLM summary DIFERIDO=saldo) — verif `?mock=1`**. Suggestions heurísticas por keyword (degradadas: sin AltorraAI/NER/KB) que llenan el composer + modal "Resumen" local (cliente/conteos/estado/últimos msgs + copiar) con nota IA-diferida. **→ F-4 COMPLETO.**
