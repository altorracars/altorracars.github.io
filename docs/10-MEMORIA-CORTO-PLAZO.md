...
> ✅ **TODO MERGEADO a `main`=`bc58234`** (PR #865+#868, 19/06) — ④a PASO 2-4 + §216/§217 + gobernanza Antigravity + doctrina git §2, en producción (cron bumpeó cache→05). Resta dueño (admin clásico, live): **Ctrl+Shift+R → crear Departamentos → asignarlos a usuarios → 1 clic "Backfill niveles"**. Vender=Pipeline (fin.=TODO-25). dealers F2 FROZEN.
> 🏗️ **④ RBAC departamental (§193.4)** — blueprint ④a (bóveda) = **SSoT** (LEER al retomar). **PASO 0-4 + §217 DEPLOYADOS** (§213/§215/§216/§217/§218): catálogo Departamentos (DUPLICADO `rbac-catalog.js`↔`functions`) · callable backfill + botón · rules `departments/` · UI `admin-departments.js` · no-mint-dueño (`isOwnerDoc`) · asignación depto/nivel/scope a usuarios + CF `onUserDeptChanged` (`userCount`). ⏳ **dueño: 1 clic "Backfill niveles" tras merge**. **SIGUIENTE PASO 5** (`onUserRoleAssigned` copia `role.nivel`→`user.nivel` si falta, sin pisar en resync) → **PASO 6** (paridad dual-portal admin-app + cache bump §4). **④b** (scoping `inDeptScope` + `nivel`→int) GATEADO: Gemini + decisión negocio con ④a VISIBLE.
...
> - **19/06 ⟦OPUS-4.8⟧ (sesión maratón, 8 commits cars + 1 inmob, TODO en `main` PR #868)**: ④a RBAC **PASO 2-4** (§215 callable backfill + §216 departments rules+UI + §218 asignación a usuarios + CF `userCount`) · **§217 FIX SEGURIDAD** (cerrado vector escalada `users.edit`→mintar-dueño) · **doctrina git §2 corregida** (Claude commitea+pushea; dueño solo mergea — M-12 reincidencia) · **gobernanza Antigravity = read-only** en cars+inmob (§15+§G.2; bersaglio vía su chat) · bóveda commiteada (crudos+blueprint §212-§218). **SIGUIENTE: PASO 5** (`onUserRoleAssigned` copia `role.nivel`→`user.nivel` si falta) + **PASO 6** (paridad dual-portal admin-app + cache bump §4) → ④b GATEADO (Gemini+negocio con ④a visible). PREV: E6 cutover 6/6 (§209-§214) · Gemini UNIFICADO §208 (→TODO-28/29/30).
