...
> **🤝 TODO-54 PageSpeed (§296, spec `2026-07-08-pagespeed-audit-plan.md` = SSoT)** — 08/07 3ª ⟦OPUS⟧ dieta-JS: **2.1b ✅ `3b11ab10`** (diferir auth+SDKs; `auth/iframe.js` off-critical; nuevo `window.dbReady`; home-only) · **2.3 ✅ `73aadeaa`** (bot/IA ~18 archivos a idle) · **2.4 ✅ `c0d19faf`** (GSI a idle; NO FedCM-off ni solo-login → preserva One-Tap) · **2.6 ✅ `c0d19faf`** (reflujo parallax: scrollY al rAF). Verif preview OK (parallax visual = live). **RETOMAR desde 2.5** (subset lucide, build) · **1.3** (CSS crítico inline, ~900ms, gate FOUC). 🔻 2.2 diferido (main.js:982 ya idle P14). Sin bump (js/core+public/home=networkFirst/SWR).
...
| **TODO-54** | **⚡ PageSpeed perf home ⟦OPUS⟧ (08/07)** — spec `2026-07-08-pagespeed-audit-plan.md` (SSoT). **✅ Fase 0·1.1a·1.2a·2.1a·2.1b·2.3·2.4·2.6** (§296). RESTA: **2.5** subset lucide (build) · **1.3** CSS crítico (gate FOUC) · **1.1b** animaciones (gate visual) · 1.2 fuentes (🚫 equipo) · F3 minify · F4 Cloudflare (dinero). **2.2 🔻 diferido** (ya idle P14). | 🔄 impl | 2.5+ |
