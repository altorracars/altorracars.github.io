...
>
> 🎨 **REDISEÑO portal W-11 — F1 EN CURSO** (spec `…portal-crm-rediseno-W11.md`): **F1(a) selección múltiple + acciones masivas en Bandeja = BUILT + VALIDADO LIVE ✅** (build prod nuevo `index-G4eiZWxO`, 26/06: 2 sel→barra→limpiar, 0 errores). Hallazgo: la Bandeja real ya era fuerte → no hacía falta tabla densa; el gap era multi-select. **F1(b) "Nuevo lead" en topbar global ✅** (`587737b`) + **F1(c) drawer móvil off-canvas accesible ✅** (`da84051`: hamburguesa+backdrop+focus-trap+Escape+cierre-al-navegar; tabulabilidad por CSS `visibility` NO `inert` → sin fallo de cruce-breakpoint, L-55; desktop intacto verif 1280px). **F1 COMPLETA** (a✅LIVE · b·c built, falta validación LIVE dueño). SIGUE **F2** (chasis: `<details>`+fix `setActive`+iconos SVG; palette Ctrl+K diferido). Aliados=aliado (§205 ok; comercial→TODO-25). Bot LLM=saldo. (Build/cache → `05`.)
