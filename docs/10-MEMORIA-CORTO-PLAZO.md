...
| **TODO-34** | **REESTRUCTURA bot ALTOR → LLM-only + captura-CRM ⟦OPUS-4.8⟧**. **Diseño ✅ + Gemini red-team ✅** → spec `superpowers/specs/2026-06-22-TODO34-…` + bóveda `22d52a9`. **Hallazgo: el bot HOY NO está conectado al CRM** (suposición del dueño era falsa; grep=0 puente). Plan 5 fases: F1 puente bot→`solicitudes`origen='altor' (dedup §185) · F2 OFF-de-hierro+Circuit-Breaker · F3 LLM+Tool-Calling+guardas costo/seguridad (anti-jailbreak·App-Check·billing-alerts) · F4 rollout A/B `engine` · F5 sunset `js/ai/`(16). Pend: 2 decisiones dueño (borrar `js/ai/` + techo costo) + implementar. | 🔄 diseño✅ · impl pend | F1+F2 al decidir |
