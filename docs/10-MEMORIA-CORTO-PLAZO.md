...
> 🎯 **RETOMAR = E6 cutover · EN CURSO: diseño ④ RBAC departamental (§193.4).** §209 (fix V6 walk-in/reasignar, rules deployadas) + §210 (gates ②/③ 5/6, `admin-cutover-gates.js`) = **MERGEADOS a main / LIVE** (PR del dueño). ⏳ El dueño aún debe la **verificación EN VIVO del lote V6** → desbloquea añadir `vehicles:'vehiculos'` al gate (R-12/R-13 paridad CI). dealers FASE 2 FROZEN (TODO-25).
> 🏗️ **④ RBAC departamental (§193.4) — DISEÑO en curso** (comité Workflow `winqw8w66`: research Bersaglio + RBAC actual → 5 lentes → síntesis): usuarios por depto (administrativo/contable/marketing/comercial) · roles por cargo + rangos entre deptos · granular ver/editar/agregar/eliminar por sección · dueño=INAMOVIBLE (CEO §70/§72). Mandato §193.4: reusar **Bersaglio** + comité. **Decisión Fuerte → Gemini adversarial antes de implementar; impl. real tras el gate de vehículos (V6 live).** Luego: cutover (paso 24+: stub redirect global, NUNCA borrar admin.html) → E6.5.
