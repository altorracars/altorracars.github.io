...
> 🧭 **FOCO MAESTRO: PLAN UNIFICADO "un-solo-panel-admin"** (dueño 24/06: "muchos planes, sin un camino único, brincamos") → `…PLAN-UNIFICADO-un-solo-panel-admin.md` (survey 5-agentes `wf_668bd020-e80` + comité). **North star**: `admin-app/` (Vite) absorbe TODO **incl. Hub** → apagar `admin.html`. **Re-decisión central**: Hub = módulo admin-app (NO "sandbox-monolito" del verdicto F4/F5) → **red-team Gemini PENDIENTE**. Secuencia los planes dispersos (TODO-38/34/21/23/25/26/01) en §6/§8. **Bot widget público (F4)=track propio; Hub (F5)=parte de la migración.** F4/F5 detalle → fila TODO-38.
...
| **TODO-38** | **🎨 REDISEÑO TOTAL F4/F5 ⟦OPUS-4.8⟧** — Bot+Hub arcaico→premium/responsive. v2 paralelo flag-gated (v1 intacto), engine-agnóstico. Specs `…F4F5-fase{A,B,C,D}…`. **A-C✅** (contratos+defectos · 4 mockups+flujo+comité **D1-D6** · veredicto Gemini **verificado claim-a-claim**: fork L-c·flag sync+command-queue·storage v2 sep·modales CustomEvent·Hub sandbox; refuté bundler-LCP+applyAuthProfile). **D tramo1-2✅** `js/concierge/v2/altor-bot.js` Web Component+Shadow DOM verificado live (D1 ternario·D3 móvil-fullscreen·Free Core árbol botones·DualCore·escalado→live·typing). Free Core buttons-only NO necesita NLU de v1 (resuelto). Mandato autónomo, único stop=Gemini; valido live tras merge+5min. | 🔄 Fase D | sigo: tarjetas vehículo+gate, luego flag XOR |
