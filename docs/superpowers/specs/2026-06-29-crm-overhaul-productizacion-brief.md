...
- **P0-OWNER-DELETE · purga owner-only por sección (P0 #1 del dueño, desbloquea limpieza ZZZ):** backend YA lo permite (rules `isSuperAdmin`/`crm.delete`/`dealers.delete` + `crmPurge.js`) → solo UI. **🔄 EN CURSO:** ✅ `core/confirm.js` (modal premium reusable, shadow/blur/anim) + **Aliados** `deleteDealer` wired (danger modal + aviso de vehículos + optimistic/rollback, verificado). **FALTA:** Pipeline (`deleteDeal`), Contactos/Bandeja (vía `crmPurgeLead` — ⚠️ arreglar drift R8: gatea `rol==='super_admin'` legacy, no las 3 formas) + bulkbar reusable + export por sección.
