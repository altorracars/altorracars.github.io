...
const CACHE_VERSION = 'v20260511100000'; // §47.ter — 4 fixes coordinados: (1) Isla dinámica z-index 9985→9995 para que pueda moverse POR ENCIMA del topnav (9990). (2) En lugar de mostrar "rol técnico" (super_admin/editor/viewer) la isla muestra el CARGO personalizado del perfil (CEO, Asesor, etc.). Ubicación/sección ELIMINADA del detail por solicitud cliente. cargo persistido al sessionData de RTDB en startPresence. (3) Mobile menú hamburguesa real: nuevo botón .atn-hamburger en topnav, visible solo <768px, abre la sidebar drawer (body.is-sidebar-open) reusando admin-v2-sidebar.js. Tabs ocultos en mobile (van a la drawer). admin-v2-sidebar.js bindea TODOS los .hamburger-btn (antes solo el primero). Welcome card con padding/font responsive (1.5rem en <600px, 1.3rem <380px). (4) Multi-store trust persistence (Safari iOS ITP): cookies con SameSite=Lax + Secure + Max-Age=30d en paralelo a localStorage. saveDeviceTrust escribe a ambos. isDeviceTrusted lee de ambos via readStoredTrust (cookie como fallback, repobla localStorage). revokeDevice/revokeAllDevices limpian ambos. // §47.bis — overflow + drag mobile.
