...
1. **NO PERDER lo ya documentado**: toda la información importantísima de CADA proyecto se PRESERVA (límite de guardián: cuarentenar en `_legacy/`, NUNCA borrar). Antes de tocar un cerebro, **inventario exhaustivo** de qué info crítica vive en él + plan de preservación verificado. Esto es condición de aceptación, no opcional. **✅ CUMPLIDO 2026-06-09** → `2026-06-09-inventario-preservacion-cerebros.md` (4 cuerpos catalogados, plan de preservación + mapa de cuarentena + 9 gates de verificación). Esa foto es la **línea base inmutable** del Mandato 3.
2. **Verificar AMPLIA y HOLÍSTICAMENTE cómo MEJORAR el cerebro** (con el comité de expertos = **Comité #1**, durante): no solo reorganizar las 4 capas, sino auditar el cerebro entero (gobernanza, reflejos, linter, economía de contexto, gaps) buscando mejoras de fondo. La fusión Bersaglio→cars (§168/§169 detectó mejoras: §3.7, 60-WORKFLOWS, INSTALACION-CEREBRO, check #6, §15 generalizado) es el punto de partida, no el techo. **Insumo: §9 del inventario (15 semillas dedupadas).**
3. **🆕 Comité de VALIDACIÓN FINAL (= Comité #2, al CIERRE)**: tras TODA la implementación + pendientes, convocar un segundo comité que valide cómo quedó el cerebro EN TODO SENTIDO, contrastando contra el inventario (Mandato 1) como línea base: oportunidades de mejora · errores · **huérfanos** · **vacíos** · **pérdida de memoria** (neurona-por-neurona) · **riesgo de agotar contexto** · **lectura fluida por los 3 proyectos sin perder contexto** · **sin cruce de información entre proyectos** (línea roja INSTANCE) · voz adversarial de **riesgos no estimados**. La macro-tarea NO se cierra hasta su visto bueno.

**🔒 INVARIANTE DURA (cero pérdida de conocimiento):** cada repo acumula TODO su conocimiento en sus propias neuronas; la extracción/compartición del KERNEL debe ser **ADITIVA y loss-free** — NUNCA arrancar conocimiento de ninguna neurona. Riesgo concreto: portable y específico viven mezclados en los mismos archivos (CLAUDE.md de cars = §G portable + §1 específico; §G editado in-place §118) → al congelar el KERNEL desde bersaglio (PASO 2) y promover por copia (PASO 4), la reconciliación debe ser **unión del texto más maduro de AMBOS lados, jamás reemplazo ciego**. Verificable por los 9 gates del inventario.

## 8 — Punteros
- **Inventario + plan de preservación (Mandato 1, línea base):** `docs/superpowers/specs/2026-06-09-inventario-preservacion-cerebros.md`. ⚠️ Su §8 **corrige el §R6 de abajo**: verificado leyendo cada SKILL.md, `crm-architect` es UNIVERSAL (el spec se equivocó), `arquitecto-software` es la contaminación cruzada más grave (bloque "En el proyecto Bersaglio" dispara en cars/inmobiliaria), `legal-colombia` es acoplamiento BENÉFICO (Colombia ayuda a bersaglio joyero). El acoplamiento dañino real = rutas `docs/NN-*.md` hardcodeadas.
- Skills: `~/.claude/skills/` (global; las 4 de Altorra ahí — ver §R6, **matizado por el inventario §8**). Catálogo cars → `docs/skills-inventory.md`.
