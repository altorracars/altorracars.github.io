...
> ⚡ **BLOQUE DE DISEÑO §150 — DROPDOWN DEPURADO (§150.f, 2026-06-02):**
> Bloque §150 (consistencia de diseño) **completo y consolidado en ADR §150 (`99`)**: near-black `#0D0B09`, skip-link removido (§48=5/6), QuickTools fijo, dropdown cross-page por hover, **§150.d** fix layout (panel 120→580px: `*{max-width:100%}` global acotaba `.nav-dd` → `.nav-dd-pro{max-width:none}`; L-23), **§150.e** quitado "Camionetas" duplicado.
> **§150.f (esta sesión, Opción A) — dropdown single-column**: el cliente reportó "Nuevos/Usados fallan" en `busqueda.html?tipo=nuevo`. **Causa (RCA, render+código):** `busqueda.html` **NO lee el query param `?tipo=`** → mostraba los 27. **Pero el filtro nuevo/usado YA existe** en el panel ("Tipo de Vehículo" = `#tipoSelect` con opciones `["", "nuevo", "usado"]`; `database.js` filtra `v.tipo===filters.tipo`). Modelo: `vehicle.tipo`=condición, `vehicle.categoria`=carrocería (→ L-24). **Fix (aprobado):** eliminada la columna "Por condición" del dropdown → queda "Por categoría" (SUV/Sedán/Pickup/Hatchback) + **CTA "Ver todos los vehículos"→busqueda**; panel `.nav-dd` 580→300px single-column. **Render-verificado** (1440px): 5 enlaces, sin Por condición/Nuevos/Usados, CTA dorado, panel 300px, sin overflow. `busqueda.html`/su JS **intactos**.
> **GIT**: §150→**§150.e commiteados** (§150.e=`56bd195`, §150.d=`85972ab`, §150.c=`40605a1`). **§150.f en working tree SIN commit** → código: `index.html` + `snippets/header.html` + `css/home/chrome-redesign.css` + `service-worker.js` + `js/core/cache-manager.js`; + docs (`10`/`05`/`99`/`00`/`30`). Cache `v…602130000`(§150.e)→working **`v20260602140000`**(§150.f).
> **PRÓXIMOS PASOS**: (a) cliente commitea §150.f (mensaje listo abajo) + Ctrl+Shift+R en prod. (b) Con eso **bloque §150 CERRADO** — sitio sin pendientes de código (resta input externo: /cartagena, tráfico CSAT). (c) Rama `refactor/estructura` adelante de `origin/main` (`8da557a`) → cliente mergea por PR (ojo cron↔cache L-02). (d) Opcional menor: sincronizar la ref de diseño `Home.jsx` (NO shippeada) con los cambios del dropdown, si el cliente quiere.
