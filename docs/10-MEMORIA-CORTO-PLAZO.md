...
> 🔜 **TODO-34 — BLOQUEADO en 3 decisiones dueño** (presentadas 23/06): (1) bot **LLM-puro vs HÍBRIDO** (recomiendo híbrido por costo/seguridad) · (2) techo costo USD/mes · (3) borrar `js/ai/`. Tras decidir → F1 (puente bot→CRM) + F2 (OFF server-side breaker Firestore). Spec `…2026-06-22-TODO34-…` (con §Verificación 23/06). ✅ **Contratos verificados (23/06)**: claim "colisión anónimos `sanitizeContactId`" REFUTADO; **GAP real = la ingestión RECHAZA leads sin email/tel → anónimos se PIERDEN (no se fusionan)** → F1 debe capturar handle antes de escribir `solicitud`; consent Ley 1581 ya plomeado (`mapConsent`). Decisión Fuerte → skill **`proceso-decision-fuerte`**.
