...
> **▶️ SIGUIENTE (disparador: "continúa"): FEEDBACK DUEÑO PRIMERO** — (1) **XLSX corporativo** (§270.11 resta: tablas con diseño+logo ALTORRA+gráficas en reportes; exceljs via dynamic import para no engordar el bundle; aplicar a los 6 exports) → (2) **borrado de CONTACTOS owner** (`crmSuppressContact` 72h existe SIN UI → cablear al portal + variante inmediata owner-only con limpieza de dedup) → (3) OLA 2.6 seguridad P2 → 2.7-2.12. Cadencia: build → preview mock → commit(+dist!)+push+merge; emulador solo si tocas rules (L-63).
>
