...
(function () {
    ...
    /* ─── Configuración ─────────────────────────────────────────── */
    const APP_VERSION = '20260511280000'; // §57.7 — admin _chatsUnsub globalmente activo (no auto-cancel on section change) + heartbeat 30s self-healing → admin recibe chats nuevos en tiempo real sin refresh — §57.6 _resetting flag + Descargar en admin close + diag — fix unificado cleanSessionAndRender helper — 4 bugs tiempo real coordinados — finalCloseAndCleanup robusto + admin realtime detail re-render — fix botón Cerrar chat no respondía — refactor flow finalización chat cliente — fix definitivo radicado push Telegram — race condition radicado fix + disable_notification explícito — onChatEscalatedTelegram con onDocumentWritten + sin pin region + logs verbosos diagnostic
