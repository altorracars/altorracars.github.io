...
- **EN CURSO: SP-5.2 body migration soft pages — DECOMPUESTO en 3 lotes** (es un PORT;
  ...
  - **SP-5.2.c**: App-like (contenido generado por JS — reescribir el render a cinematic
    ...
    - **SP-5.2.c.2 COMPLETO** (perfil §131 + favoritos §132). **SP-5.2.c.3 comparar EN CURSO** (brainstorm hecho, spec en `docs/superpowers/specs/2026-05-30-comparador-cinematic-design.md`): Paso 1 flotante ✅ (§134, working tree). **← RETOMAR: Paso 2 — reescribir `comparar.html` → Compare.jsx** (slots A/B + VS + picker inline + diff dorado + veredicto, máx 2). Luego c.4 simulador (sprint propio, 2389 líneas).
