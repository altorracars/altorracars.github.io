...
> ⚡ **BLOQUE DE DISEÑO §150 — DROPDOWN LAYOUT RESUELTO + DEDUP (§150.d + §150.e, 2026-06-02):**
...
> **§150.d (esta sesión) — FIX del layout del dropdown que quedaba pendiente de diagnóstico**: causa raíz hallada por **render local** (preview http-server, ya disponible) + verificada en código: la regla universal `* { max-width:100% }` de `style.css:6450` acotaba el `width:580px` de `.nav-dd` a su containing-block (`.nav-dd-wrap` position:relative ≈120px = ancho del trigger) → el panel colapsaba a 120px y la grilla 2-col (~316px) se desbordaba del panel near-black. **Fix:** `.nav-dd-pro { max-width:none }` en `chrome-redesign.css` (vence al `*` por especificidad 0,1,0; `*` no es !important). **Verificado renderizando**: 120px→**580px**, grilla 2×263px, contenido dentro del panel, sin overflow de viewport; mega-dropdown es **desktop-only** (`nav.alt-nav-links` display:none en móvil → cero regresión móvil). Pura CSS, markup/JS intactos. → L-23 en `30`.
> **§150.e — eliminado enlace DUPLICADO "Camionetas" del dropdown** (pedido del cliente: la categoría Pickup ya cubre camionetas). El enlace "Camionetas" de la columna "Por condición" apuntaba a `vehiculos-pickup.html`, mismo destino que "Pickup" en "Por categoría" → redundante. Removido de `index.html` + `snippets/header.html` (el snippet propaga a las 64+ páginas con header dinámico; las generadas NO lo tienen inline → sin regen). Render-verificado: dropdown con **7 enlaces (4+3)**, sin "Camionetas", panel 580px intacto. (El `vehiculos-camionetas.html` redirect legacy queda intacto, es invisible.) (La ref de diseño `altorra-cars-design-system/.../Home.jsx` aún tiene el enlace — NO shippeada; pendiente decisión del cliente si sincronizar.)
> **GIT**: §150/§150.b/§150.c/**§150.d YA commiteados** (último §150.d=`85972ab`, incluyó `chrome-redesign.css` + L-23 en `30`; §150.c=`40605a1`). **§150.e en working tree SIN commit** → código: `index.html` + `snippets/header.html` + `service-worker.js` + `js/core/cache-manager.js`; + docs (`10`/`05`/`99`/`00`). Cache commiteada `v…602120000` (§150.d) → working **`v20260602130000`** (§150.e).
> **PRÓXIMOS PASOS**: (a) cliente commitea §150.d+e (mensaje listo abajo) + Ctrl+Shift+R en prod para confirmar el dropdown abierto a 580px sin "Camionetas". (b) Con eso **bloque §150 CERRADO** — sitio sin pendientes de código (resta input externo: contenido /cartagena, tráfico CSAT). (c) Rama `refactor/estructura` diverge de `origin/main` (`8da557a`) → el cliente mergea por PR (ojo conflicto cron↔cache L-02).
