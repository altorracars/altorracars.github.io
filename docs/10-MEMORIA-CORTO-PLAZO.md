...
- **EN CURSO: SP-5.2 body migration soft pages — DECOMPUESTO en 3 lotes** (es un PORT;
  ...
  - **SP-5.2.c**: App-like (contenido generado por JS — reescribir el render a cinematic
    ...
    - **c.2b favoritos ← RETOMAR AQUÍ** (959 líneas): NO es token-remap (favoritos casi no usa tokens propios) → es reescritura de render estilo §130 + CSS de cards a portar. hero+stats (`#favHero`/`#favStatCount`) + controles (`#favSearchInput`/`#favSortSelect`) + grid `#favoritesGrid` que usa **`renderVehicles` de `render.js`** (COMPARTIDO — NO tocar; para cards cinematic: render LOCAL en el JS inline portando `.cat-card*` a soft-redesign.css, o usar `.cin-av-card`) + 5 estados (skeleton/auth-gate/empty/no-match/badges del watcher) + JS inline (~640 líneas). css: `favorites-empty-fullpage.css`.
