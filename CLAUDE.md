...
## 33. ADR-033 ALTORRA ADMIN V2 — Reconstrucción Total (2026-05-10)

> **Standard**: Apple macOS Sonoma + HarmonyOS NEXT.
> Resuelve el bug crítico de scroll, FOUC visual al recargar y la
> brecha gigante en monitores HD/4K.
>
> Scope: ÚNICAMENTE admin (admin.html, admin*.css, admin-*.js).
> Sitio público intocable.

### 33.1 Sprint V2-1 — Layout core inquebrantable

**Bug crítico resuelto**: al hacer scroll, la página completa baja y
el menú lateral desaparece de la vista.

**Causa raíz**: `body { overflow-y: scroll }` heredado del sitio
público. El BODY scrolleaba en vez de `<main>`. La sidebar usaba
`position: relative` y se quedaba atrás.

**Fix arquitectónico** (`css/admin-v2.css` ~480 líneas):
...
Resultado: el body **JAMÁS** scrollea. Solo el `<main>` lo hace.
Sidebar siempre visible, jamás desaparece.
...
### 33.2 Sprint V2-2 — Anti-FOUC con preloader masking

**Bug visual resuelto**: al refrescar, la página carga con baja
resolución y líneas extrañas antes de aplicar el CSS (efecto "TV
glitch").
...
- Doble `rAF` garantiza que el browser completó al menos 1 paint
- Fade-in suave elimina el "TV glitch"

### 33.3 Sprint V2-3 — Sidebar Island HarmonyOS + Collapse

**Sidebar como "isla" moderna**:
- Background translúcido vía pseudo `::before` con `backdrop-filter: blur(48px) saturate(200%)`
- Border 1px white/0.08 (refractive)
- Border-radius 20px (HarmonyOS NEXT)
- Margin de 14px del borde izquierdo (separación elegante)
- Box-shadow stacked + inset highlight

**Comportamiento collapsable**:
- Toggle ⌘+B / Ctrl+B (ignora si focus en input/textarea)
- Click en `.sidebar-collapse-btn` (auto-attach)
- Persistencia en `localStorage.av2_sidebar_collapsed`
- Width 268px → 76px con transition spring 0.36s
- Labels fade-out + width:0 cuando collapsed
- Tooltips automáticos en collapsed (data-tooltip-text auto-rellenado)
- Mobile: drawer slide-in spring + scrim backdrop + Esc cierra

### 33.4 Sprint V2-4 — DOMBus + ScrollBus consolidados
...
mutation triggeaba 4 callbacks → 4x repaint cost.

**Solución** (`js/admin-v2-core.js`):
...
- Se ejecuta automáticamente al salir de cada sección via hook al
  `AltorraSections.onChange`
- Resuelve memory leak progresivo de los 25 `onSnapshot`

**Auto-scroll-to-top en section change**: cada vez que el admin
cambia de sección, el `<main>` scrollea al top con `behavior: smooth`
(o `auto` si `prefers-reduced-motion`).

### 33.5 Sprint V2-5 — Fluid sections + Performance

**Override masivo de inline styles legacy**:
- Selectores de atributo `[style*="max-width: 1280px"]` etc. → forzar
...
- Cards (kpi/stat/reports/workflow/pipeline/nba) → `contain: layout style` + `content-visibility: auto`
- Tabla rows → `contain: layout style`

Resultado: cambio de sección **instantáneo**. CPU idle al scrollear.

### 33.6 Archivos creados / modificados

**Nuevos**:
- `css/admin-v2.css` (627 líneas)
- `js/admin-v2-bootstrap.js` (anti-FOUC reveal)
- `js/admin-v2-sidebar.js` (collapse + ⌘+B + mobile drawer)
- `js/admin-v2-core.js` (DOMBus + ScrollBus + SectionCleanup)

**Modificados**:
- `admin.html`:
  - `<head>`: inline `<style>` anti-FOUC + `html.av2-loading` script
  - `<head>`: link `css/admin-v2.css` cargado ÚLTIMO
  - `<body>`: preloader `<div id="av2Preloader">`
  - 4 `<script>` defer nuevos
- `service-worker.js` + `js/cache-manager.js`: cache bump v20260510270000

### 33.7 Compromisos de no-regresión cumplidos

- ✅ JS de la app NO se tocó (admin-vehicles, admin-crm, admin-concierge siguen idénticos)
- ✅ Sitio público — **CERO** modificaciones
- ✅ Datos Firestore intactos
- ✅ `prefers-reduced-motion` respetado
- ✅ Mobile drawer con touch
- ✅ A11y AAA (focus rings, keyboard nav, Esc cierra drawer)
- ✅ Cache versionado

### 33.8 Test E2E

1. **Scroll fix**: refrescar admin → scrollear hacia abajo → ✅ sidebar SIEMPRE visible
2. **FOUC fix**: refrescar (Ctrl+Shift+R) → ✅ ve solo preloader → fade in suave al admin
3. **Sidebar collapse**: ⌘+B (Mac) o Ctrl+B → ✅ sidebar colapsa a 76px con animación spring + tooltips aparecen al hover
4. **Layout fluid**: redimensionar a 1920px → ✅ contenido aprovecha todo el ancho
5. **Mobile drawer**: resize a 480px → click hamburger → ✅ slide-in con scrim
6. **Custom scrollbars**: hover sobre tabla con muchos vehículos → ✅ scrollbar dorado tenue 8px
7. **Section transition**: click otra sección → ✅ scroll-to-top suave + content-visibility instantáneo
8. **Performance**: 35 setInterval + 25 onSnapshot ahora con cleanup hooks → CPU baja
