...
| **TODO-29** | **Cerebro: endurecer el lazo (Nivel-2 §207) ⟦OPUS-4.8⟧** — reincidencias cuya cura ES gate, no doctrina (M-16); todas en el kernel `brain-check.mjs` (×3, mismo canon que TODO-28): boot/cache-vs-`origin/main` · techo duro de boot · anclas §G. + contrato-template→Gemini · shard 99a/99b · `ignoreDirs`. → §207 + bóveda. | 🔄 | kernel ×3 → bersaglio |
