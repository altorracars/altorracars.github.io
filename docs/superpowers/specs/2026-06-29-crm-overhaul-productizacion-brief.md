...
- **DIRECTIVAS DEL DUEÑO (29/06 d):** **(a) ✅ accent picker ELIMINADO** ("cambiar color del panel = irrelevante") — oro fijo `#D4A85A` (commit `0d3aa02e`). **(b) 🔲 PERFIL paridad con el admin viejo (PENDIENTE, cliente):** el perfil nuevo (`modules/perfil/perfil.ui.js`) **ya tiene** nombre/teléfono/avatar/**documento-cédula**(lock+request)/cargo/cuenta/cambio-contraseña, pero le **FALTA** lo del viejo `js/admin/admin-profile.js` (1118L) + `js/admin/admin-telegram.js` (170L): **① Telegram** (vincular para notificaciones — el dueño lo pidió explícito; flujo SIMPLE: deep-link `https://t.me/{BOT_USERNAME}?start=ASESOR_{uid}` → webhook `linkTelegramChat` YA deployed liga `telegramChatId`; status desde `profile.telegramChatId`; unlink = self-update, campos ya en whitelist rules L251); **② 2FA/MFA** (TOTP/SMS — TODO-43); **③ recovery** (backup codes + security questions, ya en el schema del user); **④ trusted devices**. ⚠️ Scope a confirmar con el dueño: ¿solo Telegram, o también 2FA/recovery? (es reconstrucción multi-archivo). Nota: el perfil nuevo se reorganiza en el rediseño FE final (Fase C) — coordinar para no rehacer.
